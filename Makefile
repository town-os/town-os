...
test-integration: lint test-image btrfs .integration-port registry-populate .cache/registries.conf gitea-populate
	...
	rc=0; sudo -E podman exec -w /test $(PODMAN_CONTAINER) /integration-test -test.v -test.timeout 60m || rc=$$?; \
	exit $$rc
...
clean-btrfs-dev:
	...
	@sudo -E losetup -a 2>/dev/null | grep '$(CURDIR)/btrfs-dev\.' | awk -F: '{ print $$1 }' | while read dev; do \
...
clean-btrfs:
	...
	@sudo -E losetup -a 2>/dev/null | grep '$(CURDIR)/btrfs\.' | awk -F: '{ print $$1 }' | while read dev; do \
		sudo -E umount "$$dev" 2>/dev/null || true; \
		sudo -E losetup -d "$$dev" 2>/dev/null || true; \
	done || true
