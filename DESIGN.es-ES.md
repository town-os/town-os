...
13. **Derivar la imagen del NC** — `quay.io/town/networkcontroller:<tag>`, anulable con `NC_IMAGE`. Se descarga (paso 18), nunca se construye.
14. **Arrancar el refresco de repositorios en segundo plano** — una goroutine consulta cada 5 minutos.
15. **Etapa `boot_dns`: construir el gestor de rolodex y esperar al DNS** **(no fatal)** — Rolodex es un servicio de arranque gestionado por systemd, y a estas alturas ya está respondiendo: arranca antes que el systemcontroller a partir del `rolodex.yml` que escribió la imagen de instalación, porque el controlador no puede descargar ninguna imagen hasta que algo resuelva nombres. **Aquí no se escribe `rolodex.yml` ni se reinicia el servicio** — véase «rolodex.yml es solo arranque; los ajustes se programan por gRPC» más abajo, y [La configuración que se congeló](#la-configuración-que-se-congeló) para el fallo que acabó con aquella escritura. Lo que esta etapa construye es el `rolodex.Manager`, a partir de los ajustes que Town OS posee: `resolution.mode` desde `dns_resolution_mode` (un valor almacenado que no se pueda analizar recurre al predeterminado en lugar de programar un modo que rolodex rechazaría), la lista de reenviadores desde `dns_local_forwarders` (cuando está activo, la lista se descubre a partir de los resolutores del host en cada arranque, de modo que un equipo que cambió de red toma el nuevo sin que el operador toque nada — véase [Reenviadores locales](#reenviadores-locales)), y la lista de bloqueo desde su ajuste almacenado. Todos ellos se empujan al servidor **en marcha** más adelante en este mismo arranque, mediante `ProgramRolodex` en el paso de reconciliación. El contenedor de rolodex se ejecuta con `--net host` y enlaza el DNS directamente a `127.0.0.2:{puerto}`. Después espera a que el DNS esté listo (sondeo de conexión TCP) y configura systemd-resolved para encaminar el TLD a rolodex — **se omite cuando `TOWN_OS_DNS_PORT` ha movido rolodex fuera de `:53`**, ya que una dirección de servidor por dominio en resolved no lleva puerto y enviaría al vacío todas las consultas de ese TLD.
...
17. **Etapa `boot_services`, primer paso: autoactualización** **(no fatal)** — `SelfUpdate` refresca la imagen *propia* del controlador y, cuando la etiqueta bajo la que se ejecuta ha pasado a nombrar otra imagen, pide a systemd que reinicie `town-os-systemcontroller.service` y detiene ahí este arranque (véase *El controlador se actualiza a sí mismo*).
18. **Etapa `boot_services`: descargar las imágenes de contenedor centrales** **(no fatal)** — la imagen del NC, Prometheus, Node Exporter, la imagen de la interfaz, la imagen del almacenamiento de objetos (gfeh), la imagen del ingress y Grafana cuando ese backend está seleccionado, en paralelo mediante `parallelEnsureImages` (una etiqueta **flotante** se vuelve a descargar en cada arranque; una fijada solo se descarga cuando falta — véase *El arranque refresca las etiquetas de imagen flotantes*). Toda imagen que referencie una unidad de arranque pertenece aquí: una unidad cuya imagen no es local la descarga desde dentro de `podman run`, así que su espera de disponibilidad compite con una descarga del registro. gfeh y después el ingress faltaron en esta lista por turnos, y cada uno se leyó como un servicio que simplemente estaba caído. La interfaz de monitorización no necesita entrada propia — en el backend uPlot ejecuta la imagen del NC, que ya es la primera del conjunto.
...
30. **Etapa `ready`: intercambiar el manejador raíz** — el stub de arranque se sustituye atómicamente por el router Echo completo en el listener ya enlazado, así que no se produce ningún parpadeo de puerto y los suscriptores de `/boot-status` (SSE) en vuelo sobreviven al traspaso. `BootStatus.Done()` cierra entonces el flujo. **El sistema ya está listo.**
...
32. **Apagado ordenado** — con SIGINT: cancela el contexto y apaga el servidor HTTP con un tiempo límite de 30 s. Todas las goroutines de fondo salen mediante la cancelación del contexto.
...
Un backend detrás de una ruta programada puede estar roto de dos maneras, y el ingress responde a ambas con la misma página: un **503 que dice que el servicio no está disponible y que se recarga solo cada cinco segundos** (`unavailableRefreshSeconds`, que es también el `Retry-After`) hasta que el servicio vuelve a responder. No se reprograma nada ni se retira ninguna ruta — la dirección sigue enrutada todo el tiempo, así que en cuanto el backend vuelva a tener un listener, la siguiente recarga lo sirve.

Este es el estado corriente de una caja doméstica, no una excepción: un paquete se reinicia al actualizarse, un cambio de ajustes rebota un servicio, un reinicio devuelve veinte contenedores en algún orden. Lo que Caddy dice durante esos segundos es `502 Bad Gateway`, que se lee como *roto* en lugar de como *volviendo*, y no le deja al lector nada que hacer salvo reintentar a mano.

Las dos formas de fallo son mecanismos separados en Caddy, y una implementación que cubriera solo una se dejaría el caso común:

- **El backend respondió, con un 5xx.** Eso es una *respuesta* de upstream, interceptada con un matcher de respuesta y `handle_response` **dentro** del bloque `reverse_proxy` — que es por lo que todo proxy que emite el renderizador abre ahora un bloque, incluidos los de HTTP plano.
- **El backend nunca respondió** — contenedor caído, nombre irresoluble, conexión rechazada. `reverse_proxy` falla y Caddy levanta su propio error, que `handle_response` no ve nunca; eso es `handle_errors` a nivel de **bloque de sitio**. Este es el caso común en una caja doméstica, y el que una implementación centrada solo en el 5xx se dejaría en silencio, porque el 5xx que vigila no llega jamás.

Ambos caminos se renderizan como **snippets** de Caddyfile (`render_unavailable.go`), definidos una vez por fichero e importados por cada bloque de sitio — así la página no puede derivar entre vhosts, y el fichero mantiene un tamaño legible por muchas rutas que sirva la caja. Los snippets se emiten también con un conjunto de rutas vacío: un snippet que nadie importa nunca se analiza como directivas, y un preámbulo que apareciera y desapareciera rebotaría Caddy en la primera ruta.

- **Solo un navegador recibe la página.** Ambos caminos se condicionan a `method GET HEAD` más `Accept: *text/html*`. Un cliente de API — un XHR, un webhook, `curl`, un resolver DoH detrás de un path backend — recibe el estado, el cuerpo y las cabeceras propios del upstream copiados tal cual en el camino de respuesta, y un 503 escueto con `Retry-After` en el camino de error. Entregar HTML a algo que está parseando JSON convierte un backend roto en un segundo fallo, más extraño, dentro de quien lo llamó.

  El paso a través es `copy_response` **a solas**, sin `copy_response_headers` al lado, y esa omisión es estructural: `reverse_proxy` ya ha dejado puestas las cabeceras del upstream en el response writer para cuando corre un manejador de respuesta, así que copiarlas otra vez emite cada una dos veces — `Content-Length` incluida, lo que convierte el mensaje en malformado y no meramente en desprolijo. Parece la directiva compañera evidente, y por eso la fijan tanto una prueba del renderizador como una aserción de integración.
- **La página nombra el servicio desde la configuración renderizada, nunca desde la petición.** La etiqueta es un argumento de snippet sustituido en el momento del import — el hostname de la ruta, ya validado por `validSiteHost`. El bloque de reserva de `:80` se etiqueta en cambio con la constante `Town OS`, porque responde por *todo* host no emparejado: su cabecera `Host` es lo que el cliente haya enviado, y reflejar eso en HTML sería una inyección en el único vhost que acepta cualquier nombre. `safeUnavailableLabel` es la guarda que mantiene eso cierto para un futuro llamador.
...
Las cuatro frases que lleva son entradas del catálogo (`ingress.unavailable.*` en `src/i18n`), no literales en el renderizador: cada idioma que Town OS trae recibe su propia versión de la página, y Caddy elige una por petición a partir de `Accept-Language`.

Eso reproduce lo que la interfaz ya hace, y por la misma razón: **el idioma del lector es el del navegador, no el de la caja.** `I18nContext` resuelve primero una elección guardada, luego `navigator.languages`, y solo recurre al ajuste global `locale` del servidor cuando el navegador pide un idioma que ningún catálogo cubre. La página de reintento resuelve ese mismo orden dentro de Caddy — un `map` sobre `Accept-Language` con una fila por catálogo de idioma, y una fila `default` que lleva el locale de la caja. Un invitado con el móvil en portugués lee portugués en una caja configurada en alemán.

- **Un `map`, y una sola copia de la página — no un bloque `handle` por idioma.** La forma con bloques funciona, y se midió: como un `import` es textual, cada expansión incrusta el selector entero, así que treinta ramas significaban treinta copias de la página en el JSON que se le entrega a Caddy en cada recarga — **115 KB para una sola ruta**, y una caja con veinte rutas habría empujado ~2,3 MB en cada instalación de paquete, edición de página y reconciliación horaria. El map guarda las frases traducidas en una tabla compacta y la página se escribe una vez, leyendo de ella `{townos_title}` y compañía.
- **El camino del 5xx no renderiza nada; lanza `error 503`** al `handle_errors` del propio sitio, que es lo que hace que la página se expanda una vez por bloque de sitio en lugar de una vez por proxy. Verificado: un `error` lanzado dentro de `handle_response` sí llega al manejador de errores que lo envuelve, con la petición original intacta, así que la condición de `Accept` sigue aplicándose.
- **El selector tiene una fila por IDIOMA, no por locale.** Las variantes de país (`de-AT`, `en-GB`, `es-MX`…) derivan de su idioma base y solo sobrescriben lo que ese país dice de otra manera — nada de lo cual son estas cuatro frases — así que una fila para cada una serían treinta expresiones regulares más renderizando la misma página. `TestUnavailableLocalesCoverEveryCatalog` deriva la pertenencia esperada de `i18n.PopulatedLocales()` y falla en ambas direcciones, de modo que un idioma añadido a los catálogos no puede quedarse aquí en inglés sin que nadie se entere.
- **El chino tradicional se empareja antes que el `zh` genérico.** Caddy prueba las filas del map en orden y se queda con la primera que acierta, y `^zh` reclama `zh-TW`, así que el orden equivocado sirve simplificado a Taiwán, Hong Kong y Macao — que parece una traducción correcta a todo el que no sabe leerla. Cada patrón está anclado, ignora mayúsculas y termina en un separador o al final de la cabecera, de modo que `^ja` no puede contestar en japonés al javanés.
- **El nombre del servicio es un verbo de formato, no una concatenación.** `"%s is unavailable"` coloca el nombre en un sitio distinto en japonés que en inglés, así que el orden de las palabras pertenece al catálogo; el renderizador sustituye `{args[0]}` dentro de la frase traducida, y Caddy sustituye ahí el hostname cuando el bloque de sitio importa el snippet.
- **El locale de la caja llega al ingress por la línea de órdenes de la unidad** (`--locale`, `ingressctl.Config.Locale`, leído del ajuste `locale` al generar la unidad). Es un ajuste global de la caja y no estado de rutas, así que viaja con la unidad y no con el empuje gRPC; un cambio de idioma llega al ingress igual que cualquier otro cambio de unidad — una unidad renderizada distinta en la siguiente reconciliación. Un código sin catálogo se ignora en lugar de rechazarse: el ajuste es una fila de texto libre, y el coste de una errata debe ser una página en inglés, no un ingress que no arranca.
- **Cada cadena traducida se escapa para las dos fronteras que cruza** (`caddyHTMLText`): primero es un token de Caddyfile y después el contenido de un elemento HTML. El escapado HTML se lleva por delante la comilla que terminaría el token antes de tiempo; la barra invertida se escapa aparte, porque Caddy la lee como un escape y el paquete `html` de Go no tiene motivo para tocarla.

### Métricas del ingress

El ingress sirve el formato de exposición de texto de Prometheus en un **tercer listener propio** (`ingress.MetricsPath = "/metrics"`, `ingress.DefaultMetricsPort = 9146`), renderizado por `src/metrics` exactamente como lo hace el endpoint del controlador. No puede viajar en los dos listeners que ya tiene: esos son la puerta de entrada de la caja, y un vhost para `/metrics` sería alcanzable por SNI desde la LAN y desde la overlay.

**Se publica solo en `127.0.0.1`** (`-p 127.0.0.1:9146:9146`), la única excepción deliberada a que el ingress sea agnóstico a la interfaz. El scrape nombra cada hostname que la caja sirve y marca los que aún no tienen certificado — un mapa de lo que está publicado y de lo que está a medio aprovisionar — y nada lo autentica. Prometheus corre con `--net host`, así que alcanza la publicación en loopback sin salto por red de podman, igual que los objetivos de node-exporter y del controlador.

**El objetivo del scrape no se recompone en ninguna parte.** `ingressctl.MetricsAddrFor(port)` construye tanto la especificación `-p` de la unidad como `monitoring.Ports.IngressMetrics`, la misma disposición de fuente única de verdad que tiene `rolodex.Manager.MetricsAddr()`. El job se **omite** cuando `INGRESS_IMAGE` está vacío: una caja sin ingress debe informar que no hay ingress, no un router que se lee como permanentemente caído.
...
| `townos_ingress_route_https_ready{hostname}` | gauge | 1/0 por ruta, para que se vea *qué* nombre no puede servir HTTPS |
...
Más las **propias familias de caddy**, obtenidas de la API de admin local al contenedor del hijo (`127.0.0.1:<puerto de administración>/metrics`, construida a partir de la misma dirección que lleva el Caddyfile, de modo que un hijo reubicado no puede dejar el scrape atrás en el 2019) y añadidas a la respuesta tal cual. Van bajo `caddy_*`/`go_*`/`process_*` y no pueden colisionar con las `townos_ingress_*`. Pasarlas es la única forma de que lleguen a Prometheus, porque la API de admin está deliberadamente sin publicar, y describen el proceso que de hecho sirve cada petición que la caja responde. El cuerpo no se vuelve a renderizar: analizarlo de vuelta a familias solo podría perder información (caddy exporta histogramas, que `src/metrics` deliberadamente no modela).

Varias decisiones son el objetivo, no un accidente:

- **`townos_ingress_dropped_total` es la razón de que exista `renderCaddyfileTally`.** Una ruta con un hostname malformado, un backend inutilizable o un matcher de ruta de acceso inválido se descarta **individualmente** en lugar de hacer fallar el render — es decir, se acepta por gRPC, se registra una vez y luego simplemente nunca se sirve. Un renglón de log es el instrumento equivocado: nadie lee el journal del ingress hasta que ya se sabe que algo está roto.
- **Los backends por ruta de acceso se validan una vez por ruta, no una vez por vhost.** Una ruta de página está lista para HTTPS y además es `ServeHttp`, así que renderiza dos bloques de sitio a partir de un mismo conjunto; validar dentro del escritor contaría cada rechazo dos veces.
- **Cada serie de counter se siembra en cero.** `metrics.Render` omite una familia sin muestras, así que un `townos_ingress_reloads_total{result="failure"}` sin sembrar estaría ausente hasta el primer fallo — una alerta que no puede dispararse hasta que el evento que vigila ya ha ocurrido.
- **`townos_ingress_last_reload_success_time_seconds` se omite hasta que una recarga tiene éxito**, en vez de emitirse como cero: `time() - 0` se lee como una configuración de hace 56 años en lugar de como "todavía no hay configuración".
- **Un scrape nunca falla como unidad**, y nunca tumba el router. Un hijo caddy inalcanzable cuesta un gauge, no la respuesta; un listener de métricas que no puede enlazar se registra y se sobrevive, porque negarse a enrutar tráfico por un hueco de monitorización es el intercambio equivocado.
...
- **`Generation`** -- la identidad del socket gRPC que rolodex enlaza al arrancar (dispositivo, inodo, mtime), que cambia exactamente cuando rolodex se reinicia y es por tanto como se advierte un reinicio que se llevó por delante cada ajuste programado. Vacía cuando el socket no está, lo que significa que rolodex no está en marcha.
...
El modo es configurable por el operador en tiempo de ejecución mediante el ajuste `dns_resolution_mode` (`auto` | `recursive` | `forward`; validado por `ValidateDNSResolutionMode`, así que un valor no analizable nunca puede llegar a `rolodex.yml` y dejar el DNS inservible). `main.go` lo lee en `rolodex.Config` en el arranque; un cambio mediante `POST /settings/set` ejecuta `Controller.RefreshDNSResolutionMode`, que programa el servidor en marcha con **`SetResolutionMode`** — sin archivo, sin reiniciar la unidad, sin interrupción del DNS. rolodex rechaza de plano un modo que no reconoce en lugar de recurrir al predeterminado, y por eso el valor se valida antes de enviarlo.

### La configuración que se congeló

`WriteConfig` solía saltarse cualquier `rolodex.yml` cuyo mtime fuera más reciente que el del binario del systemcontroller, con la teoría de que un archivo más nuevo había sido editado a mano. **El archivo que escribe cada arranque es más reciente que el binario de la imagen que lo escribió**, así que desde el segundo arranque en adelante esa guarda coincidía con la propia salida del controlador, y `WriteConfig` se convirtió en una operación nula permanente. `rolodex.yml` quedó congelado en lo que renderizó el primer controlador que llegó a ejecutarse en ese equipo, durante toda la vida del equipo, y ninguna actualización de imagen podía moverlo. `RewriteConfig` existía solo para rodear esto en los cambios iniciados por el operador.

Por tanto, ninguna sección añadida a la plantilla después del primer arranque de un equipo llegó nunca a ese equipo. Eso se envió dos veces — `dnsbl:` (`31f7e80`) y luego `metrics:` (`9689461`) — y la segunda es la razón de que un equipo desplegado no tuviera ningún listener de métricas de rolodex. Prometheus recolectaba `127.0.0.2:9153` cada 15 s puntualmente, allí nunca hubo nada enlazado, las series `rolodex_*` no existieron jamás, y todos los paneles de DNS renderizaban una **gráfica vacía en lugar de un error** — indistinguible de un resolutor ocioso. `systemctl --failed` estuvo vacío y todos los contenedores en marcha durante todo ese tiempo.

No se protegía nada real. Todos los valores del archivo se renderizan a partir del estado de Town OS — puerto de DNS, reenviadores, modo de resolución, ambas listas de bloqueo, el puerto de métricas — y toda ruta en tiempo de ejecución que cambie uno de ellos ya sobrescribía el archivo de todos modos (eso era `RewriteConfig`). Una edición a mano antes sobrevivía hasta el siguiente cambio de ajustes; ahora sobrevive hasta el siguiente arranque.

**Esta sección es historia.** `WriteConfig` y `RewriteConfig` ya no existen, y Town OS no escribe ningún archivo de configuración de rolodex: los ajustes que antes renderizaban se programan ahora en el servidor en marcha, y el archivo que queda pertenece a la imagen de instalación. Véase «rolodex.yml es solo arranque» más abajo. Se conserva porque el fallo que describe — una escritura de configuración que en silencio no hacía nada, y que solo aflora como una función que nunca funcionó — es la forma de error que este subsistema no deja de producir.

Esas pruebas de regresión se fueron con el código que guardaban; lo que las sustituye está en la sección siguiente.
...
**Corregido aguas arriba.** rolodex lanzaba su sondeo de «se llega al :53» solo cuando una lista de bloqueo estaba habilitada *en su fichero de configuración*, así que al dejar de escribirse las listas ahí el sondeo no arrancaba nunca y una lista configurada se degradaba justo en las redes para las que el sondeo existe. Ahora es `DnsblChecker::resolver_availability_loop`, lanzado incondicionalmente y gobernado por la marca de habilitado *en tiempo de ejecución* del comprobador — la misma forma que ya tenía el bucle de recuperación de la resolución automática. Una lista activada por gRPC recibe el sondeo en segundos.
...
**La imagen y el archivo son un par emparejado, y nada negocia entre ellos.** `rolodex.yml` lo escribe `../install` y lo analiza la imagen de rolodex que el equipo esté ejecutando, sin apretón de manos de versión en ninguna dirección: serde rechaza de plano un campo desconocido o ausente. Un campo que es obligatorio en la revisión de la imagen y falta en el archivo, o que está en el archivo y la imagen no conoce, es un `failed to parse config file` duro en el arranque, y bajo `Restart=always` eso es un bucle de caídas con el DNS caído para todo lo que hay en la caja. Ya ocurrió una vez en el banco de pruebas: la imagen publicada era anterior al renombrado `rbl` → `dnsbl`, y todas las pruebas que arrancaban un rolodex real morían con `missing field \`rbl\`` mientras todas las que usaban el mock pasaban. La regla que se sigue es que el `rolodex-config.sh` del repositorio de instalación y la imagen publicada de rolodex se mueven **juntos**, y que subir `rolodex-dns/go` aquí es solo media subida: véase «Transportes cifrados que la caja SIRVE» para el pin de SVCB que necesitó ambas.

**Una sola guarda, en una sola dirección.** `TestRolodexDohBackendMatchesTheInstallScript` extrae `doh.bind` de `../install/scripts/rolodex-config.sh` y lo compara con `RolodexDohBackend`, así que una edición en **este** lado que se aleje del script de instalación falla aquí. La dirección contraria — una edición al script de instalación — solo se detecta si alguien ejecuta después las pruebas de este repositorio, y la prueba **se salta** allí donde el repositorio de instalación no está clonado, lo que incluye el contenedor de pruebas. No hay comprobación del lado de instalación, ni una constante compartida generada entre los dos repositorios. Trata la guarda como algo que cubre la dirección que cubre.
...
**`auto` descubre por su cuenta, sin el conmutador.** Al escalón solo se llega después de que las raíces *y* los upstreams cifrados hayan fallado, así que la elección que tiene delante nunca es «las raíces o el resolutor local»: es «el resolutor local o SERVFAIL». Dejar ahí `DefaultForwarders` en una red que filtra el `:53` hacia cualquier cosa que no sean sus propios servidores no compra privacidad y garantiza el fallo: esas son precisamente las direcciones que se descartan, de modo que toda consulta que llega al escalón paga los dos tiempos de espera y muere igualmente. Nada queda protegido porque el equipo se niegue a fijarse en el único resolutor que responde. Por eso `Manager.forwarders` toma el camino del descubrimiento cuando el modo es `auto` y no hay una lista explícita configurada, diga lo que diga `dns_local_forwarders`.

**Desactivado sigue siendo lo predeterminado en los modos donde el intercambio es real, y esa dirección sigue importando.** El resolutor local ve todos los nombres que busca la casa, que es justo lo que resolver desde las raíces existe para evitar. Ese es un intercambio que un operador hace a sabiendas, no uno que un equipo hace por él la primera vez que una red se porta mal — y en `forward`, donde el escalón es el único upstream y se lleva siempre todas las consultas, es exactamente el intercambio que se está haciendo. Así que el conmutador sigue gobernando `forward`; en `recursive` la lista no se usa de todos modos.

**En `auto`, apagar el conmutador deja de suprimir el descubrimiento solo allí donde no hay una lista explícita a la que recurrir.** Es fácil enunciar esto de forma demasiado amplia — «auto ignora el conmutador» es falso. Con `dns_forwarders` puesto, el conmutador sigue decidiendo: apagado conserva la lista del operador, encendido deriva una y la sustituye, que es lo que activar el conmutador ha significado siempre. El conmutador queda inerte precisamente en el caso en que honrar «apagado» significaría programar `DefaultForwarders` en un escalón al que solo se llega porque todo lo demás ya falló.

Tiene que quedar inerte en ese caso, porque el conmutador no puede expresar «apagado a propósito» separadamente de «nunca tocado» — es un booleano cuyo valor predeterminado es `false`, así que ambos son el mismo valor almacenado, y honrar el primero significaría no descubrir nunca para el segundo, que es justo el equipo para el que existe esta función. Un operador que no quiere ningún resolutor local en el camino pide `recursive`, que lo dice directamente, o nombra el contenido del escalón con `dns_forwarders`. Las pruebas de regresión son `TestAutoDiscoversWithoutTheFlag`, `TestAutoIgnoresTheFlagBeingOffWithNoExplicitList`, `TestAutoStillConsultsTheFlagAgainstAnExplicitList`, `TestAutoLeavesAnExplicitForwarderListAlone` y `TestNonAutoModesDoNotDiscoverWithoutTheFlag`.

**El descubrimiento está ahora en el camino de código predeterminado, lo que lo convierte en un peligro para las pruebas.** `auto` es el modo predeterminado, así que un `rolodex.Config` con valores cero toma el camino del descubrimiento, y `ForwarderDiscovery` recurre a los `/run/systemd/resolve/resolv.conf` y `/proc/net/route` reales de la máquina, con un `Probe` nulo que significa una consulta DNS de verdad. Cualquier prueba que construya un `Manager` y llegue a `Forwarders` — `ProgramRolodex` lo hace, en cada llamada — debe fijar `ResolvConfPaths`, `RouteTablePath` y `ForwarderProbe`, o pasa a depender en silencio de la red en la que se ejecuta y aprueba en cualquier equipo cuyo gateway resuelva. `newProgramTestManager` rellena los tres cuando quien lo llama no lo hizo, y `pinRolodexDiscovery` hace lo mismo con cada `rolodex.Config` que construye la suite de integración; la suite de reenviadores locales es el único sitio que los apunta a fixtures a propósito, porque allí lo que el descubrimiento encuentra *es* el asunto. Leer el modo dentro de `forwarders` lo puso además bajo el cerrojo del gestor: `SetResolutionMode` escribía `cfg.ResolutionMode` sin cerrojo cuando solo lo leía la generación de rolodex.yml, y eso es una carrera en cuanto cada escritura de ajustes y cada pasada de reprogramación resuelven la lista de reenviadores.
...
### Los transportes cifrados que el equipo SIRVE: DoH a través del ingress, DoT y DoQ directos

Todo lo anterior es rolodex como *cliente*: DoH y DoT aparecen en la lista de escalones de `auto` como maneras en que este equipo llega a internet. Esta sección es la dirección contraria — DNS cifrado **servido a los clientes de la LAN**, que es otro conjunto de listeners con otro dueño.

Los tres los abre el `rolodex.yml` de la imagen de instalación (`scripts/rolodex-config.sh` en `../install`) y nada de aquí. rolodex abre cada uno una sola vez al arrancar, por la presencia de su sección de configuración, igual que el listener de métricas, así que ninguno se puede encender por gRPC — véase [rolodex.yml es solo arranque](#rolodexyml-es-solo-arranque-los-ajustes-se-programan-por-grpc).

**El ingress va delante de DoH.** rolodex enlaza DoH en loopback, en `127.0.0.2:4443` (`RolodexDohBackend`), y el ingress lo publica como un vhost cualquiera en `dns.<tld>` (`DohHostLabel`), pasando ese salto interno por proxy con la verificación desactivada (`dohIngressRoute`). El certificado que valida un cliente es por tanto la hoja del ingress, emitida por la CA del equipo bajo su propio repo de hojas (`doh`/`resolver`/`current`), de modo que reemitirla nunca perturba las hojas de pages ni de gfeh. Ese es todo el sentido del montaje: rolodex termina su propio TLS con un certificado autofirmado que ningún cliente DoH que valide acepta, y nunca hace falta que nadie lo acepte, porque termina un salto que jamás sale del equipo. Los clientes usan `https://dns.<tld>/dns-query`. Se pasa por proxy el vhost entero y no solo esa ruta, porque el router DoH de rolodex sirve esa ruta y devuelve 404 a todo lo demás — un matcher de ruta aquí solo añadiría un segundo sitio donde la ruta puede estar mal.

`4443` en vez de `443` no es un detalle. El ingress se publica en `0.0.0.0:443` y rolodex corre con `--net host`, así que un `:443` comodín y un `127.0.0.2:443` concreto en el mismo namespace es EADDRINUSE para el que enlace segundo — y se cae el DNS o se cae el ingress, según el orden de arranque. El número está además escrito en dos repositorios que no pueden leerse entre sí: aquí, y `doh.bind` en `../install`. `TestRolodexDohBackendIsTheLoopbackPortInstallWrites` fija este lado contra el literal; `TestRolodexDohBackendMatchesTheInstallScript` lee el propio script de instalación allí donde ambos repositorios están descargados, y se salta dentro del contenedor de pruebas, donde `../install` no existe.

**El vhost necesita un nombre, y solo los reconciliadores de DNS pueden dárselo.** Detrás de `dns.<tld>` no hay paquete, ni página, ni partición de almacenamiento de objetos, así que ninguna otra cosa lo nombraría jamás: `RebuildDNS` publica su registro A explícitamente (y el AAAA en un equipo de doble pila, porque la hoja lleva las dos direcciones del host como SAN), y `ReconcileDNS` lo lleva en el conjunto deseado para que el barrido horario de huérfanos no borre una hora después del arranque lo que el arranque publicó. Sin el registro, el fallo ocurre antes del TLS y antes de llegar al ingress — un NXDOMAIN pelado, que se lee exactamente igual que una función que nunca se construyó. Pruebas de regresión: `TestRebuildDNSPublishesTheDoHEndpointsName` y `TestReconcileDNSKeepsTheDoHEndpointsName` (unitarias), `TestDohResolvesThroughTheIngress` y `TestDohEndpointResolvesOnRealDNS` (integración).

**DoT y DoQ se saltan el ingress por completo**, y eso es una carencia conocida, no un diseño. El ingress publica solo TCP y su Caddy está fijado a `protocols h1 h2` (`renderCaddyfile`), así que no hay camino de capa 4 a través de él para un protocolo que no sea HTTP. La imagen de instalación enlaza ambos en la LAN, en `0.0.0.0:853` — la pareja estándar, TCP para DoT y UDP/QUIC para DoQ (el predeterminado propio de rolodex para DoQ es `8853`, que ningún cliente busca). La dirección es `0.0.0.0` y no una concreta porque cada sección admite **una sola** cadena de bind: fijar la primera dirección global no serviría nada antes de la primera concesión DHCP, y nada en absoluto en una segunda dirección. `dns.bind` no puede hacer lo mismo porque `:53` choca con el propio listener DNS de podman; `:853` no tiene ese choque, a cambio de enlazar todas las interfaces, incluidos los puentes de podman y la overlay. IPv6 no está cubierto — una sola cadena de bind no puede nombrar las dos familias.

**Ambos sirven una hoja emitida por la CA de la propia máquina.** La imagen de instalación apunta `dot.tls`/`doq.tls` a `/data/tls/dot/{cert,key}.pem` y `issueRolodexTransportLeaf` escribe exactamente ese par en cada `RebuildDNS`, con los mismos SAN que lleva la hoja del vhost de DoH: `dns.<tld>` más las direcciones de la máquina. Un cliente DoT o DoQ que valide verifica por tanto contra una CA en la que la casa ya confía, en vez de recibir un certificado autofirmado que fijar.

El orden es la razón de que esto sea un fichero y no una RPC. rolodex arranca *antes* que el systemcontroller — el controlador no puede bajar ninguna imagen hasta que algo resuelva nombres — así que en un primer arranque la CA no existe cuando rolodex lee su configuración. rolodex trata un certificado nombrado pero ausente como «sirve uno generado y vigila el real», de modo que emitir la hoja **es** el traspaso: se adopta dentro de un sondeo de 30 segundos, sin reinicio y sin nada que coordinar. Eso también pone en marcha por primera vez en esta máquina la recarga en caliente de certificados de rolodex; el material generado no se sondea nunca a propósito, así que hasta que una escucha apuntó a ficheros, todo ese mecanismo era código inalcanzable.

La ruta está bajo `/data` porque es lo único que el contenedor de rolodex puede ver — su unidad monta `/town-os/rolodex:/data` y nada más — así que la hoja va al directorio de datos en lugar de bajo el subvolumen TLS de btrfs con todas las demás. `RolodexTLSSubdir` aquí y `cert_path` en `scripts/rolodex-config.sh` de `../install` son las dos mitades de un mismo montaje escritas en dos repositorios; cambiar una sola deja a rolodex vigilando un fichero que nadie escribe. Pruebas: `TestRolodexTransportLeafLandsInTheMountedDataDir`, `…NamesTheHostAndItsAddresses`, `…IsSignedByTheBoxCA`, `…FollowsAnAddressChange`.

**Un cliente que no confía ya en la CA de la máquina puede verificar el certificado igualmente**, porque la hoja queda fijada en la zona de la que rolodex es autoritativo. `collectRolodexTransportTLSA` publica registros DANE-EE —`3 1 1`, el SPKI de la hoja troceado— en `_853._tcp.dns.<tld>` y `_853._udp.dns.<tld>` en cada `RebuildDNS`, junto a las fijaciones de los paquetes, las páginas y gfeh. Eso es lo que hace que emitir la hoja valga de algo para un dispositivo que nunca se inscribió en la casa: puede alcanzar este resolutor, así que puede obtener la fijación, así que puede comprobar lo que le están entregando sin instalar antes una CA.

Dos registros para un certificado, porque un registro TLSA lo posee un **endpoint** de servicio y no un certificado, y DoT y DoQ comparten el puerto y la hoja mientras difieren en el transporte. Publicar uno y no el otro es peor que no publicar ninguno: un cliente que comprueba DANE y no encuentra registro para el transporte que eligió falla en cerrado, así que la mitad ausente se lee como «el DoQ de esta máquina está roto» y no como «esta máquina no tiene DANE». Para poder expresarlo, `rolodex.TLSAEntry` tuvo que llevar un `Proto`: tenía `_tcp` escrito a fuego, así que la fijación de DoQ no tenía forma de existir. Vacío sigue significando tcp, que es todo paquete que sirve HTTP.

No se publica nada antes de que la hoja esté en disco, por la misma razón por la que `collectGfehTLSA` se salta una partición cuya hoja no se ha emitido: una fijación para un certificado que todavía no existe hace que todo cliente rechace la conexión cuando por fin aparezca. Pruebas: `TestRolodexTransportTLSAPinsBothEncryptedDNSEndpoints`, `…PublishesNothingWithoutALeaf`, y `TestTLSANameProtocol` para la construcción del nombre propietario. De extremo a extremo, `TestDotAdoptsTheCAIssuedLeafWithoutARestart` demuestra que rolodex cambia a la hoja emitida dentro de un sondeo y sin reiniciar — el relevo que ninguna prueba unitaria puede observar, porque trata de otro proceso dándose cuenta de un fichero — y `TestRolodexPublishesTheDoQPinUnderUDP` demuestra que un rolodex vivo guarda y devuelve la fijación bajo `_853._udp`, cosa que un cliente simulado no puede.

**Y se renueva mientras la máquina simplemente está encendida.** `IssueLeaf` reemite un certificado que está a menos de 30 días de caducar, pero solo cuando algo la llama, y quienes la llamaban eran el arranque y un cambio confirmado de IP interna: la renovación era un efecto secundario de reiniciar. `reconcileRolodexTransportTLSA` la pone en la pasada horaria de reparación de deriva (`ReconcileDNS`, que ahora lleva la CA y la base btrfs precisamente para esto), donde los anclajes se reconcilian en la misma llamada: una hoja reemitida lleva una clave nueva, así que la asociación publicada para la anterior deja de casar en el instante en que se escribe el fichero, y un cliente DANE que encuentra registros TLSA y no casa ninguno RECHAZA la conexión. El anclaje nuevo sube **antes** de que baje el viejo: rolodex adopta el certificado renovado en un sondeo de 30 segundos, y tener los dos registros es el único estado en el que cualquiera de los dos certificados valida. La misma pasada vuelve a publicar un anclaje que la zona haya perdido (un rolodex reinicializado, un desmontaje que se llevó la zona) sin esperar a una renovación, y no reescribe nada mientras el certificado siga vigente.

**A los clientes se les dice dónde está todo esto, dos veces.** `publishDDRDesignation` almacena registros SVCB en `_dns.resolver.arpa.` (RFC 9462) nombrando la URL de DoH, el puerto de DoT y el de DoQ bajo `dns.<tld>`, en ese orden de preferencia — DoH primero porque el `:443` sobrevive al DPI que filtra el `:853`. Un cliente que entienda DDR resuelve ese único nombre contra esta máquina y no necesita que se le diga nada más. La designación vive en `arpa.`, que rolodex no resuelve nunca aguas arriba, y ese es el punto: el rechazo se sitúa por debajo de toda búsqueda local, así que solo el resolutor al que se pregunta puede responder por su propia designación.

Se elimina antes de reescribirse, a diferencia de todos los demás publicadores de aquí. `RebuildDNS` empieza derribando la zona del TLD, que es lo que hace idempotentes a los otros — pero `_dns.resolver.arpa.` no está en ninguna zona que esta máquina posea, así que el derribo nunca lo alcanza, y sin esa eliminación cada reconstrucción apilaría otra copia o registraría un fallo por duplicado. Eliminar primero es además lo que hace que un cambio de TLD surta efecto en vez de anunciar ambos. Pruebas: `TestDDRDesignationIsIdempotentAcrossRebuilds`, `TestDDRDesignationFollowsATLDChange`.

**La designación necesita un rolodex que conozca SVCB**, es decir `v0.6.0` o posterior — ese tipo de registro se añadió allí, y el pin de `rolodex-dns/go` en go.mod se movió con él. Un servidor anterior rechaza el registro, `publishDDRDesignation` lo anota en debug y sigue, y el equipo sirve un DNS cifrado que nada puede descubrir: funcionando, e invisible. Por eso el viaje de ida y vuelta se comprueba contra un rolodex **real** (`TestDDRDesignationRoundTripsThroughRealRolodex`) y no solo contra un mock, que aceptaría cualquier tipo de registro que se le entregue.

**Y una vez más, en una forma que una persona puede leer.** `GET /dns/status` lleva un objeto `encrypted` — la URL de DoH, los nombres y puertos de DoT/DoQ y el nombre de descubrimiento — que la pantalla de DNS presenta en solo lectura. Los extremos se derivan del TLD y del ingreso, así que ahí no hay nada que ajustar; lo que faltaba era cualquier manera de *verlos*, en una máquina que llevaba tiempo sirviendo DoH, DoT y DoQ sin que nada le dijera a nadie dónde. El puerto de DoH publicado es el `443` del ingreso, nunca el `4443` interno de rolodex: un cliente enviado a `4443` alcanzaría una escucha solo de loopback con un certificado que rechazaría.

**DoT tiene una prueba; DoQ no tiene ninguna.** `TestDotServesDNSOverTLS` arranca un rolodex real con una sección `dot:` y completa un intercambio RFC 7858 contra él — la comprobación de que la sección que escribe la imagen de instalación produce de verdad un listener que habla el protocolo, y no un socket que se queda colgado. Enlaza loopback en un puerto efímero y no el `0.0.0.0:853` de producción: bajo `--net host` un puerto bien conocido aquí es el *del host*, y dos `make test-full` colisionarían en él. DoQ sigue sin pruebas — un cliente QUIC es una dependencia que este repositorio no tiene, y la misma regla de puertos vale para lo que acabe probándolo.

### Un cambio de TLD tiene que llevarse el DNS cifrado con él

El DNS cifrado lleva el nombre del TLD en cuatro sitios a la vez, y cada uno es un publicador distinto:
...
`rolodex.ChangeTLD` mueve la zona y los registros de los paquetes, y ninguno de estos. Así que `POST /dns/tld` renombraba un equipo de `home` a `lan` y lo dejaba anunciando `dns.home`, sirviendo a `dns.lan` un certificado que decía `dns.home`, y sin publicar anclaje alguno en el nombre que un cliente iba a marcar a partir de entonces.

**Eso último es una rotura, no una degradación.** Un cliente que comprueba DANE y llega a un endpoint sin encontrar un registro TLSA para él rechaza la conexión — para eso está DANE. El fallo no es «el DNS cifrado va sin verificar», es «el DNS cifrado deja de funcionar», y sigue así hasta que el `RebuildDNS` del siguiente arranque lo republica todo. En un equipo que se reinicia una vez al mes, eso es un mes.

Los cuatro son ahora una sola llamada, `publishEncryptedDNS`, y la usan los dos llamadores: `RebuildDNS` y el cambio de TLD. Son una única función precisamente porque nada en los puntos de llamada hacía visible que iban juntos — cada uno se escribió como su propio publicador con un solo llamador, y el segundo llamador se olvidó de los cuatro. `retireEncryptedDNS` es su inversa para los anclajes: los registros del TLD viejo caen con su zona en `ChangeTLD`, pero un renombrado que deje una zona en pie dejaría varado un anclaje para un certificado que ya no sirve nadie, que es el mismo fallo-en-cerrado por el otro lado.

`POST /dns/tld` llama además a `reprogramIngress`, por la misma razón: los nombres de los vhosts llevan el TLD dentro, el del endpoint DoH entre ellos, y el ingress seguía sirviendo los nombres viejos hasta un reinicio.

Pruebas de regresión: `TestTLDChangeRepublishesTheEncryptedDNSEndpoints` y `TestTLDChangeLeavesNoPinAtTheOldName` (unitarias), `TestTLDChangeMovesTheEncryptedEndpointsOnRealDNS` (integración, contra un rolodex real y su resolutor en marcha).
...
- `POST /dns/tld` (requiere admin) -- establece el TLD. Cambia el TLD existente, vuelve a registrar todos los paquetes instalados, republica los endpoints de DNS cifrado bajo el nombre nuevo y reprograma el ingress para que sus vhosts lo sigan. Véase «Un cambio de TLD tiene que llevarse el DNS cifrado con él».
...
- `POST /dns/dnsbl` (requiere admin) -- sustituye la configuración de DNSBL. Acepta una marca de habilitado, un `refusal_cooldown_secs` para toda la lista y una lista de proveedores `{zone, enabled, refusal_codes, refusal_cooldown_secs}`. Las zonas se validan como nombres de host completamente cualificados, se pasan a minúsculas, se recortan y se deduplican; los códigos de rechazo los valida `ValidateRefusalCodes` (dirección IPv4 o `dirección/prefijo`, enmascarada al prefijo, `"none"` solo como entrada única, sin duplicados).
- `GET /dns/rbl/local` (requiere autenticación) -- lista las entradas de la lista de bloqueo local (`{name, reason}`). La ruta conserva su grafía histórica `rbl`: es un contrato publicado con la interfaz, y las entradas no han cambiado -- lo que desapareció son solo las consultas a *proveedores* RBL que antes compartían el prefijo.
- `POST /dns/rbl/local/add` (requiere admin) -- añade una entrada de la lista de bloqueo local. Acepta un nombre (dominio o IP) y un motivo opcional. El nombre se valida (dominio o IP), se pasa a minúsculas y se recorta.
...
Los endpoints DNS de solo lectura (`/dns/status`, `/dns/records`, `/dns/rbl/local`, `/dns/dnsbl/allowlist`, `/dns/services`, `GET /dns/tld`, `GET /dns/dnsbl`) se excluyen del registro de auditoría. Las *escrituras* de la lista de permitidos sí se auditan (eximir un nombre de todas las listas de bloqueo es un cambio del que hay que rendir cuentas); igual que las escrituras de listas de bloqueo que reflejan, no llevan ninguna acción con nombre en `account.RouteActions` — la ruta las identifica.

### Listas de bloqueo

Rolodex (0.2.4+) proporciona dos mecanismos complementarios de bloqueo de spam/malware/anuncios, más (0.4.3+) un mecanismo para deshacerlos y otro para no creerle a un proveedor que rechazó la consulta, todos expuestos mediante la API de DNS y la envoltura `rolodex.Client` (`SetDnsblConfig`/`GetDnsblConfig`, `AddLocalBlocklistEntry`/`RemoveLocalBlocklistEntry`/`ListLocalBlocklistEntries`, `AddDnsblAllowlistEntry`/`RemoveDnsblAllowlistEntry`/`ListDnsblAllowlistEntries`). Todos los **consulta rolodex bajo demanda** — Town OS nunca descarga, analiza ni precachea fuentes de listas de bloqueo.

**La mitad RBL está retirada.** Las consultas a proveedores por IP inversa, `SetRblConfig`/`GetRblConfig` y los endpoints `GET`/`POST /dns/rbl` ya no existen; rolodex renombró `src/rbl.rs` a `src/dnsbl.rs` y conservó solo la comprobación por nombre. Lo que sobrevive al renombrado es la lista de bloqueo *local*, cuyas rutas HTTP siguen diciendo `/dns/rbl/local*` porque son un contrato publicado, y la clave de ajustes `dns_rbl_config`, que ya no lee nadie y que `controller_dns_blocklists.go` nombra solo para que no se reutilice.

Ten en cuenta que `SetDnsblConfig` de la envoltura toma el tiempo de enfriamiento de rechazo de toda la lista como argumento final (`SetDnsblConfig(ctx, enabled, providers, refusalCooldownSecs)`); se mapea sobre `SetDnsblConfigWithRefusalCooldown` del proyecto original, ya que la grafía original que preserva la aridad existe por compatibilidad de API externa, algo que una envoltura interna no necesita.

- **DNSBL** (lista de bloqueo de dominios) -- zonas de lista de bloqueo de dominios consultadas bajo demanda anteponiendo el dominio buscado a la zona (p. ej. `googleadservices.com` + `dbl.spamhaus.org`). Las coincidencias de DNSBL tienen prioridad sobre las respuestas reenviadas/iterativas. Se configura mediante `/dns/dnsbl` como una lista de proveedores `{zone, enabled, refusal_codes, refusal_cooldown_secs}` más una marca global de habilitado y un `refusal_cooldown_secs` para toda la lista.
- **Entradas locales de la lista de bloqueo** -- una lista respaldada por la base de datos con nombres/IP gestionada manualmente mediante `/dns/rbl/local*`, comprobada antes que los proveedores externos. Una entrada local de **nombre de dominio** bloquea las búsquedas directas A/AAAA de ese dominio con `NXDOMAIN`, y surte efecto de inmediato (rolodex actualiza una caché en memoria al añadirla).
- **Lista de permitidos de DNSBL** (rolodex 0.4.3+) -- la escotilla de escape del operador ante un falso positivo de una fuente de terceros, gestionada mediante `/dns/dnsbl/allowlist*`. Una entrada cubre el nombre **y todos los nombres por debajo de él**, así que permitir `vendor.example` exime también a `cdn.vendor.example`. **Hace cortocircuito de toda la comprobación por nombre**, ganando tanto a los proveedores DNSBL configurados como a cualquier entrada local de la lista de bloqueo coincidente, y se ejecuta *antes* de la búsqueda del proveedor, así que un nombre eximido nunca emite ninguna. También está respaldada por la base de datos con una caché en memoria, así que surte efecto de inmediato.

  Sin ella, el único remedio ante una fuente que lista un nombre que la casa necesita es deshabilitar el proveedor entero. Fíjate en la asimetría con la lista de bloqueo local: una entrada de la lista de permitidos es **solo un nombre**, nunca una IP, porque la comprobación que cortocircuita es la basada en nombres — que es el único tipo de consulta a proveedores que hay, ahora que la mitad RBL basada en IP está retirada.
...
**Versión mínima:** un rolodex anterior al manejo de rechazos acepta estos campos — proto3 ignora los campos desconocidos — y no guarda nada. Las pruebas con mocks no pueden distinguir eso del éxito, porque un mock devuelve lo que le entregaron. `TestRolodexDnsblRefusalCodesRoundtripReal` comprueba que una lista configurada **vacía** se lee de vuelta *resuelta*, que es la comprobación que una imagen antigua no supera.

**No hay ingestión ni precacheo de fuentes**: las zonas de proveedor son la unidad de configuración, y la interfaz ofrece una lista curada de zonas DNSBL conocidas como añadidos rápidos de un clic, pero el usuario puede añadir cualquier zona. Las escrituras de zonas de proveedor sustituyen la configuración entera (validada, en minúsculas y deduplicada).

**La lista de añadidos rápidos es un respaldo, y se cura sobre esa base** (`DNSBL_SUGGESTIONS` en `ui/src/routes/dns/BlocklistsTab.jsx`). Una zona pertenece ahí solo si un equipo doméstico puede usarla tal cual: que siga operativa, sea gratuita y responda a un resolutor que recursa por su cuenta sin ningún paso de registro. Actualmente Spamhaus DBL, SURBL, URIBL, NordSpam DBL y Spam Eating Monkey.
...
Ten en cuenta además que las zonas DNSBL (de dominios) están ajustadas para URL de spam en el correo más que para anuncios o rastreadores — el bloqueo de anuncios/rastreadores sería territorio de fuentes, que está [deliberadamente fuera de alcance](#listas-de-bloqueo). Las zonas por IP inversa que antes las complementaban se fueron con la mitad RBL, y para la navegación apenas importaban: solo se consultaban para las IP encontradas en consultas DNS inversas, que la navegación ordinaria casi no genera.
...
Cuando la máquina no tiene todavía la imagen de Grafana, esa descarga corre en su propia goroutine y la unidad se cambia solo cuando la imagen ha llegado. De ese orden dependen tres cosas: la petición de ajustes no se bloquea sobre ~771 MB, el backend anterior sigue respondiendo en `:5308` mientras tanto y — dado que la unidad es `Type=simple`, así que systemd la da por arrancada en cuanto `podman run` hace fork — la máquina no informa de que Grafana está en marcha mientras aún se está descargando. `/monitoring/status` informa `grafana: false` durante toda esa ventana (`MonitoringUIPending`), que es lo que mantiene al panel en su pantalla de «arrancando» en lugar de enmarcar un puerto que todavía sirve el backend anterior. Un segundo guardado concurrente no hace nada en vez de lanzar una segunda descarga, y una descarga fallida limpia la marca y queda registrada.
...
ocho paneles de DNS y los catorce de controller dentro de la vista general enterraría
...
catorce paneles sobre el trabajo de recolección `systemcontroller`, y el único panel
que lee las [métricas `townos_*`](#métricas-del-controlador-del-sistema) del propio
equipo.

El orden es el orden en que se leen cuando algo va mal: qué está caído, qué está
haciendo la API, qué está fallando, qué pinta tiene el disco, cómo se está
aguantando el propio proceso del controlador — y solo entonces el inventario, que
es una pantalla en otra parte de la UI y aquí una línea que apenas se mueve. Los
paneles cuyas series nunca se mueven en un equipo que funciona van abajo, y no en
las primeras filas, donde desplazan a los que sí se mueven.

1. **Service Health** -- `townos_system_unit_active` y `townos_package_unit_active`, una serie por unidad, fijado a 0--1. Este es el panel que dice *qué* servicio está caído, no cuántos, y por eso encabeza. El eje se fija porque la métrica es booleana: en escala automática, un equipo completamente sano se dibuja como ruido alrededor de 1,0 y se lee como alarmante justo cuando no pasa nada.
2. **Service Units by State** -- `townos_system_units` y `townos_package_units` por estado, en un mismo panel y **sin apilar**: son dos totales separados, y apilarlos dibujaría una altura combinada que no cuenta nada que nadie administre.
...
4. **API Latency** -- `rate(townos_http_request_seconds_total)` entre `rate(townos_http_requests_total)`, ambos sumados por `method`: segundos medios por petición. Es el panel que separa "el equipo está ocupado" de "el equipo está atascado", cosa que el panel 3 no puede hacer — un plano de control que sirve dos peticiones por segundo se dibuja igual tanto si cada una tarda 5 ms como 5 s. Una media y no un cuantil porque aquí el formato de exposición no lleva histogramas (véase [Métricas del controlador del sistema](#métricas-del-controlador-del-sistema)); la media se mueve cuando algo empieza a bloquear, que es justo lo que se pregunta.
...
8. **Town OS Disk Fill** -- el mismo disco como porcentaje de `townos_disk_total_bytes`, fijado a 0--100. El panel 7 no puede responder "¿cuánto le falta para llenarse?" sin que quien lo lee haga cuentas contra un eje cuya escala depende del equipo, y un eje fijo hace legible la pendiente: en escala automática, subir del 4 % al 5 % se dibuja igual que subir del 90 % al 99 %. El denominador va sin acotar, así que un sistema de archivos que el recolector no pudo leer rompe la línea en lugar de dibujar un 0 % con toda seguridad.
9. **Controller CPU** -- `100 * rate(townos_process_cpu_seconds_total)`. El suelo se fija en 0 y el techo deliberadamente **no**: esto es por segundo de núcleo, así que un controlador que usa dos núcleos marca 200, y un tope en 100 recortaría justo la desbandada que el panel existe para mostrar.
10. **Controller Memory** -- `townos_memory_heap_bytes` junto a `townos_memory_rss_bytes`. La distancia entre ambos es el diagnóstico: si sube el heap, el controlador retiene objetos que debería haber soltado; si sube el RSS con el heap plano, la memoria se fue a algún sitio que el asignador de Go no contabiliza.
11. **Controller Concurrency** -- `townos_goroutines`, `townos_open_files`, `townos_http_requests_in_flight`. Tres recuentos que son planos en un equipo sano y suben sin límite en uno con fugas; comparten panel porque suelen moverse juntos — un manejador que nunca retorna retiene una goroutine, un descriptor y una petición en vuelo a la vez — y porque cualquiera de ellos por separado es una sola línea por la que nadie abriría una pestaña.
...
13. **Inventory** -- paquetes instalados, actualizaciones pendientes, repositorios configurados y `townos_filesystems` por espacio de nombres. Un panel en lugar de los tres que eran: todos son recuentos en las decenas, así que comparten eje de forma legible y responden "qué hay en este equipo" en un solo sitio.
14. **Accounts** -- `townos_accounts` por tipo con `townos_accounts_granted` al lado, **sin apilar**. Los tipos sí particionan la lista de cuentas, pero el recuento de concesiones es un *subconjunto* del grupo de usuarios; apilado, el panel dibujaría un total mayor que el número de cuentas que existen.

Los paneles 9 a 11 son los que responden "por qué va lento el equipo" en vez de
"qué está ejecutando el equipo", y nada más en ningún panel puede hacerlo: todas
las demás familias de aquí describen lo que el controlador *gestiona*, y todo eso
se ve sano mientras el controlador filtra goroutines hacia el swap. Node Exporter
tampoco cubre el hueco — informa del anfitrión, y en un equipo cuyo trabajo entero
es ejecutar contenedores, la parte del propio controlador es invisible en los
totales del anfitrión.

`townos_up` y `townos_packages_available` deliberadamente **no** se grafican. El
primero es una constante de vitalidad de la recolección, y una línea plana en 1 no
es un panel. El segundo es un tamaño de catálogo en los miles: en un eje compartido
aplana los recuentos de al lado en una línea pegada al fondo, y lo que un operador
quiere de él — "¿ha dejado de responder algún repositorio?" — es
`townos_repository_errors`, que tiene su propio panel. `townos_disk_total_bytes`
solo aparece como denominador del panel 8, nunca como serie: el panel 7 ya apila las
dos mitades que suman su valor.
...
**La salud de los scrapes también se informa aquí, porque no la informa nada más.** `/monitoring/status` lleva además `scrape_targets` (la propia visión de Prometheus de cada job que scrapea, cada uno con `job`, `instance`, `health`, `scrape_url` y — en los que fallan — el `last_error` que Prometheus registró), `down_jobs` (solo los nombres con al menos un target fallando, que es lo que dice el aviso del panel) y `scrape_targets_error` (por qué la lista viene vacía en un equipo donde Prometheus sí está en marcha). `monitoring.FetchScrapeTargets` lee `GET /api/v1/targets` del Prometheus de loopback, con un límite de 3 segundos para que un Prometheus atascado no mantenga abierto un sondeo de estado, y se pregunta **solo cuando la unidad prometheus está activa**: con la unidad parada su estado ya lo dice, y un error de conexión encima se leería como un segundo fallo distinto.

Un target que Prometheus todavía no ha scrapeado informa `unknown`, no `down`: todos los targets están en ese estado durante un intervalo tras cada reinicio, y llamarlo caído haría que cada arranque pareciera una avería.

Esto existe porque un job de scrape que nunca ha funcionado es invisible desde cualquier otro sitio. Los dos fallos de métricas que este equipo ha enviado fueron exactamente eso: `prometheus.yml` decía `https` mientras `:5309` servía en claro, y la sección `metrics:` de rolodex nunca llegó a su archivo de configuración. En ambos casos todas las unidades estaban activas, `systemctl --failed` estaba vacío, y los paneles afectados dibujaban una **gráfica vacía en lugar de un error** — indistinguible de un servicio que no tiene nada que contar. Las pruebas de regresión son `TestFetchScrapeTargets*` y `TestHTTPMonitoringStatus{ReportsDownScrapeJobs,SaysWhyItCouldNotAsk,SkipsTargetsWhenPrometheusIsStopped}` (unitarias) y `TestMonitoringStatusReportsADownScrapeJob` (integración, contra un Prometheus real con un job apuntado a un puerto muerto).
...
| `townos_http_request_seconds_total{method,status}` | counter | segundos empleados en servir, bajo las *mismas* etiquetas que la línea anterior, porque los dos solo sirven divididos |
| `townos_http_requests_in_flight` | gauge | peticiones que se están sirviendo ahora mismo; se emite en cero, y la propia petición a `/metrics` queda excluida |
| `townos_goroutines` | gauge | del proceso del controlador, no del anfitrión |
| `townos_memory_heap_bytes` | gauge | objetos vivos del heap (`HeapAlloc`), no `HeapSys` — la cifra que sube cuando algo se filtra |
...
| `townos_open_files` | gauge | entradas en `/proc/self/fd`; nada en el runtime de Go rastrea descriptores, y una fuga aflora como `EMFILE` horas después |
| `townos_process_cpu_seconds_total` | counter | tiempo de usuario más de sistema, vía `getrusage` |

Todas ellas salvo `townos_up` y `townos_packages_available` las grafica el [panel de
...
separarse. `townos_disk_total_bytes` aparece solo como denominador del panel de
ocupación de disco.

Las familias de proceso son las más recientes y las únicas que describen el
controlador y no lo que gestiona. `/proc` y `getrusage` se leen en cada recolección,
cada uno por su cuenta, y una lectura que falla omite su familia en lugar de hacer
fallar la recolección — por eso la prueba de integración exige valores positivos y
no la mera presencia: un cero significaría que la lectura nunca ocurrió.
...
**El objetivo de recolección no se recompone en ningún sitio.** `MetricsScrapeTarget(listenAddr)` lo deriva de la misma cadena a la que se enlaza el servidor y `main.go` entrega el resultado a `monitoring.Ports.ControllerMetrics` — la misma razón de fuente única de verdad por la que existen `PackageNetworkState.FQDN` y `Manager.MetricsAddr()`. Un enlace comodín (`:5309`, `0.0.0.0:5309`, `[::]:5309`) se reescribe a `localhost` porque un comodín no es una dirección a la que nada pueda conectarse; un host fijado explícitamente se deja en paz, ya que reescribirlo apuntaría la recolección a una dirección en la que el controlador deliberadamente no está. Un resultado vacío omite el trabajo en lugar de apuntarlo a una suposición.

**Y el esquema se le pregunta al socket, no se deriva por segunda vez.** `ControllerMetricsScheme` es `https` cuando `systemcontroller.ListenerSpeaksTLS` completa un handshake TLS contra el propio listener del controlador, y el trabajo lleva entonces además `insecure_skip_verify` — la hoja la emite la propia CA del equipo, en la que Prometheus no tiene razón para confiar ni forma limpia de que se la entreguen, y la recolección es por loopback dentro del espacio de nombres del host, así que nada más puede responder por él.

Antes leía `TOWN_OS_TLS` por segunda vez, y en un equipo desplegado las dos derivaciones no coincidieron: `prometheus.yml` llevaba `scheme: https` mientras `:5309` servía en claro, así que Prometheus abría una conexión TLS, recibía una respuesta HTTP en texto plano y **falló todas las recolecciones durante la vida de esa configuración**. `insecure_skip_verify` no sirve de nada contra eso — relaja la validación del certificado, y el problema es que no hay handshake en absoluto. Cada una de las veintitantas métricas `townos_*` de arriba se exportaba y se tiraba al suelo, y el único rastro era un trabajo `down` dentro de la propia lista de objetivos de Prometheus, algo que nada en el equipo saca a la superficie. Un handshake no puede estar en desacuerdo con el socket; además cubre `TOWN_OS_TLS_CERT`/`TOWN_OS_TLS_KEY`, que terminan TLS sin pasar por la variable que leía la derivación antigua.

El sondeo está acotado a tres segundos y recurre a la visión del propio arranque cuando nada acepta la conexión — no hay socket al que preguntar, y no emitir ningún trabajo cuesta la recolección tan seguro como un esquema equivocado. Cuando el esquema observado y el configurado no coinciden, `SchemeDisagreement` lo informa una vez por stderr: la recolección queda reparada de cualquier modo, pero un equipo que sirve la contraseña de acceso en claro después de que el operador pidiera TLS es un problema de seguridad que la reparación taparía. Cubierto por `TestListenerSpeaksTLS*` (unitaria) y `TestPrometheusScrapesTheSchemeTheListenerActuallySpeaks` (de integración), que ejecuta un Prometheus real contra un listener en claro y contra uno con TLS.
...
- **Modo uPlot**: paneles renderizados directamente en React usando uPlot, consultando Prometheus en el puerto 5308. La rejilla de System se ajusta al viewport (cuatro paneles, dos por fila); las de DNS y Controller **no** — ocho o catorce paneles apretados en una pantalla dejan a cada uno unos 100 px de lienzo o menos, punto en el que un gráfico de latencia es decoración, así que los paneles tienen una altura fija y la página se desplaza.
...
- **Flotante** — la familia `latest` en cualquiera de sus grafías: `latest`, `rc.latest`, `release.latest`, con o sin el sufijo `-x86_64` / `-aarch64`, más una referencia que no lleva etiqueta alguna (podman la resuelve a `:latest`). Estas se vuelven a descargar en **cada** arranque, incluso cuando la imagen ya está en el almacén local. Para una etiqueta que se mueve, «presente» y «actual» son preguntas distintas, y un `podman pull` sobre una etiqueta que no ha cambiado es una comprobación de manifiesto, no una descarga.
- **Fijada** — etiquetas con fecha (`rc.<date>-<arch>`, `release.<date>-<arch>`), referencias por digest, etiquetas de prueba por instancia y cualquier cosa bajo `localhost/` (que no tiene registro del que refrescarse). Se descargan solo cuando faltan.

Que falle el refresco de una imagen que *sí* está presente localmente no es un error: se usa la copia local y se registra el fallo, de modo que una máquina sigue arrancando sin registro — la misma razón por la que su propia unidad usa `--pull=missing` y no `--pull=always`.

**El controlador se actualiza a sí mismo** — `SelfUpdate` corre como primer paso de `boot_services` y es la otra mitad del mismo problema: `EnsureImage` mantiene al día todos los servicios que una máquina ejecuta, pero no puede hacer nada con el proceso que lo ejecuta. La unidad del propio controlador es `--pull=missing` a propósito (un reinicio tras un fallo no debe depender de que el registro esté en pie), así que el controlador en ejecución se quedaba en la imagen con la que se instaló la máquina hasta que un operador pulsaba *refresh system services* — una máquina que actualiza todo menos lo que hace las actualizaciones es el único componente que nunca puede entregar un arreglo para sí mismo.

El paso:

1. Le pregunta a podman de qué referencia de imagen se arrancó *este* contenedor (`{{.ImageName}}`) en lugar de derivarla de `TOWN_OS_TAG`. El sistema de build de instalación sustituye en la unidad el `CONTROLLER_IMAGE` que le hayan dado — posiblemente otro registro entero — y refrescar una referencia que la unidad no ejecuta compararía dos imágenes sin relación y reiniciaría la máquina en cada arranque.
2. No hace nada cuando esa referencia está fijada (un operador fijando la máquina, o una imagen `localhost/` bajo `make dev`).
3. Le aplica `EnsureImage` y luego compara el id que la etiqueta nombra ahora con el id que este proceso está ejecutando.
4. Cuando difieren, escribe el id de destino en `<btrfsPath>/town-os-self-update` y pide a systemd que reinicie `town-os-systemcontroller.service`. El `--pull=missing` de la unidad arranca entonces la imagen que el refresco acaba de dejar en local. El arranque espera ahí en vez de seguir hacia reconcile con un trabajo que está a punto de descartarse.

La marca es el seguro contra bucles: si un reinicio vuelve ejecutando la misma imagen vieja — una unidad que resuelve la etiqueta distinto a como lo hicimos nosotros — la máquina lo intenta una vez, lo registra y sigue, en lugar de reiniciarse en cada arranque para siempre. Una etiqueta que se mueve otra vez es un destino nuevo y tiene su propio intento, y un reinicio que systemd rechaza borra la marca, porque un reinicio que nunca ocurrió no debe suprimir el siguiente.

La consecuencia para una máquina ya instalada: el controlador que ejecuta tiene que contener este código antes de poder usarlo, así que sigue haciendo falta exactamente un *refresh system services* manual (o una reinstalación) para cruzar. Después basta con reiniciar.

`TOWN_OS_IMAGE_REFRESH=0` (también `false`/`no`/`off`) desactiva el refresco — incluida la autoactualización, que entonces no encuentra nada movido — con lo que la regla de las fijadas se aplica a todo; una imagen que falta se sigue descargando. `make dev` y ambos contenedores de prueba lo ponen, porque cargan a propósito desde la caché de imágenes local todas las imágenes que ejecutan — un refresco allí iría a quay.io en cada arranque a reemplazar una imagen colocada deliberadamente. Cualquier valor no reconocido deja el refresco activo: una máquina que deja de adquirir sus propios servicios en silencio es un fallo peor que una descarga de más. Ponerlo en el contenedor es solo la mitad: el PID 1 de esos contenedores es systemd, así que la lista blanca `PassEnvironment` de la unidad del banco de pruebas (`integration/testdata/town-os-systemcontroller.service`) también tiene que nombrar la variable, o se queda en el PID 1 y el controlador — el único proceso para el que se puso — nunca la ve. No la nombraba, así que el refresco corría en cada arranque de test y de dev: rolodex, Prometheus y node-exporter se volvían a descargar de quay.io dentro del conjunto de arranque mientras el banco daba 120 segundos a `/status/ping` para responder, y que una ejecución fallara dependía del enlace. `TestHarnessEnvironmentReachesTheController` deriva ahora los nombres requeridos de `make/test.sh`, `make/dev.sh` y `system_port_env`, y la suite de integración lee el `/proc/<pid>/environ` del controlador en marcha para probar que llegaron.

La distinción importa porque el instalador no lleva ninguna imagen y la etiqueta de toda máquina flota por omisión (`rc.latest-<arch>`). Sin ella, «ya cargada» significaba «al día», y cada servicio de sistema se quedaba en la imagen que la máquina descargó la primera vez que arrancó — una VM reinstalada seguía sirviendo una interfaz de hace meses por muchas veces que se reiniciara, y lo único que la hacía avanzar era un `POST /system-services/refresh` del operador.
...
Esa es también la razón de que `make dev` sea el único objetivo al que un puerto puede *impedirle* arrancar: en una máquina hay un solo `127.0.0.2:53`, y más de una cosa lo quiere — el rolodex de host del repositorio install, el equipo de dev de otro checkout, un contenedor de servicio de sistema huérfano de una ejecución de dev que fue matada a lo bruto. Por eso `require_free_rolodex_dns` (`make/dns.sh`) se niega a arrancar cuando, ya retirado el contenedor de este checkout, la dirección sigue ocupada, y dice qué hay que parar. La redirección esperaba antes a una simple conexión TCP a esa dirección, que tiene el mismo éxito cuando quien responde es el resolutor de otro: el rolodex de este checkout no lograba enlazar y el host quedaba apuntado a un rolodex que no sabe nada de este equipo de dev — indistinguible, desde el navegador, de que el DNS propio del equipo de dev estuviera roto. `wait_for_rolodex_dns` exige ahora las dos mitades, que la unidad esté activa (lo que hace que el resolutor sea *nuestro*) y que la dirección esté enlazada (lo que lo hace utilizable, ya que la unidad se activa en cuanto `podman run` está en pie, antes de que rolodex haya abierto un socket), y es **fatal** en vez de una advertencia: un equipo de dev cuyo resolutor nunca subió no es un equipo de dev, y seguir adelante producía uno que parecía arrancado mientras cada nombre `.home` del host se iba al resolutor de la LAN. La redirección ocurre en cuanto el controlador está listo, antes de la cuenta y del servidor de UI, porque todo lo que hay después de ese punto es un equipo de dev que ya está en pie. Lo cubren `TestDevDNSRefusesToStartWhenTheRolodexAddressIsTaken` y las pruebas `TestDevDNSWait*`, que sustituyen `ss`, `resolvectl` y `${SUDO}` — ninguna prueba puede tocar el resolutor del host.

Queda la cuestión de quién renderiza el `rolodex.yml` desde el que arranca sus binds una caja de dev. No lo hacía nadie, y ese es el fallo que encontró la espera fatal de arriba: rolodex abre su listener de DNS **una sola vez**, al arrancar, desde ese fichero, y nada de lo que Town OS pueda decirle por gRPC abrirá uno — `ProgramRolodex` posee los forwarders, el modo de resolución y ambas listas de bloqueo, y deliberadamente no los binds (`controller_rolodex_config.go`). En una caja real lo renderiza la imagen de install (`../install/scripts/rolodex-config.sh`, su único escritor). `make dev` no ejecuta instalador alguno, así que `${STATE_DIR}/dev-rolodex` estaba vacío y `town-os-system--rolodex.service` — cuyo `ExecStartPre` crea el *directorio* — arrancaba rolodex con `--config /data/rolodex.yml` contra un fichero que nunca había existido. La unidad pasaba a activa igualmente, porque el contenedor arranca perfectamente y lo único que falta es el listener, que es justo por lo que la espera pregunta por la dirección y no sólo por la unidad. Ahora `seed_dev_rolodex_config` (`make/dns.sh`) lo escribe en el anfitrión antes de que el contenedor monte el directorio: sólo loopback — los binds de DNS, el listener de métricas, el socket gRPC que marca el controlador, un par de forwarders de partida — y reescrito en cada ejecución, porque `dev-rolodex` persiste para guardar `rolodex.db` y una configuración de un esquema antiguo sobreviviría al checkout que la escribió. Se siembra en el anfitrión en vez de hornearse en la imagen a propósito: ese mismo fichero de unidad viaja en la imagen de los tests de integración, y un `rolodex.yml` que ate `127.0.0.2:53` dentro de un contenedor de test con `--net host` ataría el `:53` **del anfitrión** en cada ejecución. Sin esto, cada arranque pagaba además dos esperas incondicionales de treinta segundos — `Manager.WaitForDNSReady` y luego el bucle de marcado del socket gRPC en `reconcileDNSAndNetworks` — dentro del presupuesto de 120 s que `require_controller_ready` concede a un arranque que responde 503 a `/status/ping` hasta estar del todo terminado.

Ese mismo arranque es lo que decide qué tiene que meter `make dev` en el contenedor antes de arrancarlo. Toda unidad generada ejecuta `podman run` **sin** `--pull=never`, así que una imagen que no está en el almacén interno no es un error que falle rápido: es un tirón del registro ocurriendo dentro de `podman run` mientras los 120 s de `require_controller_ready` y la espera de disponibilidad del propio servicio corren a la vez. El ingress y el caddy del servicio de páginas faltaban ambos en esa lista (caddy también en el banco de integración), así que una caja de dev los traía de quay y de Docker Hub durante el arranque con los tar ya guardados en la caché de imágenes del checkout. Ahora se cargan los dos, con el ingress construido por un prerrequisito `ingress-image`. Dos cosas se cargaban que no debían: **Grafana** ocupa ~1,17 GB y es el backend de monitorización que *no* es el predeterminado, así que el arranque no lo ejecuta jamás salvo que el backend almacenado sea `grafana` — se omite salvo con `DEV_LOAD_GRAFANA=1`; y **gfeh** ya no es prerrequisito de `dev`, porque es la única imagen local cuyo contenido viene de fuera del repositorio y por tanto recompila el árbol de dependencias entero de `gfehd` una vez por día natural, para todo el mundo, se use o no. `make gfeh-image` habilita el almacenamiento de objetos en dev exactamente como `make/dev.sh` ha dicho siempre. La carga misma pasa ahora por la caché de imágenes montada de solo lectura en `/image-cache` en vez de `podman cp`, lo que elimina una escritura completa de cada tar; un contenedor sin ese montaje sigue recurriendo a la copia. Por último, el systemcontroller **ya no se habilita** en la imagen de dev: systemd lo arrancaba en el instante en que `podman run -d` retornaba, contra un almacén vacío, lo que compraba un arranque malo garantizado — los 30 s completos de espera de DNS, tirones de todo lo aún no cargado, unidades arrancadas contra imágenes ausentes, cada repositorio de paquetes clonado — todo ello descartado por el arranque que `make/dev.sh` emite una vez terminada la carga, y todo ello compitiendo con esa misma carga. rolodex sigue habilitado, porque tiene que estar sirviendo antes de que arranque el controlador y su unidad está hecha para sobrevivir a esa espera.
...
- `INGRESS_METRICS_PORT` -- el puerto en el que el ingress sirve su endpoint `/metrics` de Prometheus, publicado solo en `127.0.0.1` (`9146` por defecto). Es un tercer listener junto a los dos anteriores y necesita su propia anulación por la misma razón que el de rolodex; `ingressctl.MetricsAddrFor()` es la única función de la que se construyen tanto la especificación `-p` de la unidad como el objetivo del scrape, así que reubicarlo mueve ambos. Véase [Métricas del ingress](#métricas-del-ingress).
