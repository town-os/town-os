...
Ninguna de las dos fallas se ve en `make test` — tanto la suite unitaria como la de integración se apoyan en un **gfehd falso**, así que un demonio real pero equivocado compila, se publica y se instala perfecto, y solo sale como almacenamiento de objetos que nunca arranca en un equipo real. Para que quede: esa es la razón por la que existía el mínimo. 0.1.1 no puede parsear la configuración de una partición que traiga cualquier usuario SMB (cada struct de configuración de gfehd es `#[serde(deny_unknown_fields)]` y su `SmbConfig` no tiene campo `users`), y se autentica apenas arranca, que durante el arranque es el stub de `:5309` contestando 403 a todo menos al ping.

**La compilación de release pasa `--no-cache`, y eso es estructural.** `cargo install gfehd` es un renglón `RUN` idéntico byte por byte en cada compilación, así que su capa es un acierto de caché permanente — sin `--no-cache`, una compilación cuyo contrato completo es "lo que crates.io tenga hoy" serviría para siempre el crate de la primera compilación, calladita y con bitácoras limpias cada vez. No hay una llave de caché más barata: saber cuándo cambió el crate significa preguntarle a crates.io, que es justo para lo que existe esta compilación. El volumen del registro de cargo sobrevive a `--no-cache`, así que el árbol de dependencias se recompila pero no se vuelve a descargar. El fixture local `gfeh-image` se queda cacheado a propósito — es prerrequisito de cada corrida de integración y de dev, y solo necesita un gfehd real, no el de hoy.
