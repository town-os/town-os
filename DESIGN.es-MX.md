...
- **`WriteConfig`** -- renderiza `rolodex.yml` en `DataDir` y lo escribe siempre que los bytes difieran de lo que hay en disco. Devuelve un booleano que indica si el archivo cambió de verdad (para que quien llama reinicie la unidad de systemd solo cuando hay algo por lo que reiniciarla). **Reconcilia incondicionalmente, y eso carga peso** — véase [La configuración que se congeló](#la-configuración-que-se-congeló).
