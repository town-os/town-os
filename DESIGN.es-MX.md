...
**El descubrimiento ya está en el camino de código predeterminado, y eso lo vuelve un peligro para las pruebas.** `auto` es el modo por omisión, así que un `rolodex.Config` en ceros agarra el camino del descubrimiento, y `ForwarderDiscovery` se va a los `/run/systemd/resolve/resolv.conf` y `/proc/net/route` reales de la máquina, con un `Probe` nulo que significa una consulta DNS de verdad. Cualquier prueba que construya un `Manager` y llegue a `Forwarders` — `ProgramRolodex` lo hace, en cada llamada — tiene que fijar `ResolvConfPaths`, `RouteTablePath` y `ForwarderProbe`, o pasa a depender en silencio de la red en la que corre y pasa en cualquier equipo cuyo gateway resuelva. `newProgramTestManager` llena los tres cuando quien lo llama no lo hizo.
