...
13. **Derivar la imagen del NC** — `quay.io/town/networkcontroller:<tag>`, sobrescribible con `NC_IMAGE`. Se baja (paso 18), nunca se construye.
14. **Arrancar el refresco de repositorios en segundo plano** — una goroutine consulta cada 5 minutos.
15. **Etapa `boot_dns`: construir el gestor de rolodex y esperar al DNS** **(no fatal)** — Rolodex es un servicio de arranque que administra systemd, y para este punto ya está respondiendo: arranca antes que el systemcontroller con el `rolodex.yml` que escribió la imagen de instalación, porque el controlador no puede bajar ninguna imagen hasta que algo resuelva nombres. **Aquí no se escribe `rolodex.yml` ni se reinicia el servicio** — ve «rolodex.yml es solo arranque; los ajustes se programan por gRPC» más abajo, y [La configuración que se congeló](#la-configuración-que-se-congeló) para la falla que acabó con esa escritura. Lo que esta etapa construye es el `rolodex.Manager`, a partir de los ajustes que Town OS posee: `resolution.mode` desde `dns_resolution_mode` (un valor guardado que no se pueda analizar se va al predeterminado en lugar de programar un modo que rolodex rechazaría), la lista de reenviadores desde `dns_local_forwarders` (cuando está prendido, la lista se descubre a partir de los resolvedores del host en cada arranque, de modo que un equipo que cambió de red agarra el nuevo sin que el operador toque nada — ve [Reenviadores locales](#reenviadores-locales)), y la lista de bloqueo desde su ajuste guardado. Todos ellos se empujan al servidor **en marcha** más adelante en este mismo arranque, con `ProgramRolodex` en el paso de reconciliación. El contenedor de rolodex corre con `--net host` y enlaza el DNS directamente a `127.0.0.2:{puerto}`. Después espera a que el DNS esté listo (sondeo de conexión TCP) y configura systemd-resolved para enrutar el TLD a rolodex — **se omite cuando `TOWN_OS_DNS_PORT` movió a rolodex fuera de `:53`**, ya que una dirección de servidor por dominio en resolved no lleva puerto y mandaría al vacío todas las consultas de ese TLD.
...
17. **Etapa `boot_services`, primer paso: autoactualización** **(no fatal)** — `SelfUpdate` refresca la imagen *propia* del controlador y, cuando la etiqueta bajo la que corre pasó a nombrar otra imagen, le pide a systemd que reinicie `town-os-systemcontroller.service` y detiene ahí este arranque (ve *El controlador se actualiza a sí mismo*).
18. **Etapa `boot_services`: bajar las imágenes de contenedor centrales** **(no fatal)** — la imagen del NC, Prometheus, Node Exporter, la imagen de la interfaz, la imagen del almacenamiento de objetos (gfeh), la imagen del ingress y Grafana cuando ese backend está seleccionado, en paralelo con `parallelEnsureImages` (una etiqueta **flotante** se vuelve a bajar en cada arranque; una fijada solo se baja cuando falta — ve *El arranque refresca las etiquetas de imagen flotantes*). Toda imagen que referencie una unidad de arranque va aquí: una unidad cuya imagen no está local la baja desde dentro de `podman run`, así que su espera de disponibilidad compite con una descarga del registro. gfeh y luego el ingress faltaron en esta lista por turnos, y cada uno se veía como un servicio que nada más estaba caído. La interfaz de monitoreo no necesita entrada propia — en el backend uPlot corre la imagen del NC, que ya es la primera del conjunto.
19. **Arrancar los servicios de monitoreo del sistema** **(todos no fatales)** — primero se desmantelan las unidades NC/socket heredadas del diseño anterior (todavía retienen `-p 9090`/`-p 5308` y meterían en bucle de caídas a los servicios nuevos). Node Exporter, Prometheus y la interfaz de monitoreo corren todos con `--net host`; node-exporter y Prometheus enlazan al loopback, y solo el `:5308` de la interfaz de monitoreo da a la LAN. Los tres puertos vienen de `monitoringPortsFromEnv()`, cuyo valor cero son los predeterminados de producción ([Puertos de host de los servicios del sistema](#puertos-de-host-de-los-servicios-del-sistema)). Después se instala el temporizador nocturno de poda de podman **(no fatal)**. El temporizador diario de actualización no se instala aquí — viene con el instalador, ve [Actualizaciones automáticas](#actualizaciones-automáticas).
20. **Asegurar la CA TLS local** **(no fatal)** — `tls.EnsureCA(<btrfsPath>/tls)` antes de la reconciliación, para que esta pueda emitir certificados hoja mientras recorre los paquetes instalados.
21. **Arrancar el ingress y el servicio de pages** **(no fatal)** — `ingressctl.Manager` instala y arranca `town-os-system--ingress` (router compartido SNI en `:443` + Host en `:80`), en doble pila solo cuando el host tiene una IPv6 global. El servicio Caddy de pages arranca junto con él. Los dos se omiten cuando `INGRESS_IMAGE` se define explícitamente como vacío (modo de desarrollo).
22. **Reconciliar el almacenamiento de objetos** **(no fatal)** — `ReconcileGfeh` asegura una partición gfeh por red: el subvolumen `gfeh/<network>` (con chown a uid 2000), el `gfehd.yaml` renderizado y la unidad `town-os-system--gfeh-<network>`, que se reinicia solo cuando el contenido renderizado cambió. Se omite por completo cuando `GFEH_IMAGE` está explícitamente vacío, y se omite cuando el ingress está deshabilitado (las cuatro vistas HTTP solo se alcanzan a través de él). Los *nombres* de las particiones se publican después y de forma asíncrona — ve el paso 31. Ve [Almacenamiento de Objetos (gfeh)](#almacenamiento-de-objetos-gfeh).
23. **Detectar el cambio de versión** — compara el SHA de la imagen del contenedor en ejecución (`/proc/1/cgroup` → `podman inspect`) contra `<btrfsPath>/town-os-version`. Define `versionChanged` para la reconciliación.
24. **Reconciliar** — recorre todos los paquetes instalados y restaura el estado de ejecución:
...
26. **Programar el ingress** **(no fatal)** — espera a que esté listo, marca su socket gRPC y `RebuildIngress` empuja el conjunto completo de rutas (paquetes HTTP + páginas + vistas e índices del almacenamiento de objetos) de forma declarativa, el mismo modelo que `RebuildDNS`. También renderiza las dos páginas que genera una partición — su [índice](#el-índice-de-la-partición) y su [índice de archivos publicados](#el-índice-de-archivos-publicados) — a partir de exactamente el mismo conjunto de sitios con el que se construyen esas rutas, en la misma pasada, porque una ruta no se puede programar antes de que existan los bytes que sirve.
...
El índice trae **solo las vistas**, que ya están en el DNS. Ni principales, ni
permisos otorgados, ni cuota, y tampoco los enlaces publicados: es la página que
un operador reparte para explicar qué es el almacenamiento de objetos, y un
...
`http.gfeh.<tld>` es el único nombre cuyo propósito entero es entregarle
archivos a la gente, y era también el que daba 404: gfehd contesta `/f/<token>`
y nada más, incluida su propia raíz. Esa raíz ahora sirve un **índice de
archivos publicados** — cada exposición habilitada, con nombre y enlazada al
token bajo el que se sirve.

Este es el único nombre del equipo con **dos backends**, y para eso existe
`PathBackend` en una `Route` del ingress (ver [Ingress](#ingress)):

- **La `/` la atiende el contenedor de pages**, renderizando
  `gfeh.RenderPublicIndex` desde `gfeh-index/http.gfeh.<tld>/`, pasando por
  exactamente la misma revisión de directorio, la misma escritura solo si
...
- **Todas las demás rutas siguen siendo de gfehd.** El matcher es la raíz y nada
  más: un prefijo taparía rutas que gfehd pueda agregar después, que es justo la
  falla que esta página arregla, al revés. La raíz es la única ruta que gfehd ha
  dicho que no sirve, dando 404 en ella.
- **Solo se programa un backend por ruta para un nombre cuyos bytes existen.**
  ...
  de pages para un nombre sin entrada en el webroot cambia el 404 de gfehd por
  el de Caddy — la misma falla con otro logotipo.
...
- **Un demonio inalcanzable conserva su página.** Si `ListExposures` falla, el
  ...
  ha publicado nada": una caída breve no es prueba de que alguien haya retirado
...
**Enumera credenciales al portador, a propósito.** Una URL `/f/<token>` es una
capacidad — tenerla es toda la autorización — así que publicar un archivo ya no
significa "alcanzable por quien reciba el enlace" y pasa a significar "listado
para cualquiera que pueda resolver el nombre". Ese es el trato que es esta
página: un directorio de archivos publicados vale la pena, y uno que omitiera
los enlaces sería una lista de nombres de archivo que nadie podría abrir. De ahí
se sigue que **retirar es lo único que vuelve a hacer privado un archivo**; una
exposición deshabilitada no aporta ningún renglón, porque ese renglón anunciaría
un enlace que no resuelve a nada.
...
**Town OS compila el gfehd actual, y a propósito no hay ningún control de versión.** `Containerfile.gfeh` corre un `cargo install gfehd` pelón — sin `--version` y sin `--locked` — así que la imagen se lleva lo que crates.io tenga al momento de compilar, resuelto contra dependencias actuales.

Sí hubo un control (`GFEH_VERSION`, con una salida de emergencia `GFEH_LATEST`) y hacía daño de verdad. El Makefile pasaba la versión en cada compilación como `--build-arg`, que **le gana al valor por defecto de un `ARG`**, así que el número que se publicaba era el del Makefile y el del Containerfile era adorno. Los dos se separaron: el Containerfile decía `0.1.2` y explicaba largo y tendido por qué nada más viejo puede correr bajo Town OS, mientras el Makefile compilaba calladito `0.1.1`. Agarrar la versión publicada actual cumple el mínimo por construcción y quita el segundo lugar donde vivía la respuesta.

Ninguna de las dos fallas la atrapan las suites, y por razones distintas. La suite **unitaria** se apoya en un gfehd falso y nunca llega a correr el demonio. Las suites de **integración y de interfaz** sí corren el real — `make/test.sh` carga la imagen construida en los dos contenedores, porque no hay sustituto que pruebe que una partición arranca, contesta en su socket de administración y aplica sus propios topes — pero un demonio nomás *viejo* de todos modos compila, se publica, se instala y arranca. Solo sale como almacenamiento de objetos que nunca arranca en un equipo real. Para que quede: esa es la razón por la que existía el mínimo. 0.1.1 no puede parsear la configuración de una partición que traiga cualquier usuario SMB (cada struct de configuración de gfehd es `#[serde(deny_unknown_fields)]` y su `SmbConfig` no tiene campo `users`), y se autentica apenas arranca, que durante el arranque es el stub de `:5309` contestando 403 a todo menos al ping.

**Las dos compilaciones derrotan la caché de capas, por medios distintos.** `cargo install gfehd` es un renglón `RUN` idéntico byte por byte en cada compilación, así que su capa es un acierto de caché permanente — de otro modo, una compilación cuyo contrato completo es "lo que crates.io tenga hoy" serviría para siempre el crate de la primera compilación, calladita y con bitácoras limpias cada vez. No hay una llave de caché más barata: saber cuándo cambió el crate significa preguntarle a crates.io, que es justo para lo que existe esta compilación.

- `release-gfeh` pasa **`--no-cache`**. Nada más débil es aceptable en algo que se distribuye.
- `gfeh-local` pasa un build-arg **`GFEH_CACHE_DATE`** con granularidad de día. El fixture es prerrequisito de cada corrida de integración y de dev, así que `--no-cache` ahí recompilaría el árbol de dependencias de Rust en cada una; pero un acierto de caché puro lo congelaría en el gfehd que fuera actual la primera vez que se construyó en esa máquina — y las suites de integración y de interfaz arrancan particiones reales contra él. Lo diario es el punto medio: acierto de caché dentro del día, recompilación al cambiar la fecha.

En los dos casos el volumen del registro de cargo sobrevive, así que el árbol de dependencias se recompila pero no se vuelve a descargar. Este es un caso concreto de una regla general de compilación ([CLAUDE.md](CLAUDE.md)): una imagen local construida a partir del código del repositorio no puede separarse, porque un cambio en el código invalida su caché; una cuyo contenido se descarga al momento de construir necesita una invalidación explícita.
...
El rechazo antes era **incidental**: nada revisaba la red, y el manejador se caía hasta `netip.ParsePrefix` sobre el `Subnet` vacío del renglón sembrado, que fallaba y salía como un **500**. Eso se lee como un equipo descompuesto y no como un rechazo, no le dice a quien llama por qué, y habría dejado de rechazar en cuanto algo escribiera una subred en ese renglón. La guarda va por nombre, antes de que el servidor genere el par de llaves, y detrás lleva una revisión de renglón sin transporte para cualquier red cuya subred esté vacía por otro motivo.
...
- `POST /networks/peers/add` (`requirePeerEnroll`: admin o el permiso `wireguard`, confinado a las redes de quien llama) -- registra un par. Cuando `public_key` está vacío, el servidor genera un par de llaves y devuelve la llave privada más una configuración de dispositivo lista para importar. Acepta un `endpoint` opcional y una marca `rolodex`. **La red del hogar es un 400** — es solo DNS y no trae pares, lo pida quien lo pida (ver [La red del hogar siempre existe](#la-red-del-hogar-siempre-existe)).
...
pelón que lleva el backend propio de la ruta. Existe porque un backend puede ser
dueño de un nombre sin serlo de todas las rutas debajo de él — su único llamador
...
- **El `handle` pelón siempre va al final.** Los bloques `handle` son mutuamente
  excluyentes y se evalúan en el orden escrito, y uno pelón empata con todo, así
  que cualquier cosa después de él sería configuración muerta.
- **Los backends por ruta se renderizan en el orden en que se dieron**, no
  ordenados. Ordenarlos sería seguro para los matchers que Town OS programa de
  verdad (no se traslapan) y cambiaría en silencio cuál gana para un par que sí
  lo hiciera.
- **Una ruta se valida igual que un nombre de host y un backend**
  ...
  longitud acotada). Una mala se tira **individualmente**, no junto con su ruta:
  el backend propio de la ruta es el comodín, así que perder un backend por ruta
  cuesta la única ruta que reclamaba mientras el servicio detrás del nombre
  sigue contestando. Una ruta duplicada también se tira — Caddy acepta dos
  matchers idénticos y sencillamente nunca llega al segundo.
- **Un backend por ruta no tiene `backend_tls` propio** y se proxea por HTTP
...
La salida está **ordenada por nombre de host** para que los bytes renderizados sean deterministas entre reconciliaciones — eso es lo que deja al supervisor no hacer nada ante una recarga cuyo contenido no cambió. Los globales son `auto_https off` (Town OS administra los certificados), `protocols h1 h2` (el ingress publica solo TCP, así que H3/QUIC sobre UDP no se alcanza) y el endpoint de administración. La API de administración de Caddy se deja a propósito **habilitada**: el supervisor programa las rutas nuevas con `caddy reload`, que habla con ese endpoint, así que `admin off` rompería todas las actualizaciones de rutas después del primer arranque.

Su dirección se **escribe en la configuración** como `admin 127.0.0.1:<puerto>` en vez de dejarse al valor por omisión de Caddy, y por omisión es el `2019` del propio Caddy (`ingress.DefaultAdminPort`, que se puede sustituir con `--caddy-admin-port` / `ingress.WithCaddyAdminPort`). Producción se queda en 2019 — el contenedor del ingress tiene su propio espacio de nombres de red y no publica más que :443, :80 y el puerto de métricas en loopback, así que la API de administración se alcanza desde dentro de ese contenedor y desde ningún otro lado. **Cualquier cosa que corra Caddy en el espacio de nombres del anfitrión tiene que reubicarla**, y toda prueba lo hace: `go test ./src/...` corre directo en el anfitrión y el contenedor de integración es `--net host`, así que dos `make test-full` concurrentes pondrían dos hijos de Caddy en el mismo :2019 — el segundo no logra el bind y termina, y antes de hacerlo, el `caddy reload` de cualquiera de las dos corridas puede aterrizar en la API de administración de la otra y programarla con el Caddyfile equivocado (REGLA DE HIERRO). Renderizar la dirección es también lo que *apunta* `caddy reload` hacia ella: ese comando lee la dirección de administración de la configuración que está adaptando. La grafía `127.0.0.1` es deliberada — `localhost` resuelve primero a `::1` en una caja de doble pila, mientras que Caddy solo hace bind de una de las dos.

El ingress es **agnóstico a la interfaz**: publica `-p 443:443` / `-p 80:80` sin ninguna IP de host y su Caddyfile no trae **ninguna directiva `bind`**, así que Caddy escucha en todas las interfaces y escoge el vhost puramente por SNI/Host. (El único listener fijado a una IP de host es el endpoint de Prometheus, que es para la propia caja y no para sus clientes — véase [Métricas del ingress](#métricas-del-ingress).) Un cliente de la LAN y un par de la superposición llegan al mismo listener, escogen por SNI el mismo vhost, obtienen la misma hoja de la CA local y se les hace proxy al mismo contenedor. No agregues directivas `bind` ni listeners por red.
...
### Cuando el backend está caído: la página de reintento

Un backend detrás de una ruta programada puede estar roto de dos maneras, y el ingress responde a las dos con la misma página: un **503 que dice que el servicio no está disponible y que se recarga solo cada cinco segundos** (`unavailableRefreshSeconds`, que es también el `Retry-After`) hasta que el servicio vuelve a responder. No se reprograma nada ni se retira ninguna ruta — la dirección se queda enrutada todo el tiempo, así que en cuanto el backend vuelva a tener un listener, la siguiente recarga lo sirve.

Este es el estado normal de una caja doméstica, no una excepción: un paquete se reinicia al actualizarse, un cambio de ajustes rebota un servicio, un reinicio regresa veinte contenedores en algún orden. Lo que Caddy dice durante esos segundos es `502 Bad Gateway`, que se lee como *descompuesto* en vez de como *ya viene de regreso*, y no le deja al lector nada que hacer más que reintentar a mano.

Las dos formas de falla son mecanismos separados en Caddy, y una implementación que cubriera solo una se dejaría el caso común:

- **El backend respondió, con un 5xx.** Eso es una *respuesta* del upstream, interceptada con un matcher de respuesta y `handle_response` **dentro** del bloque `reverse_proxy` — que es por lo que todo proxy que emite el renderizador ahora abre un bloque, incluidos los de HTTP plano.
- **El backend nunca respondió** — contenedor caído, nombre que no resuelve, conexión rechazada. `reverse_proxy` falla y Caddy levanta su propio error, que `handle_response` no ve nunca; eso es `handle_errors` a nivel de **bloque de sitio**. Este es el caso común en una caja doméstica, y el que una implementación centrada solo en el 5xx se dejaría en silencio, porque el 5xx que vigila no llega nunca.

Los dos caminos se renderizan como **snippets** de Caddyfile (`render_unavailable.go`), definidos una vez por archivo e importados por cada bloque de sitio — así la página no puede derivar entre vhosts, y el archivo mantiene un tamaño legible por muchas rutas que sirva la caja. Los snippets se emiten también con un conjunto de rutas vacío: un snippet que nadie importa nunca se analiza como directivas, y un preámbulo que apareciera y desapareciera rebotaría Caddy en la primera ruta.

- **Solo un navegador recibe la página.** Los dos caminos se condicionan a `method GET HEAD` más `Accept: *text/html*`. Un cliente de API — un XHR, un webhook, `curl`, un resolvedor DoH detrás de un path backend — recibe el estado, el cuerpo y los encabezados propios del upstream copiados tal cual en el camino de respuesta, y un 503 escueto con `Retry-After` en el camino de error. Entregarle HTML a algo que está parseando JSON convierte un backend roto en una segunda falla, más rara, dentro de quien lo llamó.

  El paso a través es `copy_response` **solito**, sin `copy_response_headers` al lado, y esa omisión es estructural: `reverse_proxy` ya dejó puestos los encabezados del upstream en el response writer para cuando corre un manejador de respuesta, así que copiarlos otra vez emite cada uno dos veces — `Content-Length` incluido, lo que vuelve el mensaje malformado y no nada más descuidado. Parece la directiva compañera obvia, y por eso la fijan tanto una prueba del renderizador como una aserción de integración.
- **La página nombra el servicio desde la configuración renderizada, nunca desde la petición.** La etiqueta es un argumento de snippet sustituido al momento del import — el hostname de la ruta, ya validado por `validSiteHost`. El bloque de respaldo de `:80` se etiqueta en cambio con la constante `Town OS`, porque responde por *todo* host no emparejado: su encabezado `Host` es lo que el cliente haya mandado, y reflejar eso en HTML sería una inyección en el único vhost que acepta cualquier nombre. `safeUnavailableLabel` es la guarda que mantiene eso cierto para un futuro llamador.
- **El bloque `:80` de redirección de un paquete no recibe ninguno de los dos**, ya que no hace proxy a nada — ahí no hay backend que pueda estar no disponible.

#### La página está traducida

Las cuatro frases que trae son entradas del catálogo (`ingress.unavailable.*` en `src/i18n`), no literales en el renderizador: cada idioma que Town OS trae recibe su propia versión de la página, y Caddy escoge una por petición a partir de `Accept-Language`.

Eso repite lo que la interfaz ya hace, y por la misma razón: **el idioma del lector es el del navegador, no el de la caja.** `I18nContext` resuelve primero una elección guardada, luego `navigator.languages`, y solo recurre al ajuste global `locale` del servidor cuando el navegador pide un idioma que ningún catálogo cubre. La página de reintento resuelve ese mismo orden dentro de Caddy — un `map` sobre `Accept-Language` con un renglón por catálogo de idioma, y un renglón `default` que lleva el locale de la caja. Un invitado con el celular en portugués lee portugués en una caja configurada en alemán.

- **Un `map`, y una sola copia de la página — no un bloque `handle` por idioma.** La forma con bloques funciona, y se midió: como un `import` es textual, cada expansión incrusta el selector completo, así que treinta ramas querían decir treinta copias de la página en el JSON que se le entrega a Caddy en cada recarga — **115 KB para una sola ruta**, y una caja con veinte rutas habría empujado ~2.3 MB en cada instalación de paquete, edición de página y reconciliación por hora. El map guarda las frases traducidas en una tabla compacta y la página se escribe una vez, leyendo de ahí `{townos_title}` y compañía.
- **El camino del 5xx no renderiza nada; lanza `error 503`** al `handle_errors` del propio sitio, que es lo que hace que la página se expanda una vez por bloque de sitio en lugar de una vez por proxy. Verificado: un `error` lanzado dentro de `handle_response` sí llega al manejador de errores que lo envuelve, con la petición original intacta, así que la condición de `Accept` sigue aplicando.
- **El selector tiene un renglón por IDIOMA, no por locale.** Las variantes de país (`de-AT`, `en-GB`, `es-MX`…) derivan de su idioma base y solo sobrescriben lo que ese país dice de otra manera — nada de lo cual son estas cuatro frases — así que un renglón para cada una serían treinta expresiones regulares más renderizando la misma página. `TestUnavailableLocalesCoverEveryCatalog` deriva la pertenencia esperada de `i18n.PopulatedLocales()` y falla en las dos direcciones, de modo que un idioma agregado a los catálogos no puede quedarse aquí en inglés sin que nadie se dé cuenta.
- **El chino tradicional se empareja antes que el `zh` genérico.** Caddy prueba los renglones del map en orden y se queda con el primero que le atina, y `^zh` reclama `zh-TW`, así que el orden equivocado le sirve simplificado a Taiwán, Hong Kong y Macao — que parece una traducción correcta para todo el que no sabe leerla. Cada patrón está anclado, ignora mayúsculas y termina en un separador o al final del encabezado, de modo que `^ja` no puede contestarle en japonés al javanés.
- **El nombre del servicio es un verbo de formato, no una concatenación.** `"%s is unavailable"` pone el nombre en un lugar distinto en japonés que en inglés, así que el orden de las palabras le pertenece al catálogo; el renderizador sustituye `{args[0]}` dentro de la frase traducida, y Caddy sustituye ahí el hostname cuando el bloque de sitio importa el snippet.
- **El locale de la caja llega al ingress por la línea de comandos de la unidad** (`--locale`, `ingressctl.Config.Locale`, leído del ajuste `locale` al generar la unidad). Es un ajuste global de la caja y no estado de rutas, así que viaja con la unidad y no con el empuje gRPC; un cambio de idioma llega al ingress igual que cualquier otro cambio de unidad — una unidad renderizada distinta en la siguiente reconciliación. Un código sin catálogo se ignora en vez de rechazarse: el ajuste es una fila de texto libre, y el costo de una errata debe ser una página en inglés, no un ingress que no arranca.
- **Cada cadena traducida se escapa para las dos fronteras que cruza** (`caddyHTMLText`): primero es un token de Caddyfile y después el contenido de un elemento HTML. El escapado HTML se lleva de paso la comilla que terminaría el token antes de tiempo; la diagonal invertida se escapa aparte, porque Caddy la lee como un escape y el paquete `html` de Go no tiene motivo para tocarla.

### Métricas del ingress

El ingress sirve el formato de exposición de texto de Prometheus en un **tercer listener propio** (`ingress.MetricsPath = "/metrics"`, `ingress.DefaultMetricsPort = 9146`), renderizado por `src/metrics` igual que el endpoint del controlador. No puede ir montado en los dos listeners que ya tiene: esos son la puerta de entrada de la caja, y un vhost para `/metrics` quedaría alcanzable por SNI desde la LAN y desde la overlay.

**Se publica solo en `127.0.0.1`** (`-p 127.0.0.1:9146:9146`), la única excepción deliberada a que el ingress sea agnóstico a la interfaz. El scrape nombra cada hostname que sirve la caja y marca los que todavía no tienen certificado — un mapa de lo publicado y de lo que quedó a medio aprovisionar — y nada lo autentica. Prometheus corre con `--net host`, así que alcanza la publicación en loopback sin salto por red de podman, igual que los objetivos de node-exporter y del controlador.

**El objetivo del scrape no se recompone en ninguna parte.** `ingressctl.MetricsAddrFor(port)` construye tanto la especificación `-p` de la unidad como `monitoring.Ports.IngressMetrics`, el mismo arreglo de fuente única de verdad que tiene `rolodex.Manager.MetricsAddr()`. El job se **omite** cuando `INGRESS_IMAGE` está vacío: una caja sin ingress debe reportar que no hay ingress, no un router que se lee como permanentemente caído.

Lo que se exporta bajo el job `ingress`:

| Métrica | Tipo | Notas |
|---|---|---|
| `townos_ingress_up` | gauge | siempre 1 mientras sirve; ausente cuando no |
| `townos_ingress_start_time_seconds` | gauge | el uptime es `time() - esto`, en el reloj del scraper |
| `townos_ingress_caddy_up` | gauge | 1 cuando el hijo caddy respondió a su API de admin en este scrape |
| `townos_ingress_routes{tls}` | gauge | `local`/`acme`/`pending` — `pending` está programada pero aún sin hoja |
| `townos_ingress_route_https_ready{hostname}` | gauge | 1/0 por ruta, para ver *cuál* nombre no puede servir HTTPS |
| `townos_ingress_path_backends` | gauge | backends por ruta de acceso en todas las rutas |
| `townos_ingress_reloads_total{result}` | counter | `success`/`failure` |
| `townos_ingress_route_changes_total{op}` | counter | `set`/`add`/`remove` |
| `townos_ingress_dropped_total{kind,reason}` | counter | lo que el renderizador rechazó |

Más las **familias propias de caddy**, traídas de la API de admin local al contenedor del hijo (`127.0.0.1:<puerto de administración>/metrics`, construida a partir de la misma dirección que lleva el Caddyfile, de modo que un hijo reubicado no puede dejar el scrape atrás en el 2019) y agregadas a la respuesta tal cual. Van bajo `caddy_*`/`go_*`/`process_*` y no pueden chocar con las `townos_ingress_*`. Pasarlas es la única forma de que lleguen a Prometheus, porque la API de admin está deliberadamente sin publicar, y describen el proceso que de hecho sirve cada petición que la caja contesta. El cuerpo no se vuelve a renderizar: analizarlo de regreso a familias solo podría perder información (caddy exporta histogramas, que `src/metrics` deliberadamente no modela).

Varias decisiones son el punto, no un accidente:

- **`townos_ingress_dropped_total` es la razón de que exista `renderCaddyfileTally`.** Una ruta con un hostname malformado, un backend inservible o un matcher de ruta de acceso inválido se descarta **individualmente** en vez de hacer fallar el render — o sea, se acepta por gRPC, se registra una vez y luego nunca se sirve. Un renglón de log es el instrumento equivocado: nadie lee el journal del ingress hasta que ya se sabe que algo está roto.
- **Los backends por ruta de acceso se validan una vez por ruta, no una vez por vhost.** Una ruta de página está lista para HTTPS y además es `ServeHttp`, así que renderiza dos bloques de sitio a partir del mismo conjunto; validar dentro del escritor contaría cada rechazo dos veces.
- **Cada serie de counter se siembra en cero.** `metrics.Render` omite una familia sin muestras, así que un `townos_ingress_reloads_total{result="failure"}` sin sembrar estaría ausente hasta la primera falla — una alerta que no puede dispararse hasta que el evento que vigila ya ocurrió.
- **`townos_ingress_last_reload_success_time_seconds` se omite hasta que una recarga tiene éxito**, en vez de emitirse como cero: `time() - 0` se lee como una configuración de hace 56 años en lugar de "todavía no hay configuración".
- **Un scrape nunca falla como unidad**, y nunca tumba el router. Un hijo caddy inalcanzable cuesta un gauge, no la respuesta; un listener de métricas que no puede enlazar se registra y se sobrevive, porque negarse a enrutar tráfico por un hueco de monitoreo es el intercambio equivocado.

Ningún dashboard renderiza esto todavía — a diferencia de los jobs `rolodex` y `systemcontroller`, cuyos paneles se declaran contra `RolodexDashboardMetrics()`/`ControllerDashboardMetrics()`. Las series se recogen y se pueden consultar; un conjunto de paneles es un cambio aparte.
...
- **`Forwarders` / `ResolutionMode` / `Blocklists`** -- los ajustes que Town OS posee, resueltos a partir de la configuración almacenada y del descubrimiento en el anfitrión. El gestor no escribe ningún archivo: esto es lo que `ProgramRolodex` empuja al servidor en marcha. Véase «rolodex.yml es solo arranque» más abajo.
- **`Generation`** -- la identidad del socket gRPC que rolodex enlaza al arrancar (dispositivo, inodo, mtime), que cambia justo cuando rolodex se reinicia y es por eso como se advierte un reinicio que se llevó por delante cada ajuste programado. Vacía cuando el socket no está, lo que significa que rolodex no está corriendo.
...
El modo lo puede configurar el operador en tiempo de ejecución con el ajuste `dns_resolution_mode` (`auto` | `recursive` | `forward`; validado por `ValidateDNSResolutionMode`, así que un valor que no se pueda analizar nunca puede llegar a `rolodex.yml` y dejar inservible el DNS). `main.go` lo lee en `rolodex.Config` al arrancar; un cambio con `POST /settings/set` corre `Controller.RefreshDNSResolutionMode`, que programa el servidor en marcha con **`SetResolutionMode`** — sin archivo, sin reiniciar la unidad, sin interrupción del DNS. rolodex rechaza de plano un modo que no reconoce en lugar de irse al predeterminado, y por eso el valor se valida antes de mandarlo.

### La configuración que se congeló

`WriteConfig` solía saltarse cualquier `rolodex.yml` cuyo mtime fuera más nuevo que el del binario del systemcontroller, con la teoría de que un archivo más nuevo había sido editado a mano. **El archivo que escribe cada arranque es más nuevo que el binario de la imagen que lo escribió**, así que del segundo arranque en adelante esa guarda coincidía con la salida del propio controlador, y `WriteConfig` se volvió una operación nula permanente. `rolodex.yml` quedó congelado en lo que renderizó el primer controlador que llegó a correr en ese equipo, por toda la vida del equipo, y ninguna actualización de imagen podía moverlo. `RewriteConfig` existía nada más para rodear esto en los cambios que iniciaba el operador.

Por eso, ninguna sección agregada a la plantilla después del primer arranque de un equipo llegó nunca a ese equipo. Eso se envió dos veces — `dnsbl:` (`31f7e80`) y luego `metrics:` (`9689461`) — y la segunda es la razón de que un equipo desplegado no tuviera ningún listener de métricas de rolodex. Prometheus recolectaba `127.0.0.2:9153` cada 15 s puntualmente, ahí nunca hubo nada enlazado, las series `rolodex_*` jamás existieron, y todos los paneles de DNS renderizaban una **gráfica vacía en lugar de un error** — indistinguible de un resolutor ocioso. `systemctl --failed` estuvo vacío y todos los contenedores arriba todo ese tiempo.

No se protegía nada real. Todos los valores del archivo se renderizan a partir del estado de Town OS — puerto de DNS, reenviadores, modo de resolución, ambas listas de bloqueo, el puerto de métricas — y toda ruta en tiempo de ejecución que cambie alguno ya sobrescribía el archivo de todos modos (eso era `RewriteConfig`). Una edición a mano antes sobrevivía hasta el siguiente cambio de ajustes; ahora sobrevive hasta el siguiente arranque.

**Esta sección es historia.** `WriteConfig` y `RewriteConfig` ya no existen, y Town OS no escribe ningún archivo de configuración de rolodex: los ajustes que antes renderizaban se programan ahora en el servidor en marcha, y el archivo que queda pertenece a la imagen de instalación. Véase «rolodex.yml es solo arranque» más abajo. Se conserva porque la falla que describe — una escritura de configuración que en silencio no hacía nada, y que nada más aflora como una función que nunca funcionó — es la forma de error que este subsistema no deja de producir.

Esas pruebas de regresión se fueron junto con el código que guardaban; lo que las reemplaza está en la sección siguiente.

### rolodex.yml es solo arranque; los ajustes se programan por gRPC

`rolodex.yml` lo escribe la imagen de instalación (`scripts/rolodex-config.sh` en `../install`, como `ExecStartPre` de rolodex) y nadie más. Lleva exactamente las dos cosas que no se pueden fijar en un rolodex en marcha: la **lista de enlaces** de DNS (`127.0.0.2:53`, UDP y TCP — la dirección a la que apunta systemd-resolved, y los listeners tienen que existir antes de que se pueda hacer ninguna llamada a la API) y el **listener de métricas**, que rolodex abre una sola vez al arrancar por la presencia de esa sección. También lleva `resolution.mode: auto` y los reenviadores públicos como VALORES DE ARRANQUE, iguales a `DefaultForwarders` aquí, de modo que el hueco antes del primer envío es un estado definido y no lo que rolodex traiga compilado. Todos sus valores son constantes, así que el archivo es idéntico byte a byte en cada arranque. Town OS no escribe ningún archivo de configuración.

Todo lo demás que Town OS posee — la lista de reenviadores, `resolution.mode` y la lista de bloqueo — lo programa `ProgramRolodex` en el servidor EN MARCHA a través de la API de gestión. Eso es lo que hace gratis un cambio de ajuste: aplicarlo antes significaba reescribir el archivo y reiniciar la unidad, y reiniciar el único resolutor de la caja es una caída de DNS para todo lo que hay en ella. `SetForwarders` se envía sin condiciones (del lado de rolodex es un simple almacenamiento, y no hay lectura con la que comparar); `SetResolutionMode` se compara primero, porque pasar a `auto` reinicia el descubrimiento de escalones y repetirlo por horario tiraría una y otra vez un escalón ya asentado; la lista de bloqueo va por `ReconcileBlocklists`, que ya era leer y comparar (una lista, no dos — la mitad RBL está retirada).

**Tiene que volver a ejecutarse después de cada reinicio de rolodex.** rolodex toma todo esto de su archivo de configuración al arrancar y no persiste nada de lo fijado por gRPC, así que un reinicio que Town OS no ordenó — una caída bajo `Restart=always`, un cambio de concesión DHCP que mueve la unidad a través de `town-os-rolodex-config.path`, un operador reiniciándolo a mano — devuelve la caja en silencio a los valores por defecto de rolodex: resolviendo de una manera distinta a la que el operador eligió, y sin filtrar nada. Nada saca eso a la luz; estos ajustes no los lee de vuelta nadie.

La señal de reinicio es `rolodex.Manager.Generation()`: la identidad (dispositivo, inodo, mtime) del socket gRPC que rolodex enlaza al arrancar. rolodex lo desenlaza y lo vuelve a crear en cada arranque, así que una ejecución nueva siempre tiene un inodo nuevo, y darse cuenta cuesta un solo `stat` — sin ida y vuelta por D-Bus, sin una propiedad de unidad que este código todavía no expone, sin un RPC propio. `reconcileRolodexProgramming` corre en su propio tic de **15 segundos** y no hace nada mientras la generación no cambie; el intervalo es de segundos y no de minutos porque el hueco es una caja resolviendo y filtrando de forma distinta a la configurada. La generación se registra solo tras una pasada completamente correcta, de modo que un fallo parcial se reintenta en vez de quedar recordado como hecho.

**Corregido aguas arriba.** rolodex lanzaba su sondeo de «se llega al :53» nada más cuando una lista de bloqueo estaba habilitada *en su archivo de configuración*, así que al dejar de escribirse las listas ahí el sondeo nunca arrancaba y una lista configurada se degradaba justo en las redes para las que el sondeo existe. Ahora es `DnsblChecker::resolver_availability_loop`, lanzado incondicionalmente y gobernado por la marca de habilitado *en tiempo de ejecución* del revisor — la misma forma que ya tenía el bucle de recuperación de la resolución automática. Una lista prendida por gRPC recibe el sondeo en segundos.

Las pruebas de regresión son `TestProgramRolodex*` y `TestReconcileProgramming*` (unitarias), y `TestProgrammedSettingsSurviveARolodexRestart` y `TestGenerationChangesOnlyWhenRolodexRestarts` (integración) — que programan un rolodex real, lo reinician, confirman que el propio servidor lo ha olvidado y lo vuelven a programar.

**La imagen y el archivo son un par emparejado, y nada negocia entre ellos.** `rolodex.yml` lo escribe `../install` y lo analiza la imagen de rolodex que el equipo esté corriendo, sin apretón de manos de versión en ninguna dirección: serde rechaza de plano un campo desconocido o ausente. Un campo que es obligatorio en la revisión de la imagen y falta en el archivo, o que está en el archivo y la imagen no conoce, es un `failed to parse config file` duro en el arranque, y bajo `Restart=always` eso es un ciclo de caídas con el DNS caído para todo lo que hay en la caja. Ya pasó una vez en el banco de pruebas: la imagen publicada era anterior al cambio de nombre `rbl` → `dnsbl`, y todas las pruebas que levantaban un rolodex real morían con `missing field \`rbl\`` mientras todas las que usaban el mock pasaban. La regla que se sigue es que el `rolodex-config.sh` del repositorio de instalación y la imagen publicada de rolodex se mueven **juntos**, y que subir `rolodex-dns/go` aquí es nada más media subida: véase «Transportes cifrados que la caja SIRVE» para el pin de SVCB que necesitó ambas.

**Una sola guarda, en una sola dirección.** `TestRolodexDohBackendMatchesTheInstallScript` saca `doh.bind` de `../install/scripts/rolodex-config.sh` y lo compara con `RolodexDohBackend`, así que una edición de **este** lado que se aleje del script de instalación falla aquí. La dirección contraria — una edición al script de instalación — nada más se detecta si alguien corre después las pruebas de este repositorio, y la prueba **se salta** donde el repositorio de instalación no está clonado, lo que incluye el contenedor de pruebas. No hay comprobación del lado de instalación, ni una constante compartida generada entre los dos repositorios. Trata la guarda como algo que cubre la dirección que cubre.
...
**`auto` descubre por su cuenta, sin el interruptor.** Al escalón solo se llega después de que las raíces *y* los upstreams cifrados ya fallaron, así que la elección que tiene enfrente nunca es «las raíces o el resolvedor local»: es «el resolvedor local o SERVFAIL». Dejar ahí `DefaultForwarders` en una red que filtra el `:53` hacia cualquier cosa que no sean sus propios servidores no compra privacidad y garantiza la falla: esas son justamente las direcciones que se tiran, así que toda consulta que llega al escalón paga los dos tiempos de espera y se muere de todos modos. Nada queda protegido porque el equipo se niegue a fijarse en el único resolvedor que sí contesta. Por eso `Manager.forwarders` agarra el camino del descubrimiento cuando el modo es `auto` y no hay una lista explícita configurada, diga lo que diga `dns_local_forwarders`.

**Apagado sigue siendo lo predeterminado en los modos donde el intercambio es real, y esa dirección sigue importando.** El resolvedor local ve todos los nombres que busca la casa, que es justo lo que resolver desde las raíces existe para evitar. Ese es un intercambio que un operador hace a sabiendas, no uno que un equipo hace por él la primera vez que una red se porta mal — y en `forward`, donde el escalón es el único upstream y se lleva siempre todas las consultas, es exactamente el intercambio que se está haciendo. Así que el interruptor sigue gobernando `forward`; en `recursive` la lista no se usa de cualquier forma.

**En `auto`, apagar el interruptor deja de suprimir el descubrimiento solo ahí donde no hay una lista explícita a la cual recurrir.** Es fácil decir esto de más — «auto ignora el interruptor» es falso. Con `dns_forwarders` puesto, el interruptor sigue decidiendo: apagado conserva la lista del operador, prendido deriva una y la reemplaza, que es lo que prender el interruptor siempre ha significado. El interruptor queda inerte justo en el caso en que respetar «apagado» significaría programar `DefaultForwarders` en un escalón al que solo se llega porque todo lo demás ya falló.

Tiene que quedar inerte en ese caso, porque el interruptor no puede expresar «apagado a propósito» aparte de «nunca tocado» — es un booleano cuyo valor por omisión es `false`, así que ambos son el mismo valor guardado, y respetar el primero significaría no descubrir nunca para el segundo, que es justo el equipo para el que existe esta función. Un operador que no quiere ningún resolvedor local en el camino pide `recursive`, que lo dice directamente, o nombra el contenido del escalón con `dns_forwarders`. Las pruebas de regresión son `TestAutoDiscoversWithoutTheFlag`, `TestAutoIgnoresTheFlagBeingOffWithNoExplicitList`, `TestAutoStillConsultsTheFlagAgainstAnExplicitList`, `TestAutoLeavesAnExplicitForwarderListAlone` y `TestNonAutoModesDoNotDiscoverWithoutTheFlag`.

**El descubrimiento ya está en el camino de código predeterminado, y eso lo vuelve un peligro para las pruebas.** `auto` es el modo por omisión, así que un `rolodex.Config` en ceros agarra el camino del descubrimiento, y `ForwarderDiscovery` se va a los `/run/systemd/resolve/resolv.conf` y `/proc/net/route` reales de la máquina, con un `Probe` nulo que significa una consulta DNS de verdad. Cualquier prueba que construya un `Manager` y llegue a `Forwarders` — `ProgramRolodex` lo hace, en cada llamada — tiene que fijar `ResolvConfPaths`, `RouteTablePath` y `ForwarderProbe`, o pasa a depender en silencio de la red en la que corre y pasa en cualquier equipo cuyo gateway resuelva. `newProgramTestManager` llena los tres cuando quien lo llama no lo hizo, y `pinRolodexDiscovery` hace lo mismo con cada `rolodex.Config` que arma la suite de integración; la suite de reenviadores locales es el único lugar que los apunta a fixtures a propósito, porque ahí lo que el descubrimiento encuentra *es* el tema. Leer el modo dentro de `forwarders` también lo metió bajo el candado del gestor: `SetResolutionMode` escribía `cfg.ResolutionMode` sin candado cuando nomás lo leía la generación de rolodex.yml, y eso es una carrera en cuanto cada escritura de ajustes y cada pasada de reprogramación resuelven la lista de reenviadores.
...
### Los transportes cifrados que el equipo SIRVE: DoH por el ingress, DoT y DoQ directos

Todo lo de arriba es rolodex como *cliente*: DoH y DoT salen en la lista de escalones de `auto` como maneras en que este equipo llega a internet. Esta sección es la dirección contraria — DNS cifrado **servido a los clientes de la LAN**, que es otro conjunto de listeners con otro dueño.

Los tres los abre el `rolodex.yml` de la imagen de instalación (`scripts/rolodex-config.sh` en `../install`) y nada de aquí. rolodex abre cada uno una sola vez al arrancar, por la presencia de su sección de configuración, igual que el listener de métricas, así que ninguno se puede prender por gRPC — ve [rolodex.yml es solo arranque](#rolodexyml-es-solo-arranque-los-ajustes-se-programan-por-grpc).

**El ingress va delante de DoH.** rolodex enlaza DoH en loopback, en `127.0.0.2:4443` (`RolodexDohBackend`), y el ingress lo publica como cualquier otro vhost en `dns.<tld>` (`DohHostLabel`), pasando ese salto interno por proxy con la verificación apagada (`dohIngressRoute`). El certificado que valida un cliente es entonces la hoja del ingress, emitida por la CA del equipo bajo su propio repo de hojas (`doh`/`resolver`/`current`), de modo que reemitirla nunca mueve las hojas de pages ni de gfeh. Ese es todo el punto del arreglo: rolodex termina su propio TLS con un certificado autofirmado que ningún cliente DoH que valide acepta, y nunca hace falta que nadie lo acepte, porque termina un salto que jamás sale del equipo. Los clientes usan `https://dns.<tld>/dns-query`. Se pasa por proxy el vhost completo y no solo esa ruta, porque el router DoH de rolodex sirve esa ruta y devuelve 404 a todo lo demás — un matcher de ruta aquí nada más agregaría un segundo lugar donde la ruta puede estar mal.

`4443` en vez de `443` no es un detalle. El ingress se publica en `0.0.0.0:443` y rolodex corre con `--net host`, así que un `:443` comodín y un `127.0.0.2:443` específico en el mismo namespace es EADDRINUSE para el que enlace segundo — y se cae el DNS o se cae el ingress, según el orden de arranque. El número también está escrito en dos repositorios que no se pueden leer entre sí: aquí, y `doh.bind` en `../install`. `TestRolodexDohBackendIsTheLoopbackPortInstallWrites` fija este lado contra el literal; `TestRolodexDohBackendMatchesTheInstallScript` lee el script de instalación mismo donde los dos repositorios están descargados, y se salta dentro del contenedor de pruebas, donde `../install` no está.

**El vhost necesita un nombre, y solo los reconciliadores de DNS se lo pueden dar.** Atrás de `dns.<tld>` no hay paquete, ni página, ni partición de almacenamiento de objetos, así que ninguna otra cosa lo nombraría nunca: `RebuildDNS` publica su registro A explícitamente (y el AAAA en un equipo de doble pila, porque la hoja lleva las dos direcciones del host como SAN), y `ReconcileDNS` lo carga en el conjunto deseado para que el barrido horario de huérfanos no borre una hora después del arranque lo que el arranque publicó. Sin el registro, la falla cae antes del TLS y antes de llegar al ingress — un NXDOMAIN pelón, que se lee igualito que una función que nunca se construyó. Pruebas de regresión: `TestRebuildDNSPublishesTheDoHEndpointsName` y `TestReconcileDNSKeepsTheDoHEndpointsName` (unitarias), `TestDohResolvesThroughTheIngress` y `TestDohEndpointResolvesOnRealDNS` (integración).

**DoT y DoQ se brincan el ingress por completo**, y eso es un hueco conocido, no un diseño. El ingress publica solo TCP y su Caddy está fijado a `protocols h1 h2` (`renderCaddyfile`), así que no hay camino de capa 4 por él para un protocolo que no sea HTTP. La imagen de instalación enlaza los dos en la LAN, en `0.0.0.0:853` — la pareja estándar, TCP para DoT y UDP/QUIC para DoQ (el predeterminado propio de rolodex para DoQ es `8853`, que ningún cliente busca). La dirección es `0.0.0.0` y no una específica porque cada sección acepta **una sola** cadena de bind: fijar la primera dirección global no serviría nada antes de la primera concesión DHCP, y nada en absoluto en una segunda dirección. `dns.bind` no puede hacer lo mismo porque `:53` choca con el propio listener DNS de podman; `:853` no tiene ese choque, a cambio de enlazar todas las interfaces, incluidos los puentes de podman y la overlay. IPv6 no está cubierto — una sola cadena de bind no puede nombrar las dos familias.

**Los dos sirven una hoja emitida por la CA de la propia máquina.** La imagen de instalación apunta `dot.tls`/`doq.tls` a `/data/tls/dot/{cert,key}.pem` y `issueRolodexTransportLeaf` escribe exactamente ese par en cada `RebuildDNS`, con los mismos SAN que lleva la hoja del vhost de DoH: `dns.<tld>` más las direcciones de la máquina. Un cliente DoT o DoQ que valide verifica por tanto contra una CA en la que la casa ya confía, en vez de recibir un certificado autofirmado que fijar.

El orden es la razón de que esto sea un archivo y no una RPC. rolodex arranca *antes* que el systemcontroller — el controlador no puede bajar ninguna imagen hasta que algo resuelva nombres — así que en un primer arranque la CA no existe cuando rolodex lee su configuración. rolodex trata un certificado nombrado pero ausente como «sirve uno generado y vigila el real», de modo que emitir la hoja **es** el traspaso: se adopta dentro de un sondeo de 30 segundos, sin reinicio y sin nada que coordinar. Eso también pone en marcha por primera vez en esta máquina la recarga en caliente de certificados de rolodex; el material generado no se sondea nunca a propósito, así que hasta que una escucha apuntó a archivos, todo ese mecanismo era código inalcanzable.

La ruta está bajo `/data` porque es lo único que el contenedor de rolodex puede ver — su unidad monta `/town-os/rolodex:/data` y nada más — así que la hoja va al directorio de datos en lugar de bajo el subvolumen TLS de btrfs con todas las demás. `RolodexTLSSubdir` aquí y `cert_path` en `scripts/rolodex-config.sh` de `../install` son las dos mitades de un mismo montaje escritas en dos repositorios; cambiar una sola deja a rolodex vigilando un archivo que nadie escribe. Pruebas: `TestRolodexTransportLeafLandsInTheMountedDataDir`, `…NamesTheHostAndItsAddresses`, `…IsSignedByTheBoxCA`, `…FollowsAnAddressChange`.

**Un cliente que no confía ya en la CA de la máquina puede verificar el certificado de todos modos**, porque la hoja queda fijada en la zona de la que rolodex es autoritativo. `collectRolodexTransportTLSA` publica registros DANE-EE —`3 1 1`, el SPKI de la hoja troceado— en `_853._tcp.dns.<tld>` y `_853._udp.dns.<tld>` en cada `RebuildDNS`, junto a las fijaciones de los paquetes, las páginas y gfeh. Eso es lo que hace que emitir la hoja valga de algo para un dispositivo que nunca se inscribió en la casa: puede alcanzar este resolvedor, así que puede obtener la fijación, así que puede revisar lo que le están entregando sin instalar antes una CA.

Dos registros para un certificado, porque un registro TLSA lo posee un **endpoint** de servicio y no un certificado, y DoT y DoQ comparten el puerto y la hoja mientras difieren en el transporte. Publicar uno y no el otro es peor que no publicar ninguno: un cliente que revisa DANE y no encuentra registro para el transporte que eligió falla en cerrado, así que la mitad ausente se lee como «el DoQ de esta máquina está roto» y no como «esta máquina no tiene DANE». Para poder expresarlo, `rolodex.TLSAEntry` tuvo que llevar un `Proto`: tenía `_tcp` escrito a fuego, así que la fijación de DoQ no tenía forma de existir. Vacío sigue significando tcp, que es todo paquete que sirve HTTP.

No se publica nada antes de que la hoja esté en disco, por la misma razón por la que `collectGfehTLSA` se salta una partición cuya hoja no se ha emitido: una fijación para un certificado que todavía no existe hace que todo cliente rechace la conexión cuando por fin aparezca. Pruebas: `TestRolodexTransportTLSAPinsBothEncryptedDNSEndpoints`, `…PublishesNothingWithoutALeaf`, y `TestTLSANameProtocol` para la construcción del nombre propietario. De extremo a extremo, `TestDotAdoptsTheCAIssuedLeafWithoutARestart` demuestra que rolodex cambia a la hoja emitida dentro de un sondeo y sin reiniciar — el relevo que ninguna prueba unitaria puede observar, porque trata de otro proceso dándose cuenta de un archivo — y `TestRolodexPublishesTheDoQPinUnderUDP` demuestra que un rolodex vivo guarda y devuelve la fijación bajo `_853._udp`, cosa que un cliente simulado no puede.

**Y se renueva mientras la máquina simplemente está encendida.** `IssueLeaf` reemite un certificado que está a menos de 30 días de caducar, pero solo cuando algo la llama, y quienes la llamaban eran el arranque y un cambio confirmado de IP interna: la renovación era un efecto secundario de reiniciar. `reconcileRolodexTransportTLSA` la pone en la pasada horaria de reparación de deriva (`ReconcileDNS`, que ahora lleva la CA y la base btrfs precisamente para esto), donde los anclajes se reconcilian en la misma llamada: una hoja reemitida lleva una llave nueva, así que la asociación publicada para la anterior deja de coincidir en el instante en que se escribe el archivo, y un cliente DANE que encuentra registros TLSA y no coincide con ninguno RECHAZA la conexión. El anclaje nuevo sube **antes** de que baje el viejo: rolodex adopta el certificado renovado en un sondeo de 30 segundos, y tener los dos registros es el único estado en el que cualquiera de los dos certificados valida. La misma pasada vuelve a publicar un anclaje que la zona haya perdido (un rolodex reinicializado, un desmontaje que se llevó la zona) sin esperar a una renovación, y no reescribe nada mientras el certificado siga vigente.

**A los clientes se les dice dónde está todo esto, dos veces.** `publishDDRDesignation` guarda registros SVCB en `_dns.resolver.arpa.` (RFC 9462) nombrando la URL de DoH, el puerto de DoT y el de DoQ bajo `dns.<tld>`, en ese orden de preferencia — DoH primero porque el `:443` sobrevive al DPI que filtra el `:853`. Un cliente que entienda DDR resuelve ese único nombre contra esta máquina y no necesita que se le diga nada más. La designación vive en `arpa.`, que rolodex no resuelve nunca aguas arriba, y ese es el punto: el rechazo se sitúa por debajo de toda búsqueda local, así que solo el resolvedor al que se pregunta puede responder por su propia designación.

Se elimina antes de reescribirse, a diferencia de todos los demás publicadores de aquí. `RebuildDNS` empieza tirando la zona del TLD, que es lo que hace idempotentes a los otros — pero `_dns.resolver.arpa.` no está en ninguna zona que esta máquina posea, así que el derribo nunca lo alcanza, y sin esa eliminación cada reconstrucción apilaría otra copia o registraría una falla por duplicado. Eliminar primero es además lo que hace que un cambio de TLD surta efecto en vez de anunciar los dos. Pruebas: `TestDDRDesignationIsIdempotentAcrossRebuilds`, `TestDDRDesignationFollowsATLDChange`.

**La designación necesita un rolodex que conozca SVCB**, o sea `v0.6.0` o más nuevo — ese tipo de registro se agregó ahí, y el pin de `rolodex-dns/go` en go.mod se movió con él. Un servidor anterior rechaza el registro, `publishDDRDesignation` lo anota en debug y sigue, y el equipo sirve un DNS cifrado que nada puede descubrir: funcionando, e invisible. Por eso el viaje de ida y vuelta se comprueba contra un rolodex **real** (`TestDDRDesignationRoundTripsThroughRealRolodex`) y no nada más contra un mock, que aceptaría cualquier tipo de registro que le pasen.

**Y una vez más, en una forma que una persona puede leer.** `GET /dns/status` lleva un objeto `encrypted` — la URL de DoH, los nombres y puertos de DoT/DoQ y el nombre de descubrimiento — que la pantalla de DNS presenta en solo lectura. Los extremos se derivan del TLD y del ingreso, así que ahí no hay nada que ajustar; lo que faltaba era cualquier manera de *verlos*, en una máquina que llevaba tiempo sirviendo DoH, DoT y DoQ sin que nada le dijera a nadie dónde. El puerto de DoH publicado es el `443` del ingreso, nunca el `4443` interno de rolodex: un cliente mandado a `4443` alcanzaría una escucha nada más de loopback con un certificado que rechazaría.

**DoT tiene una prueba; DoQ no tiene ninguna.** `TestDotServesDNSOverTLS` arranca un rolodex real con una sección `dot:` y completa un intercambio RFC 7858 contra él — la comprobación de que la sección que escribe la imagen de instalación sí produce un listener que habla el protocolo, y no un socket que se queda colgado. Enlaza loopback en un puerto efímero y no el `0.0.0.0:853` de producción: bajo `--net host` un puerto bien conocido aquí es el *del host*, y dos `make test-full` chocarían en él. DoQ sigue sin pruebas — un cliente QUIC es una dependencia que este repositorio no tiene, y la misma regla de puertos aplica a lo que termine probándolo.

### Un cambio de TLD se tiene que llevar el DNS cifrado con él

El DNS cifrado lleva el nombre del TLD en cuatro lugares a la vez, y cada uno es un publicador distinto:

1. los registros de dirección del endpoint DoH (`publishDohRecord`),
2. la designación DDR que lo anuncia (`publishDDRDesignation`),
3. los SAN del certificado que sirven DoT y DoQ (`issueRolodexTransportLeaf`),
4. los anclajes DANE de ese certificado (`collectRolodexTransportTLSA`).

`rolodex.ChangeTLD` mueve la zona y los registros de los paquetes, y ninguno de estos. Así que `POST /dns/tld` renombraba un equipo de `home` a `lan` y lo dejaba anunciando `dns.home`, sirviéndole a `dns.lan` un certificado que decía `dns.home`, y sin publicar ningún anclaje en el nombre que un cliente iba a marcar de ahí en adelante.

**Eso último es una rotura, no una degradación.** Un cliente que revisa DANE y llega a un endpoint sin encontrar un registro TLSA para él rechaza la conexión — para eso está DANE. La falla no es «el DNS cifrado va sin verificar», es «el DNS cifrado deja de funcionar», y sigue así hasta que el `RebuildDNS` del siguiente arranque lo republica todo. En un equipo que se reinicia una vez al mes, eso es un mes.

Los cuatro son ahora una sola llamada, `publishEncryptedDNS`, y la usan los dos llamadores: `RebuildDNS` y el cambio de TLD. Son una sola función justamente porque nada en los puntos de llamada dejaba ver que iban juntos — cada uno se escribió como su propio publicador con un solo llamador, y el segundo llamador se olvidó de los cuatro. `retireEncryptedDNS` es su inversa para los anclajes: los registros del TLD viejo se caen con su zona en `ChangeTLD`, pero un renombrado que deje una zona en pie dejaría varado un anclaje para un certificado que ya nadie sirve, que es la misma falla-en-cerrado por el otro lado.

`POST /dns/tld` también llama a `reprogramIngress`, por lo mismo: los nombres de los vhosts traen el TLD adentro, el del endpoint DoH entre ellos, y el ingress seguía sirviendo los nombres viejos hasta un reinicio.

Pruebas de regresión: `TestTLDChangeRepublishesTheEncryptedDNSEndpoints` y `TestTLDChangeLeavesNoPinAtTheOldName` (unitarias), `TestTLDChangeMovesTheEncryptedEndpointsOnRealDNS` (integración, contra un rolodex real y su resolvedor en marcha).
...
- `POST /dns/tld` (requiere admin) -- define el TLD. Cambia el TLD existente, vuelve a registrar todos los paquetes instalados, republica los endpoints de DNS cifrado bajo el nombre nuevo y reprograma el ingress para que sus vhosts lo sigan. Ve «Un cambio de TLD se tiene que llevar el DNS cifrado con él».
- `POST /dns/setup` (requiere admin) -- inicializa el DNS y registra todos los paquetes instalados.
- `GET /dns/dnsbl` (requiere autenticación) -- obtiene la configuración de DNSBL (lista de bloqueo de dominios, por nombre directo): la marca global de habilitado, las zonas de proveedor con sus códigos de rechazo **resueltos a lo que está en vigor**, el `refusal_cooldown_secs` de toda la lista y `rotated_out` (los proveedores que ahorita están apartados tras rechazar una consulta, con el código y los segundos que faltan). Ve [Códigos de rechazo](#códigos-de-rechazo-que-un-proveedor-diga-que-dejes-de-preguntar-no-significa-que-esto-esté-listado).
- `POST /dns/dnsbl` (requiere admin) -- reemplaza la configuración de DNSBL. Acepta una marca de habilitado, un `refusal_cooldown_secs` para toda la lista y una lista de proveedores `{zone, enabled, refusal_codes, refusal_cooldown_secs}`. Las zonas se validan como nombres de host completamente calificados, se pasan a minúsculas, se recortan y se deduplican; los códigos de rechazo los valida `ValidateRefusalCodes` (dirección IPv4 o `dirección/prefijo`, enmascarada al prefijo, `"none"` solo como entrada única, sin duplicados).
- `GET /dns/rbl/local` (requiere autenticación) -- lista las entradas de la lista de bloqueo local (`{name, reason}`). La ruta conserva su grafía histórica `rbl`: es un contrato publicado con la interfaz, y las entradas no cambiaron -- lo que desapareció son nada más las consultas a *proveedores* RBL que antes compartían el prefijo.
- `POST /dns/rbl/local/add` (requiere admin) -- agrega una entrada de la lista de bloqueo local. Acepta un nombre (dominio o IP) y un motivo opcional. El nombre se valida (dominio o IP), se pasa a minúsculas y se recorta.
- `POST /dns/rbl/local/remove` (requiere admin) -- elimina una entrada de la lista de bloqueo local por nombre.
...
Los endpoints DNS de solo lectura (`/dns/status`, `/dns/records`, `/dns/rbl/local`, `/dns/dnsbl/allowlist`, `/dns/services`, `GET /dns/tld`, `GET /dns/dnsbl`) se excluyen del registro de auditoría. Las *escrituras* de la lista de permitidos sí se auditan (exentar un nombre de todas las listas de bloqueo es un cambio del que hay que rendir cuentas); igual que las escrituras de listas de bloqueo que reflejan, no traen ninguna acción con nombre en `account.RouteActions` — la ruta las identifica.

### Listas de bloqueo

Rolodex (0.2.4+) da dos mecanismos complementarios de bloqueo de spam/malware/anuncios, más (0.4.3+) un mecanismo para deshacerlos y otro para no creerle a un proveedor que rechazó la consulta, todos expuestos por la API de DNS y la envoltura `rolodex.Client` (`SetDnsblConfig`/`GetDnsblConfig`, `AddLocalBlocklistEntry`/`RemoveLocalBlocklistEntry`/`ListLocalBlocklistEntries`, `AddDnsblAllowlistEntry`/`RemoveDnsblAllowlistEntry`/`ListDnsblAllowlistEntries`). Todos los **consulta rolodex bajo demanda** — Town OS nunca baja, analiza ni precachea fuentes de listas de bloqueo.

**La mitad RBL está retirada.** Las consultas a proveedores por IP inversa, `SetRblConfig`/`GetRblConfig` y los endpoints `GET`/`POST /dns/rbl` ya no existen; rolodex le cambió el nombre a `src/rbl.rs` por `src/dnsbl.rs` y dejó nada más la revisión por nombre. Lo que sobrevive al cambio de nombre es la lista de bloqueo *local*, cuyas rutas HTTP siguen diciendo `/dns/rbl/local*` porque son un contrato publicado, y la llave de ajustes `dns_rbl_config`, que ya no lee nadie y que `controller_dns_blocklists.go` nombra nada más para que no se reutilice.

Fíjate en que `SetDnsblConfig` de la envoltura toma el enfriamiento de rechazo de toda la lista como argumento final (`SetDnsblConfig(ctx, enabled, providers, refusalCooldownSecs)`); se mapea sobre `SetDnsblConfigWithRefusalCooldown` del proyecto de origen, ya que la grafía original que conserva la aridad existe por compatibilidad de API externa, algo que una envoltura interna no necesita.

- **DNSBL** (lista de bloqueo de dominios) -- zonas de lista de bloqueo de dominios que se consultan bajo demanda anteponiendo el dominio que se busca a la zona (p. ej. `googleadservices.com` + `dbl.spamhaus.org`). Las coincidencias de DNSBL le ganan a las respuestas reenviadas/iterativas. Se configura con `/dns/dnsbl` como una lista de proveedores `{zone, enabled, refusal_codes, refusal_cooldown_secs}` más una marca global de habilitado y un `refusal_cooldown_secs` para toda la lista.
- **Entradas locales de la lista de bloqueo** -- una lista respaldada por la base de datos con nombres/IP que se administra a mano con `/dns/rbl/local*`, que se revisa antes que los proveedores externos. Una entrada local de **nombre de dominio** bloquea las búsquedas directas A/AAAA de ese dominio con `NXDOMAIN`, y surte efecto de inmediato (rolodex actualiza una caché en memoria al agregarla).
- **Lista de permitidos de DNSBL** (rolodex 0.4.3+) -- la salida de emergencia del operador ante un falso positivo de una fuente de terceros, administrada con `/dns/dnsbl/allowlist*`. Una entrada cubre el nombre **y todos los nombres debajo de él**, así que permitir `vendor.example` también exenta a `cdn.vendor.example`. **Hace cortocircuito de toda la revisión por nombre**, ganándoles tanto a los proveedores DNSBL configurados como a cualquier entrada local de la lista de bloqueo que coincida, y corre *antes* de la búsqueda del proveedor, así que un nombre exentado nunca emite ninguna. También está respaldada por la base de datos con una caché en memoria, así que surte efecto de inmediato.

  Sin ella, el único remedio ante una fuente que lista un nombre que la casa necesita es deshabilitar todo el proveedor. Fíjate en la asimetría con la lista de bloqueo local: una entrada de la lista de permitidos es **solo un nombre**, nunca una IP, porque la revisión que cortocircuita es la que se basa en nombres — que es el único tipo de consulta a proveedores que hay, ahora que la mitad RBL basada en IP está retirada.
...
**Versión mínima:** un rolodex anterior al manejo de rechazos acepta estos campos — proto3 ignora los campos desconocidos — y no guarda nada. Las pruebas con mocks no pueden distinguir eso del éxito, porque un mock devuelve lo que le entregaron. `TestRolodexDnsblRefusalCodesRoundtripReal` comprueba que una lista configurada **vacía** se lee de regreso *resuelta*, que es la comprobación que una imagen vieja no pasa.

**No hay ingestión ni precacheo de fuentes**: las zonas de proveedor son la unidad de configuración, y la interfaz ofrece una lista curada de zonas DNSBL conocidas como agregados rápidos de un clic, pero el usuario puede agregar cualquier zona. Las escrituras de zonas de proveedor reemplazan toda la configuración (validada, en minúsculas y deduplicada).

**La lista de agregados rápidos es un respaldo, y se cura sobre esa base** (`DNSBL_SUGGESTIONS` en `ui/src/routes/dns/BlocklistsTab.jsx`). Una zona pertenece ahí solo si un equipo casero puede usarla tal cual: que siga operando, sea gratis y le conteste a un resolvedor que recursa por su cuenta sin ningún paso de registro. Ahorita Spamhaus DBL, SURBL, URIBL, NordSpam DBL y Spam Eating Monkey.
...
Fíjate además en que las zonas DNSBL (de dominios) están afinadas para URL de spam en el correo más que para anuncios o rastreadores — el bloqueo de anuncios/rastreadores sería territorio de fuentes, que está [a propósito fuera de alcance](#listas-de-bloqueo). Las zonas por IP inversa que antes las complementaban se fueron con la mitad RBL, y para la navegación casi no importaban: solo se consultaban para las IP que aparecen en consultas DNS inversas, que la navegación normal casi no genera.
...
- **Blocklists** -- la sección de zonas de proveedor DNSBL (interruptor global de habilitado, habilitar/eliminar por zona, ajustes de códigos de rechazo por zona, agregados rápidos de zonas sugeridas, agregar zona personalizada — todo consultado bajo demanda) más una tabla manual de entradas locales (agregar/eliminar). Cada sección empieza con los proveedores que ahorita están apartados tras rechazar una consulta, cuando los hay. Sin fuentes, sin aplicar, sin nada en caché.
...
Cuando la máquina todavía no tiene la imagen de Grafana, esa descarga corre en su propia goroutine y la unidad se cambia solo cuando la imagen ya llegó. De ese orden dependen tres cosas: la petición de ajustes no se bloquea sobre ~771 MB, el backend anterior sigue respondiendo en `:5308` mientras tanto y — como la unidad es `Type=simple`, así que systemd la da por arrancada en cuanto `podman run` hace fork — la máquina no reporta que Grafana está arriba mientras se sigue bajando. `/monitoring/status` reporta `grafana: false` durante toda esa ventana (`MonitoringUIPending`), que es lo que mantiene al tablero en su pantalla de «arrancando» en vez de enmarcar un puerto que todavía sirve el backend anterior. Un segundo guardado concurrente no hace nada en vez de lanzar una segunda descarga, y una descarga fallida limpia la marca y queda registrada.
...
**La salud de los scrapes también se reporta aquí, porque no la reporta nada más.** `/monitoring/status` lleva además `scrape_targets` (la propia visión de Prometheus de cada job que scrapea, cada uno con `job`, `instance`, `health`, `scrape_url` y — en los que fallan — el `last_error` que Prometheus registró), `down_jobs` (nada más los nombres con al menos un target fallando, que es lo que dice el aviso del panel) y `scrape_targets_error` (por qué la lista viene vacía en un equipo donde Prometheus sí está corriendo). `monitoring.FetchScrapeTargets` lee `GET /api/v1/targets` del Prometheus de loopback, con un límite de 3 segundos para que un Prometheus atorado no deje abierto un sondeo de estado, y se pregunta **solo cuando la unidad prometheus está activa**: con la unidad parada su estado ya lo dice, y un error de conexión encima se leería como una segunda falla distinta.

Un target que Prometheus todavía no ha scrapeado reporta `unknown`, no `down`: todos los targets están en ese estado durante un intervalo después de cada reinicio, y llamarlo caído haría que cada arranque pareciera una falla.

Esto existe porque un job de scrape que nunca ha jalado es invisible desde cualquier otro lado. Las dos fallas de métricas que este equipo ha enviado fueron exactamente eso: `prometheus.yml` decía `https` mientras `:5309` servía en claro, y la sección `metrics:` de rolodex nunca llegó a su archivo de configuración. En los dos casos todas las unidades estaban activas, `systemctl --failed` estaba vacío, y los paneles afectados dibujaban una **gráfica vacía en lugar de un error** — indistinguible de un servicio que no tiene nada que contar. Las pruebas de regresión son `TestFetchScrapeTargets*` y `TestHTTPMonitoringStatus{ReportsDownScrapeJobs,SaysWhyItCouldNotAsk,SkipsTargetsWhenPrometheusIsStopped}` (unitarias) y `TestMonitoringStatusReportsADownScrapeJob` (integración, contra un Prometheus real con un job apuntado a un puerto muerto).
...
**El objetivo de recolección no se recompone en ningún lado.** `MetricsScrapeTarget(listenAddr)` lo deriva de la misma cadena a la que se enlaza el servidor y `main.go` le entrega el resultado a `monitoring.Ports.ControllerMetrics` — la misma razón de fuente única de verdad por la que existen `PackageNetworkState.FQDN` y `Manager.MetricsAddr()`. Un enlace comodín (`:5309`, `0.0.0.0:5309`, `[::]:5309`) se reescribe a `localhost` porque un comodín no es una dirección a la que nada se pueda conectar; un host fijado explícitamente se deja en paz, ya que reescribirlo apuntaría la recolección a una dirección en la que el controlador a propósito no está. Un resultado vacío omite el trabajo en vez de apuntarlo a una adivinanza.

**Y el esquema se le pregunta al socket, no se deriva por segunda vez.** `ControllerMetricsScheme` es `https` cuando `systemcontroller.ListenerSpeaksTLS` completa un handshake TLS contra el listener del propio controlador, y el trabajo entonces también trae `insecure_skip_verify` — la hoja la emite la propia CA del equipo, en la que Prometheus no tiene razón para confiar ni forma limpia de que se la entreguen, y la recolección es por loopback dentro del espacio de nombres del host, así que nada más puede contestar por él.

Antes leía `TOWN_OS_TLS` por segunda vez, y en un equipo desplegado las dos derivaciones no coincidieron: `prometheus.yml` traía `scheme: https` mientras `:5309` servía en claro, así que Prometheus abría una conexión TLS, recibía una respuesta HTTP en texto plano y **falló todas las recolecciones por toda la vida de esa configuración**. `insecure_skip_verify` no ayuda en nada contra eso — relaja la validación del certificado, y el problema es que no hay handshake en lo absoluto. Cada una de las veintitantas métricas `townos_*` de arriba se exportaba y se tiraba al suelo, y el único rastro era un trabajo `down` dentro de la propia lista de objetivos de Prometheus, algo que nada en el equipo saca a la superficie. Un handshake no puede estar en desacuerdo con el socket; además cubre `TOWN_OS_TLS_CERT`/`TOWN_OS_TLS_KEY`, que terminan TLS sin pasar por la variable que leía la derivación vieja.

El sondeo está acotado a tres segundos y recurre a la visión del propio arranque cuando nada acepta la conexión — no hay socket al cual preguntarle, y no emitir ningún trabajo cuesta la recolección tan seguro como un esquema equivocado. Cuando el esquema observado y el configurado no coinciden, `SchemeDisagreement` lo reporta una vez por stderr: la recolección queda arreglada de cualquier forma, pero un equipo que sirve la contraseña de acceso en claro después de que el operador pidió TLS es un problema de seguridad que el arreglo taparía. Cubierto por `TestListenerSpeaksTLS*` (unitaria) y `TestPrometheusScrapesTheSchemeTheListenerActuallySpeaks` (de integración), que corre un Prometheus real contra un listener en claro y contra uno con TLS.
...
**El instalador entrega dos imágenes y el controlador consigue las demás.** `install.sh` escribe exactamente dos referencias de imagen en las unidades que deja puestas — `quay.io/town/town` (el systemcontroller) y `quay.io/town/rolodex` — y no hornea ningún contenido de imagen en el squashfs. Todas las demás imágenes de servicio de sistema (la interfaz, el ingress, el controlador de red, el almacenamiento de objetos) las baja **en el propio equipo** el controlador: al arrancar con `parallelEnsureImages` sobre `coreBootImages`, y cuando se pide con `POST /system-services/refresh`. Por eso esos repositorios tienen que ser legibles de forma anónima: el equipo no trae credenciales de registro, y nada en el controlador escribe nunca un `auth.json` ni corre `podman login`. Una imagen cuyo repositorio es privado es una imagen que este diseño no puede bajar, y la falla se ve como una unidad en bucle de caídas con `unauthorized` en vez de algo que nombre la causa real.

**Un temporizador diario corre el mismo procedimiento de actualización que corre el botón de la interfaz, y viene con el instalador.** `town-os-update.timer` y su servicio viven en `../install/systemd/` y se habilitan al construir la imagen (la lista `ENABLE_UNITS` de `make/install.sh`), así que un equipo los tiene desde el primer arranque en vez de conseguirlos de un controlador que ya tiene que estar corriendo. El temporizador se dispara a las `04:23` y hace POST a `/system-services/refresh` — a propósito el mismo endpoint en vez de una segunda implementación de «bajar todo», para que la ruta programada no pueda separarse de la manual. La unidad decide *cuándo*; el controlador decide *qué*. Es `Persistent=true`, así que un equipo apagado a las 04:23 se actualiza en el siguiente arranque en lugar de esperar otro día — en un equipo que está apagado más de lo que está prendido, ese arrastre es la única actualización que va a recibir. El horario evita a propósito el `03:17` del podman prune: una limpieza recogiendo imágenes sin referencias mientras una actualización baja otras nuevas es una carrera sin nada que ganar.

**El POST del temporizador va sin credenciales, y por eso la ruta es `localhostOrAdmin`.** La unidad no tiene ningún token que presentar, así que `/system-services/refresh` acepta llamadas sin autenticar **solo desde loopback** y sigue pidiendo admin desde cualquier otro lado — la misma excepción en la que se apoya `GET /metrics`, sólida por la misma razón: un paquete con origen en `127.0.0.0/8` no se puede enrutar hasta el equipo desde la red. Por eso la unidad generada apunta explícitamente a `127.0.0.1` y nunca a la dirección enrutable del equipo, que mandaría un POST sin credenciales por la LAN y además sería rechazado del otro lado. El puerto sigue a la dirección `-listen` del controlador, así que un controlador reubicado (el arnés de integración le da a cada corrida su propio puerto) se sigue actualizando.

**`auto_update_enabled` limita al temporizador, no al operador.** El temporizador marca su propia llamada con `?scheduled=1`; solo una llamada que traiga esa marca consulta el ajuste. Un administrador que le pica al botón de actualizar siempre actualiza, porque un switch que dice «actualizar automáticamente» no tiene nada que opinar sobre una petición explícita. Una corrida programada que se omite responde `200` con `{"status":"skipped"}` en vez de un error — el temporizador preguntó si debía actualizar y recibió la respuesta válida «ahorita no», que no es una falla que `systemctl status` deba pintar de rojo. El temporizador se queda instalado y corriendo aunque el ajuste esté apagado, así que cambiarlo surte efecto en el siguiente disparo sin cirugía de unidades y sin manera de que el estado de la unidad y el ajuste se contradigan.

El ajuste viene **prendido** por defecto, y los valores que no se reconocen se leen como prendido. Apagado es una lista cerrada (`0`, `false`, `off`, `no`, sin distinguir mayúsculas); todo lo demás — incluyendo un dedazo y una fila de ajustes que no se puede leer — deja las actualizaciones corriendo. La asimetría es a propósito: como el instalador entrega nada más dos imágenes, un equipo que deja de bajar es un equipo que nunca consigue la mayoría de sus servicios, así que el costo de equivocarse hacia «apagado» es mucho mayor que el de una bajada de más.
...
Las fallas de arranque del monitoreo, la configuración de Rolodex, las descargas de imágenes centrales, la CA TLS, el ingress, el servicio de pages, el almacenamiento de objetos, la reconciliación de redes y el contenedor de la interfaz no son fatales; el sistema sigue sin ellos. Todas las descargas de imágenes de contenedor usan el ayudante `ensureImage` (`systemcontroller.EnsureImage`) que se describe abajo. Las fallas de descarga de servicios que no son esenciales se registran en stderr y no impiden el arranque, dejando que el sistema arranque incluso cuando la red no está disponible por un rato.

**El arranque refresca las etiquetas de imagen flotantes** — `EnsureImage` clasifica una referencia con `FloatingImageRef` antes de decidir qué hacer con ella:

- **Flotante** — la familia `latest` en cualquiera de sus formas: `latest`, `rc.latest`, `release.latest`, con o sin el sufijo `-x86_64` / `-aarch64`, más una referencia que no trae etiqueta alguna (podman la resuelve a `:latest`). Estas se vuelven a bajar en **cada** arranque, incluso cuando la imagen ya está en el almacén local. Para una etiqueta que se mueve, «presente» y «al día» son preguntas distintas, y un `podman pull` sobre una etiqueta que no cambió es una revisión de manifiesto, no una descarga.
- **Fijada** — etiquetas con fecha (`rc.<date>-<arch>`, `release.<date>-<arch>`), referencias por digest, etiquetas de prueba por instancia y cualquier cosa bajo `localhost/` (que no tiene registro del cual refrescarse). Se bajan solo cuando faltan.

Que falle el refresco de una imagen que *sí* está presente localmente no es un error: se usa la copia local y se registra la falla, así que una máquina sigue arrancando sin registro — la misma razón por la que su propia unidad usa `--pull=missing` y no `--pull=always`.

**El controlador se actualiza a sí mismo** — `SelfUpdate` corre como primer paso de `boot_services` y es la otra mitad del mismo problema: `EnsureImage` mantiene al día todos los servicios que corre una máquina, pero no puede hacer nada con el proceso que lo corre. La unidad del propio controlador es `--pull=missing` a propósito (un reinicio tras una caída no debe depender de que el registro esté arriba), así que el controlador en ejecución se quedaba en la imagen con la que se instaló la máquina hasta que un operador le picaba a *refresh system services* — una máquina que actualiza todo menos lo que hace las actualizaciones es el único componente que nunca puede entregar un arreglo para sí mismo.

El paso:

1. Le pregunta a podman de qué referencia de imagen se arrancó *este* contenedor (`{{.ImageName}}`) en vez de derivarla de `TOWN_OS_TAG`. El sistema de build de instalación mete en la unidad el `CONTROLLER_IMAGE` que le hayan dado — que puede ser otro registro completo — y refrescar una referencia que la unidad no corre compararía dos imágenes sin relación y reiniciaría la máquina en cada arranque.
2. No hace nada cuando esa referencia está fijada (un operador fijando la máquina, o una imagen `localhost/` bajo `make dev`).
3. Le aplica `EnsureImage` y luego compara el id que la etiqueta nombra ahora con el id que este proceso está corriendo.
4. Cuando difieren, escribe el id destino en `<btrfsPath>/town-os-self-update` y le pide a systemd que reinicie `town-os-systemcontroller.service`. El `--pull=missing` de la unidad arranca entonces la imagen que el refresco acaba de dejar local. El arranque espera ahí en vez de seguirse a reconcile con trabajo que está por descartarse.

La marca es el seguro contra bucles: si un reinicio regresa corriendo la misma imagen vieja — una unidad que resuelve la etiqueta distinto a como lo hicimos — la máquina lo intenta una vez, lo registra y sigue, en lugar de reiniciarse en cada arranque para siempre. Una etiqueta que se mueve otra vez es un destino nuevo y tiene su propio intento, y un reinicio que systemd rechaza borra la marca, porque un reinicio que nunca pasó no debe suprimir el siguiente.

La consecuencia para una máquina ya instalada: el controlador que corre tiene que traer este código antes de poder usarlo, así que sigue haciendo falta exactamente un *refresh system services* manual (o una reinstalación) para cruzar. Después basta con reiniciar.

`TOWN_OS_IMAGE_REFRESH=0` (también `false`/`no`/`off`) apaga el refresco — incluida la autoactualización, que entonces no encuentra nada movido — con lo que la regla de las fijadas aplica a todo; una imagen que falta se sigue bajando. `make dev` y los dos contenedores de prueba lo ponen, porque a propósito cargan desde el caché de imágenes local todas las imágenes que corren — un refresco ahí iría a quay.io en cada arranque a reemplazar una imagen que se puso a propósito. Cualquier valor que no se reconozca deja el refresco prendido: una máquina que deja de conseguir sus propios servicios en silencio es una falla peor que una descarga de más. Ponerlo en el contenedor es apenas la mitad: el PID 1 de esos contenedores es systemd, así que la lista blanca `PassEnvironment` de la unidad del arnés (`integration/testdata/town-os-systemcontroller.service`) también tiene que nombrar la variable, o se queda en el PID 1 y el controlador — el único proceso para el que se puso — nunca la ve. No la nombraba, así que el refresco corría en cada arranque de test y de dev: rolodex, Prometheus y node-exporter se volvían a bajar de quay.io dentro del conjunto de arranque mientras el arnés le daba 120 segundos a `/status/ping` para responder, y que una corrida fallara dependía del enlace. `TestHarnessEnvironmentReachesTheController` ahora deriva los nombres requeridos de `make/test.sh`, `make/dev.sh` y `system_port_env`, y la suite de integración lee el `/proc/<pid>/environ` del controlador corriendo para probar que sí llegaron.

La distinción importa porque el instalador no trae ninguna imagen y la etiqueta de toda máquina flota por omisión (`rc.latest-<arch>`). Sin ella, «ya cargada» quería decir «al día», y cada servicio de sistema se quedaba en la imagen que la máquina bajó la primera vez que arrancó — una VM reinstalada seguía sirviendo una interfaz de hace meses por más veces que se reiniciara, y lo único que la hacía avanzar era un `POST /system-services/refresh` del operador.
...
Por eso mismo `make dev` es el único objetivo al que un puerto le puede *impedir* arrancar: en una máquina hay un solo `127.0.0.2:53`, y más de una cosa lo quiere — el rolodex de host del repo install, la caja de dev de otro checkout, un contenedor de servicio de sistema huérfano de una corrida de dev que mataron a lo bruto. Por eso `require_free_rolodex_dns` (`make/dns.sh`) se niega a arrancar cuando, ya quitado el contenedor de este checkout, la dirección sigue ocupada, y dice qué hay que parar. Antes la redirección nomás esperaba una conexión TCP a esa dirección, que funciona igual de bien cuando quien contesta es el resolutor de alguien más: el rolodex de este checkout no lograba enlazar y el host quedaba apuntando a un rolodex que no sabe nada de esta caja de dev — indistinguible, desde el navegador, de que el DNS propio de la caja estuviera roto. `wait_for_rolodex_dns` ahora exige las dos mitades, que la unidad esté activa (lo que hace que el resolutor sea *nuestro*) y que la dirección esté enlazada (lo que lo hace usable, porque la unidad se activa en cuanto `podman run` está arriba, antes de que rolodex haya abierto un socket), y es **fatal** en lugar de una advertencia: una caja de dev cuyo resolutor nunca subió no es una caja de dev, y seguir adelante producía una que parecía arrancada mientras cada nombre `.home` del host se iba al resolutor de la LAN. La redirección pasa en cuanto el controlador está listo, antes de la cuenta y del servidor de UI, porque todo lo que viene después de ese punto ya es una caja de dev que está arriba. Lo cubren `TestDevDNSRefusesToStartWhenTheRolodexAddressIsTaken` y las pruebas `TestDevDNSWait*`, que sustituyen `ss`, `resolvectl` y `${SUDO}` — ninguna prueba puede tocar el resolutor del host.

Queda la cuestión de quién renderiza el `rolodex.yml` desde el que una caja de dev toma sus binds. No lo hacía nadie, y ese es el error que encontró la espera fatal de arriba: rolodex abre su listener de DNS **una sola vez**, al arrancar, desde ese archivo, y nada de lo que Town OS pueda decirle por gRPC abrirá uno — `ProgramRolodex` posee los forwarders, el modo de resolución y ambas listas de bloqueo, y deliberadamente no los binds (`controller_rolodex_config.go`). En una caja real lo renderiza la imagen de install (`../install/scripts/rolodex-config.sh`, su único escritor). `make dev` no corre instalador alguno, así que `${STATE_DIR}/dev-rolodex` estaba vacío y `town-os-system--rolodex.service` — cuyo `ExecStartPre` crea el *directorio* — arrancaba rolodex con `--config /data/rolodex.yml` contra un archivo que nunca había existido. La unidad pasaba a activa de todos modos, porque el contenedor arranca perfectamente y lo único que falta es el listener, que es justo por lo que la espera pregunta por la dirección y no sólo por la unidad. Ahora `seed_dev_rolodex_config` (`make/dns.sh`) lo escribe en el anfitrión antes de que el contenedor monte el directorio: sólo loopback — los binds de DNS, el listener de métricas, el socket gRPC que marca el controlador, un par de forwarders de arranque — y reescrito en cada corrida, porque `dev-rolodex` persiste para guardar `rolodex.db` y una configuración de un esquema viejo sobreviviría al checkout que la escribió. Se siembra en el anfitrión en vez de hornearse en la imagen a propósito: ese mismo archivo de unidad viaja en la imagen de las pruebas de integración, y un `rolodex.yml` que amarre `127.0.0.2:53` dentro de un contenedor de prueba con `--net host` amarraría el `:53` **del anfitrión** en cada corrida. Sin esto, cada arranque pagaba además dos esperas incondicionales de treinta segundos — `Manager.WaitForDNSReady` y luego el bucle de marcado del socket gRPC en `reconcileDNSAndNetworks` — dentro del presupuesto de 120 s que `require_controller_ready` le concede a un arranque que responde 503 a `/status/ping` hasta estar del todo terminado.

Ese mismo arranque es lo que decide qué tiene que meter `make dev` en el contenedor antes de arrancarlo. Toda unidad generada ejecuta `podman run` **sin** `--pull=never`, así que una imagen que no está en el almacén interno no es un error que falle rápido: es un jalón del registro ocurriendo dentro de `podman run` mientras los 120 s de `require_controller_ready` y la espera de disponibilidad del propio servicio corren al mismo tiempo. El ingress y el caddy del servicio de páginas faltaban los dos en esa lista (caddy también en el banco de integración), así que una caja de dev los traía de quay y de Docker Hub durante el arranque con los tar ya guardados en el caché de imágenes del checkout. Ahora se cargan ambos, con el ingress construido por un prerrequisito `ingress-image`. Dos cosas se cargaban que no debían: **Grafana** ocupa ~1.17 GB y es el backend de monitoreo que *no* es el predeterminado, así que el arranque nunca lo ejecuta salvo que el backend almacenado sea `grafana` — se omite salvo con `DEV_LOAD_GRAFANA=1`; y **gfeh** ya no es prerrequisito de `dev`, porque es la única imagen local cuyo contenido viene de fuera del repositorio y por eso recompila el árbol de dependencias completo de `gfehd` una vez por día natural, para todos, se use o no. `make gfeh-image` habilita el almacenamiento de objetos en dev exactamente como `make/dev.sh` lo ha dicho siempre. La carga misma pasa ahora por el caché de imágenes montado de solo lectura en `/image-cache` en vez de `podman cp`, lo que elimina una escritura completa de cada tar; un contenedor sin ese montaje sigue recurriendo a la copia. Por último, el systemcontroller **ya no se habilita** en la imagen de dev: systemd lo arrancaba en el instante en que `podman run -d` regresaba, contra un almacén vacío, lo que compraba un arranque malo garantizado — los 30 s completos de espera de DNS, jalones de todo lo aún no cargado, unidades arrancadas contra imágenes ausentes, cada repositorio de paquetes clonado — todo ello descartado por el arranque que `make/dev.sh` emite una vez terminada la carga, y todo ello compitiendo con esa misma carga. rolodex sigue habilitado, porque tiene que estar sirviendo antes de que arranque el controlador y su unidad está hecha para sobrevivir a esa espera.
...
- `INGRESS_METRICS_PORT` -- el puerto en el que el ingress sirve su endpoint `/metrics` de Prometheus, publicado solo en `127.0.0.1` (`9146` por omisión). Es un tercer listener junto a los dos de arriba y necesita su propia anulación por la misma razón que el de rolodex; `ingressctl.MetricsAddrFor()` es la única función de la que se construyen tanto la especificación `-p` de la unidad como el objetivo del scrape, así que reubicarlo mueve los dos. Véase [Métricas del ingress](#métricas-del-ingress).
...
| `dns_dnsbl_config` | sin configurar (no es lo mismo que vacío) | `POST /dns/dnsbl` |
