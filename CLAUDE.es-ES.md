...
- **Las etiquetas de imagen se particionan por arquitectura** — toda etiqueta subida lleva un sufijo de arquitectura en la forma cruda de `uname -m` (`<arch>` es `x86_64` o `aarch64`). Este sufijo de etiqueta es deliberadamente distinto del nombre de plataforma OCI `amd64`/`arm64`: Go mapea `runtime.GOARCH` al sufijo mediante `archTag()`, make usa `HOST_ARCH` (normalizado a `x86_64`/`aarch64`) y el shell usa `host_arch_tag` en `make/lib.sh`. Los valores simples `host_arch` / `runtime.GOARCH` siguen siendo `amd64`/`arm64` porque podman los necesita para `podman pull --platform linux/<arch>` y para las comparaciones de `.Architecture` — nunca pases `x86_64`/`aarch64` a `--platform`. `push-rc` sube `rc.<fecha>-<arch>` / `rc.latest-<arch>`; `push-release` sube `release.<fecha>-<arch>` / `latest-<arch>`; `push-tag PUSH_TAG=<etiqueta>` sube `<etiqueta>-<arch>` — siempre la arquitectura nativa del host que ejecuta el push. **Esto incluye la etiqueta que elige el operador**: `push-tag` subía `PUSH_TAG` tal cual, así que un `make TARGET=x86_64 push-tag` seguido de la ejecución aarch64 dejaba en quay la que corriera en segundo lugar, bajo un nombre que no nombraba ninguna arquitectura. Los nombres simples (`rc.latest`, `latest`, las etiquetas con fecha y una `PUSH_TAG` personalizada) existen SOLO como listas de manifiestos multiarquitectura ensambladas por `manifest-rc` / `manifest-release` / `manifest-tag` después de que todas las arquitecturas de `ARCHES` (`x86_64 aarch64`) hayan subido; nunca subas un nombre simple como etiqueta de una sola arquitectura. `TestEveryPushNamesAnArchitecture` comprueba que todo `podman push` de `build.sh` lleve `${ARCH}`, de modo que esta clase de fallo no puede volver por una rama nueva — los nombres simples se siguen distribuyendo, pero solo a través del `podman manifest push` de `build_manifest`, que es otro comando. El respaldo en tiempo de ejecución cuando no se incrustó ninguna etiqueta es `defaultVersionTag()` en `main.go` (`rc.latest-<arch>`, con el GOARCH mapeado por `archTag()`). Motivo: una etiqueta simple de una sola arquitectura subida desde un host falla en la otra arquitectura con `exec format error` (o peor, pasa espuriamente las pruebas de sondeo de estado mientras entra en bucle de caídas bajo `Restart=always`).
...
- **Las pruebas y el desarrollo construyen imágenes `localhost/`; los targets de push siempre construyen una imagen de release nueva** — los brazos `*-local` de `make/build.sh` producen `localhost/town-os-*:$(INSTANCE_ID)` para los arneses de pruebas y de desarrollo; los brazos `release-*` producen `quay.io/town/*`. **Ningún target de push puede construir una imagen `localhost/*`, etiquetar a partir de ella ni depender de ella**, y todo target de push debe construir una imagen de release *nueva* en lugar de reetiquetar lo que el almacén local tenga por casualidad. Esto se aplica a todas las imágenes, sin excepción. Motivo: reetiquetar una imagen local de pruebas publica bajo un nombre de release unos binarios construidos para el arnés — etiquetas por instancia, bases con `--pull=never`, solo la arquitectura del host, nunca compilación cruzada. En un checkout limpio eso falla; en la máquina de un desarrollador tiene éxito y publica los binarios equivocados, que es peor.

- **Una imagen local cuyo contenido viene de fuera del repositorio necesita una invalidación de caché explícita** — la mayoría de las imágenes `*-local` se construyen a partir del código del repositorio, así que un cambio en el código invalida su caché de capas y no pueden divergir de su contraparte de release. Una cuyo contenido se descarga en tiempo de construcción (`Containerfile.gfeh` ejecuta un `cargo install gfehd` sin versión) queda detrás de una línea `RUN` idéntica byte a byte, de modo que su capa es un acierto de caché permanente y se congela en lo que fuera actual la primera vez que se construyó en esa máquina. Las construcciones de release pasan `--no-cache`; los fixtures locales pasan un build-arg con granularidad de día (`GFEH_CACHE_DATE`) para refrescarse a diario sin recompilar en cada ejecución. Sin él, la suite de integración prueba en silencio un demonio que Town OS ya no puede ejecutar.
...
**Otros dos repositorios dependen de este a través de un contrato escrito, y cada
uno mantiene un `TOWNOS_CONTRACT.md` que solo sigue siendo cierto mientras
alguien lo cambie a la vez que el código de aquí. Ninguno de los dos
repositorios ve al otro, y nada en este checkout falla cuando discrepan — un
cambio que aterriza aquí sin su edición del contrato es una batería de pruebas
en verde de este lado y una máquina rota del otro.**
...
- **gfeh** (`~/src/github.com/town-os/gfeh`) es un *cliente* de Town OS:
  ...
  contrato, no una refactorización. Un nombre de campo que se ha desviado
  devuelve 422 contra el Town OS real mientras todas las pruebas del emulador
  siguen en verde, que es justo el fallo que el contrato existe para impedir.

- **rolodex-dns** (`../rolodex-dns`) va en la dirección contraria: es algo que
  Town OS *dirige* por gRPC, así que su contrato es sobre todo lo que este
  repositorio puede dar por supuesto de rolodex, y también cubre `../install`,
  porque el `scripts/rolodex-config.sh` de allí es el único que escribe el
  ...
  generado entre ellos, lo menos defendido del contrato) y las direcciones
  fijas: `RolodexDohBackend` (`ingress_doh.go`), `DNSLoopback` y
  `DefaultMetricsPort` (`rolodex.go`), `RolodexTLSSubdir`
  (`rolodex_transport_tls.go`). rolodex no persiste nada de lo que se le indica
  por gRPC, así que un envío de ajustes que nunca llega se ve exactamente igual
  que uno que sí llegó; por eso esto se comprueba en vez de descubrirse.

**Cuando un cambio de aquí toque cualquiera de esas cosas, edita el
`TOWNOS_CONTRACT.md` de ese repositorio junto con él y vuelve a ejecutar su
comprobación desde ese checkout** — `make check-townos-sync`, el mismo nombre de
target en ambos. Las dos son rápidas, sin red y de solo lectura, así que
ejecútalas cuando las necesites; son el único sitio donde la discrepancia se
vuelve ruidosa. **No** ejecutes el `check-townos-sync-release` de gfeh: descarga
el HEAD de Gitea por red y existe como puerta de `make publish` de aquel lado.

Tres cosas sobre esos contratos que deciden cómo trabajar con ellos:

- **Un salto no es un aprobado.** Ambos scripts se saltan limpiamente cuando el
  checkout que quieren no está (el de rolodex quiere `../town-os` *y*
  `../install`; el de gfeh usa por defecto
  `$HOME/src/gitea.com/town-os/town-os`), porque cada repositorio tiene que
  compilar en una máquina que solo lo tenga a él. Una ejecución que imprimió
  `skipping` no verificó nada.

- **Nada está fijado a una revisión en ninguno de los dos lados, y es
  deliberado.** Las comprobaciones resuelven lo que haya en el checkout en el
  momento en que se ejecutan, así que "verificado" significa contra Town OS *tal
  como es ahora* y no como era cuando alguien se acordó por última vez de subir
  una constante. No introduzcas un pin, ni escribas una revisión en un
  documento: una revisión anotada que ningún script lee es una afirmación que
  nadie mantiene.

- **El contrato de rolodex está traducido, y su comprobación de traducciones lo
  exige.** El `TOWNOS_CONTRACT.md` de allí lleva las mismas cinco traducciones
  que este archivo y lo cubre `translation-drift-check.py`, que corre en el
  `make lint` de aquel repositorio. Editar solo el archivo en inglés convierte
  un arreglo del contrato en un fallo de lint allí. El contrato de gfeh es solo
  en inglés.

## Arquitectura de las Imágenes de Release

**Dos arquitecturas deben poder compilarse a la vez, en el mismo checkout. Basta con que funcione una compilación por arquitectura a la vez — el caso que jamás debe corromper ninguna de las dos es una compilación x86_64 y una aarch64 ejecutándose de forma concurrente.**

Eso se reduce a una sola regla:

- **Nada de lo que produce una compilación de release puede llevar un nombre sin su arquitectura, y ningún objetivo de push puede etiquetar a partir de un nombre que carezca de ella.** Compila hacia `$(staged_ref "$IMAGE")` (`make/lib.sh`), que es `<imagen>:local-<arch>`, y etiqueta desde ahí cada etiqueta publicada con `tag_from_staged` — nunca `podman tag "${RELEASE_X_IMAGE}" ...`, que no nombra arquitectura alguna y por tanto resuelve a `:latest`.

POR QUÉ: porque esto llegó a distribuirse. Todas las imágenes de release salvo el systemcontroller se compilaban como el `quay.io/town/<nombre>` a secas — una única ranura `:latest` por imagen, compartida por ambas arquitecturas — y los objetivos de push reetiquetaban lo que hubiera en esa ranura en ese instante. Una compilación aarch64 y una x86_64 en el mismo checkout se sobrescriben mutuamente la ranura, así que se publicó `rc.latest-x86_64` conteniendo binarios arm64 de **ingress, networkcontroller y ui**. Todos esos servicios entraban en bucle de caídas al arrancar con `exec container process: Exec format error`, y nada falló en el momento del push para advertirlo. El systemcontroller se libró únicamente porque `push-rc` daba la casualidad de recompilarlo directamente en la etiqueta con sufijo de arquitectura.

`tag_from_staged` llama a `assert_image_arch` antes de cada etiquetado, de modo que una regresión de esta clase falla en el push indicando la arquitectura que encontró y la que esperaba, en lugar de fallar en la máquina de otra persona. Añadir una imagen de release nueva significa añadirla al patrón de staged-ref; un `podman tag` desde un `${RELEASE_*_IMAGE}` a secas es el fallo.

Las *cachés* compartidas pueden seguir siéndolo (`.cache/go-mod`, `.cache/go-build`, `.cache/cargo-registry`, la caché de bun): Go y cargo bloquean sus propias cachés, y los tar de la caché de imágenes ya están indexados por arquitectura. Lo que colisiona son los *nombres* de las imágenes.

La misma regla alcanza a las imágenes *base* que una compilación consume, no solo a las que produce:

- **Una imagen base se prepara en la arquitectura que quiere la compilación actual, nunca de forma indiscriminada en la del host.** `BASE_IMAGES_RUNTIME` (en el Makefile) es el subconjunto de `BASE_IMAGES` que un Containerfile compilable de forma cruzada nombra con un `FROM` a secas — las etapas que se distribuyen — y esas siguen a `TARGET`. Todo lo demás son bases de cadena de herramientas que cada Containerfile cruzado fija con `FROM --platform=$BUILDPLATFORM` porque se ejecutan AQUÍ y compilan de forma cruzada, así que la arquitectura del host es la correcta para ellas bajo cualquier `TARGET`. Preparar una base de cadena de herramientas en la arquitectura de destino es este mismo fallo apuntando en sentido contrario.

POR QUÉ: `load-base` es prerrequisito de casi todos los objetivos de compilación, `release-image` bajo `TARGET=aarch64` incluido, y recorría `BASE_IMAGES` llamando a `ensure_image` sin arquitectura. Así que los propios prerrequisitos de una compilación cruzada forzaban `debian:bookworm-slim` de vuelta a amd64 justo antes de que la rama de release necesitara arm64, que la volvía a preparar, y los prerrequisitos de la siguiente invocación lo deshacían. Cada invocación cruzada pagaba un `rmi` más una carga en cada sentido — y una descarga de red siempre que faltara el tar que quería — mientras `podman image inspect` informaba de la arquitectura del host todo el tiempo, lo que se lee exactamente como si la preparación en la arquitectura de destino no ocurriera en absoluto.

Dos consecuencias que conviene retener:

- **Cada rama de compilación prepara sus propias bases, las locales incluidas**, en lugar de apoyarse en la pasada global de `load-base`. `gfeh-image` y `release-gfeh-image` no tienen prerrequisito `.images-pulled`, así que hasta ahora nada había preparado nunca `debian:bookworm-slim` para ellas y se resolvían contra lo que la última compilación hubiera dejado atrás.
- **La descarga implícita del `FROM` no es gratis.** Las compilaciones cruzadas prescinden de `--pull=never` para que podman pueda traerse él mismo la base de runtime de la arquitectura de destino, y esa descarga no pasa ni cerca de `ensure_image`: ni lee la caché ni escribe en ella. Preparar la base a través de `ensure_image` *antes* de que la compilación la pida es lo que convierte la primera compilación cruzada de una base en la última que cuesta algo.

Las salvaguardas se derivan en lugar de enumerarse, de modo que una etapa nueva no puede quedarse fuera en silencio: `TestBaseImagesRuntimeMatchesTheContainerfiles` calcula la pertenencia esperada a partir de los Containerfiles compilables de forma cruzada y falla en ambas direcciones, y `TestBuildArmsStageEveryRuntimeBase` hace lo mismo por rama.

Del mismo sitio salen dos reglas más — una publicación cruzada no es cruzada de principio a fin:

- **La fase de pruebas de una publicación es NATIVA, sea cual sea el `TARGET` de los artefactos.** `release-build` depende de `release-test`, que recursa con `TARGET=` limpiado, nunca de `test-full` directamente. `test-full` construye el banco de pruebas de integración y lo *ejecuta* aquí, así que cada una de esas ramas llama a `require_native_target` y rechaza un `TARGET` ajeno — nombrar `test-full` directamente hacía que `make TARGET=aarch64 release-build` muriera en `make/build.sh ui-integration` antes de construir una sola imagen de publicación, y `push-release` moría con él. Las pruebas validan el CÓDIGO FUENTE, en la máquina que las ejecuta; la parte cruzada de una publicación son los artefactos que se construyen después. (`TestReleaseBuildRunsItsTestsNatively`.)

- **Una imagen que existe para una sola arquitectura se OMITE en las demás, no se intenta.** El runner de Proton es x86_64 por construcción (GE-Proton distribuye Wine de x86_64), así que `release-proton-image` rechaza cualquier otro `TARGET` — correcto para el objetivo suelto y equivocado para un agregado que lo nombra sin condiciones. El Makefile lo descarta mediante `$(PROTON_RELEASE_TARGET)`, y **todas las ramas tienen que coincidir**: una rama de push que siguiera etiquetándolo buscaría una imagen preparada que nadie construyó, y un `build_manifest` sobre `ARCHES` buscaría una etiqueta `-aarch64` que deliberadamente nunca se subió. De ahí la lista de arquitecturas opcional de `build_manifest`, que por defecto es `ARCHES` y a la que se le pasa `x86_64` para Proton. Añadir otra imagen de una sola arquitectura significa repetir las tres cosas. (`TestProtonStaysOnItsOwnArchitecture`.)
