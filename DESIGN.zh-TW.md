...
3. **用啟動處理器繫結 `:5309`** —— `NewBootStatus()` + `NewRootHandler(NewBootHandler(bs))` 在任何啟動工作之前立即繫結監聽。在第 30 步的切換髮生之前，該套接字只應答 `GET /status/ping`（503，附 `{booting, step, done, boot_id}`）與 `GET /boot-status`（SSE）；其餘一律 403。
...
15. **階段 `boot_dns`：建立 rolodex 管理器並等待 DNS 就緒** **(非致命)** —— Rolodex 是由 systemd 管理的啟動服務，到這一步它已經在應答了：它比 systemcontroller 更早啟動，憑的是安裝映像寫下的那份 `rolodex.yml`——因為在有東西能解析網域名稱之前，控制器根本拉不動任何映像。**這裡不寫 `rolodex.yml`，也不重啟該服務**——參見下文「rolodex.yml 只是啟動配置；設定透過 gRPC 編程」一節，以及[凍住的那份配置](#凍住的那份配置)所記錄的、終結了那次寫入的故障。這一階段建立的是 `rolodex.Manager`，內容來自 Town OS 自己擁有的那些設定：`resolution.mode` 來自 `dns_resolution_mode`（儲存值無法解析時回退到預設值，而不是把一個 rolodex 會拒絕的模式編程進去）；轉發器列表來自 `dns_local_forwarders`（開啟時，該列表在每次啟動時從宿主機的解析器中發現，因此換了網路的機器無需操作者做任何事就能用上新的解析器——參見 [Local forwarders](#本地轉發器)）；那份阻擋清單來自它的儲存設定。其中每一項，都會在本次啟動稍後由 reconcile 步驟中的 `ProgramRolodex` 推送進**正在執行**的伺服器裡。rolodex 容器以 `--net host` 執行，並直接把 DNS 繫結到 `127.0.0.2:{port}`。隨後等待 DNS 就緒（TCP 連線輪詢），並配置 systemd-resolved 把該 TLD 路由到 rolodex——**當 `TOWN_OS_DNS_PORT` 已把 rolodex 從 `:53` 遷走時，這一步被跳過**，因為 resolved 的按域名伺服器地址不攜帶埠，那樣會讓該 TLD 下的每一次查詢都被黑洞吞掉。
...
19. **啟動監控系統服務** **(全部非致命)** —— 先拆除上一版設計遺留的 NC/socket 監控單元（它們仍佔用 `-p 9090`/`-p 5308`，會讓新服務不斷崩潰重啟）。Node Exporter、Prometheus 與監控 UI 都以 `--net host` 執行；node-exporter 與 Prometheus 繫結環回地址，只有監控 UI 的 `:5308` 面向區域網。這三個埠都來自 `monitoringPortsFromEnv()`，其零值即為生產預設值（[System-service host ports](#系統服務的宿主機埠)）。隨後安裝每夜執行的 podman prune 定時器 **(非致命)**。每日的更新定時器不在這裡安裝——它隨安裝器一同交付，參見[自動更新](#自動更新)。
20. **確保本地 TLS CA 存在** **(非致命)** —— 在 reconcile 之前執行 `tls.EnsureCA(<btrfsPath>/tls)`，這樣 reconcile 遍歷已安裝包時才能簽發葉子證書。
21. **啟動 ingress 與 pages 服務** **(非致命)** —— `ingressctl.Manager` 安裝並啟動 `town-os-system--ingress`（共享的 `:443` SNI + `:80` Host 路由器），僅當宿主機擁有全域 IPv6 時才啟用雙棧。pages 的 Caddy 服務隨之啟動。當 `INGRESS_IMAGE` 被顯式設為空時（開發模式），兩者都會跳過。
22. **Reconcile 物件儲存** **(非致命)** —— `ReconcileGfeh` 確保每個網路有一個 gfeh 分割槽：`gfeh/<network>` 子卷（chown 給 uid 2000）、渲染出的 `gfehd.yaml`，以及 `town-os-system--gfeh-<network>` 單元，且僅在渲染內容發生變化時才重啟。當 `GFEH_IMAGE` 被顯式置空時整體跳過；當 ingress 被停用時也跳過（四個 HTTP 檢視只能經由它訪問）。分割槽的*名稱*會在稍後非同步釋出——見第 31 步。參見 [Object Storage (gfeh)](#物件儲存gfeh)。
23. **檢測版本變更** —— 將執行中容器的鏡像 SHA（`/proc/1/cgroup` → `podman inspect`）與 `<btrfsPath>/town-os-version` 比較。為 reconcile 設定 `versionChanged`。
24. **Reconcile** —— 遍歷所有已安裝的包並恢復執行時狀態：
...
25. **Reconcile DNS 與網路** —— 撥號 rolodex 的 gRPC socket（最多重試 30 秒）。`RebuildDNS` 清空並從零重建 rolodex，從而丟棄上一次崩潰執行留下的漂移；`RebuildNetworkDNS` 為非預設網路的包重新註冊面向區域網的全域記錄（以及 DANE pin）。隨後 `ReconcileNetworks` 將 home 網路的 TLD 與 `dns_tld` 對齊，並拉起每一個已啟用網路的 WireGuard 介面，同時傳入 rolodex 客戶端，使每個網路的 TLD 作用域都被認領——包括僅 DNS 的 home 作用域。全部非致命。之後物件儲存會被**第二次** reconcile（冪等），這樣本步驟拉起的網路無需等待重啟即可獲得自己的分割槽。
26. **編排 ingress** **(非致命)** —— 等待就緒，撥號其 gRPC socket，`RebuildIngress` 以宣告式方式推送完整路由集（HTTP 包 + pages + 物件儲存的檢視與索引），與 `RebuildDNS` 是同一模型。它還會在同一遍中，從構建這些路由所用的同一站點集合渲染分割槽生成的兩個頁面——它的[分割槽索引頁](#分割槽索引頁)與[已釋出檔案索引頁](#已釋出檔案索引頁)——因為路由不能在它所服務的位元組存在之前就被編排。
27. **啟動 UI 容器** **(非致命)** —— `town-os-system--ui.service`；當 `UI_IMAGE` 被顯式置空時跳過（開發模式，此時由 bun 提供 UI）。
28. **階段 `restart_packages`：新鮮度階段** —— 若上一個程序留下了重新整理標記，則序列重啟每一個已安裝的包單元，併為每個包發出一條進度事件，讓 UI 各渲染一行。崩潰遺留的陳舊標記是無害的。
29. **建立 HTTP 處理器** —— 把所有 manager 接入 `ServerConfig`，啟動後台輪詢器（每小時的外部 IP、DNS 漂移修復、過期 peer 回收），並配置 Echo 路由器的 CORS、失敗即拒的 grant 白名單、鑑權與審計中介軟體。
30. **階段 `ready`：切換根處理器** —— 在已經繫結的監聽套接字上，把啟動樁原子地替換為完整的 Echo 路由器，因此不會出現埠抖動，進行中的 `/boot-status` SSE 訂閱者也能安然跨過這次交接。隨後 `BootStatus.Done()` 關閉該事件流。**系統至此就緒。**
31. **釋出物件儲存的名稱** **(非致命，後台)** —— `publishGfehNames` 等待至少一個分割槽的管理 socket 有應答，然後重新執行 DNS 與 ingress 重建，使每個分割槽 `/v1/names` 的輸出變成 A 記錄、TLSA pin、葉子證書 SAN 與 ingress vhost。它在切換**之後**、且以非同步方式執行，因為 gfehd 在認證之前會輪詢 `/status/ping`——而後者在第 29 步之前一直返回 503——所以在此處同步等待會讓它所等待的這次啟動自我死鎖。若屆時沒有任何分割槽就緒，這些名稱會由下一次 reconcile 釋出。
32. **優雅關閉** —— 收到 SIGINT 時：取消 context，以 30 秒超時關閉 HTTP 伺服器。所有後台 goroutine 通過 context 取消退出。
...
索引頁**只**承載檢視，而它們本來就在 DNS 中。不含主體、授權或配額，也不含已釋出的連結——這是維運者拿來向別人解釋"物件儲存是什麼"的那一頁，而其上的 `/f/<token>` 是一份沒人要求釋出在那裡的持有即用憑據。已釋出的連結有它們自己的頁面，見下文。

### 已釋出檔案索引頁

`http.gfeh.<tld>` 是這台機器上唯一一個存在意義全在於把檔案交給別人的名稱，而它也恰恰是那個返回 404 的名稱：gfehd 只應答 `/f/<token>`，別的一概不應答，包括它自己的根。它的根現在提供一個**已釋出檔案索引頁**——每一個啟用的暴露，帶名稱，並連結到它所服務的那個令牌。

這是機器上唯一一個擁有**兩個後端**的名稱，而 ingress `Route` 上的 `PathBackend` 正是為此而存在（見 [Ingress](#ingress)）：

- **`/` 由 pages 容器處理**，從 `gfeh-index/http.gfeh.<tld>/` 渲染 `gfeh.RenderPublicIndex`，走的是與分割槽索引頁完全相同的目錄檢查、僅在變更時寫入、符號連結與剪枝（`writeGfehIndexFile`）。兩個寫入者就是兩種關於索引住在哪裡、以及剪枝可以刪除哪些名稱的意見。
- **其他每一條路徑仍歸 gfehd。** 匹配器就是根，不多一分：前綴匹配會遮蔽 gfehd 日後可能長出的路徑，而那正是這一頁所修復的失敗的反面。根是 gfehd 透過返回 404 明確說過它不提供服務的那一條路徑。
- **只有位元組確實存在的名稱才會被編排路徑後端。** `reconcileGfehPublicIndexes` 返回它寫下的內容，`gfehPublicIndexBackends` 只為這些名稱佈置路由；把一個沒有 webroot 條目的名稱的 `/` 送往 pages 容器，只是把 gfehd 的 404 換成 Caddy 的 404——同一個失敗，換了個標誌而已。
- **`gfehIndexHostnames` 也覆蓋這個名稱**，否則 pages 的剪枝會在第一遍就刪掉它的 webroot 條目，索引頁將一直 404 到 ingress 下一次重建。
- **聯繫不上的守護程序保留自己的頁面。** `ListExposures` 失敗時，上一次的渲染結果原樣留下，而不是被改寫成"尚未釋出任何內容"：短暫的中斷並不能證明有人撤回了全部檔案，而那樣處理的 reconcile 會在每次重啟時清空索引，再在下一遍裡把它填回去。

**它刻意列舉持有即用的憑據。** 一個 `/f/<token>` URL 就是一份 capability——持有它即是授權的全部——因此釋出一個檔案不再意味著"我發過連結的人可以拿到"，而是意味著"任何能解析這個名稱的人都會看到它被列出"。這就是這一頁所做的交易：一份已釋出檔案的目錄是值得擁有的，而一份省略了連結的目錄只是一串誰都打不開的檔名。由此得出的是，**撤回是讓檔案重新變回私有的唯一手段**；被停用的暴露不貢獻任何一行，因為那一行會去宣傳一個解析不到任何東西的連結。
...
- **Links 標籤頁渲染的是完整 URL，且由服務端組裝。** `GfehExposureView.URL` 由 `gfehPublishedLinkBase` 構建——`https://<http-view-fqdn>/f/`——它來自為 ingress vhost 與葉子證書 SAN 命名的同一個收集器，因此已釋出的連結在構造上就是 ingress 會路由、證書也覆蓋的名稱。它不在瀏覽器裡組裝，是因為 UI 將不得不知道四件伺服器早已掌握的事實：提供服務的名稱是 *http 檢視的*，而不是分割槽的或本機的；它是在該分割槽自己網路的 TLD 之下限定的，而非全域 TLD；路由是 `/f/<token>`；以及上報的埠絕不能出現。當分割槽不提供任何 HTTP 檢視時該欄位為空——這是誠實的答案，因為那時確實沒有任何東西在服務那個令牌——而被停用的暴露渲染為純文本，而不是一個可點選的 404。這個標籤頁裡每一個啟用的連結，也都公開列在 http 檢視的根上（[已釋出檔案索引頁](#已釋出檔案索引頁)），因此讓一個檔案不再被列出的是撤回，而不僅僅是不再分享——而且標籤頁會在表格上方明說這一點（`objects.links_public_notice`），因為這是唯一展示這些連結的介面，也就唯有它能在有人以為連結只屬於收件者、從而又釋出一個之前把話說清楚。該提示僅在連結**既啟用又確實在被提供**時出現，判據與那一欄相同，都是 `URL` 欄位：沒有 HTTP 檢視的分割槽沒有可供索引頁落腳的根，此時的警告將描述一件並未發生的暴露。
...
### 一個 vhost，兩個後端：`PathBackend`

一條路由通常只指名一個服務，機器上幾乎每條路由都是如此。`Route.path_backends` 是那個例外：一組 `(路徑, 後端)` 對，渲染成若干 Caddy `handle` 區塊，排在承載路由自身後端的那個裸 `handle` 之前。它之所以存在，是因為一個後端可以擁有一個名稱，卻並不擁有該名稱之下的每一條路徑——它唯一的呼叫者是[已釋出檔案索引頁](#已釋出檔案索引頁)，它從 pages 容器提供 `/`，而 `http.gfeh.<tld>` 上其他每一條路徑都抵達 gfehd。

- **裸 `handle` 永遠在最後。** `handle` 區塊互斥，並按書寫順序求值，而裸的那個匹配一切，因此排在它之後的任何東西都是死配置。
- **路徑後端按給定順序渲染**，不排序。排序對 Town OS 實際編排的那些匹配器是安全的（它們並不重疊），但對確實重疊的一對，它會悄悄改變誰勝出。
- **路徑像主機名和後端一樣被校驗**（`validPathMatcher`：絕對路徑、URL 非保留字元外加 `*`、長度有上限）。壞的那個被**單獨**丟棄，而不是連同它的路由一起：路由自身的後端是兜底，因此丟掉一個路徑後端只損失它所主張的那一條路徑，而這個名稱背後的服務照常應答。重複的路徑同樣被丟棄——Caddy 接受兩個相同的匹配器，只是永遠到不了第二個。
- **路徑後端沒有自己的 `backend_tls`**，一律以明文 HTTP 代理。那個標誌描述的是一條路由*為哪個*服務而設，而這個欄位唯一要抵達的東西，和其他每一個 page 一樣在 `:80` 上講 HTTP。

### 渲染

輸出**按主機名排序**，因此跨多次 reconcile 渲染出的位元組是確定性的——這正是讓監管程序對內容未變的過載做空操作的前提。全域配置為 `auto_https off`（憑證由 Town OS 管理）、`protocols h1 h2`（ingress 只發布 TCP，因此基於 UDP 的 H3/QUIC 不可達），以及管理端點。Caddy 的管理 API 被刻意**保持啟用**：監管程序用 `caddy reload` 編排新路由，而該命令正是與那個端點通訊，因此 `admin off` 會讓首次啟動之後的每一次路由更新都失效。

它的位址是**寫進設定裡的** `admin 127.0.0.1:<連接埠>`，而不是留給 Caddy 的預設值，預設取 Caddy 自己的 `2019`（`ingress.DefaultAdminPort`，可用 `--caddy-admin-port` / `ingress.WithCaddyAdminPort` 覆寫）。正式環境保持 2019——ingress 容器有自己的網路命名空間，除 :443、:80 和回環上的指標連接埠外什麼都不發布，因此管理 API 只在該容器內部可達，別處都不行。**任何在主機命名空間裡執行 Caddy 的情境都必須把它挪開**，而每個測試都屬於這種情境：`go test ./src/...` 直接跑在主機上，整合容器又是 `--net host`，所以兩次並行的 `make test-full` 會把兩個 Caddy 子行程放到同一個 :2019 上——第二個繫結失敗並結束，而在此之前，任何一次執行的 `caddy reload` 都可能落到另一次執行的管理 API 上，把錯誤的 Caddyfile 編排進去（鐵律）。把位址渲染出來同時也是*讓* `caddy reload` 指向它的方式：該命令會從它所轉接的那份設定裡讀出管理位址。`127.0.0.1` 這個寫法是刻意的——在雙堆疊機器上 `localhost` 會先解析到 `::1`，而 Caddy 只繫結其中一個。
...
位於某條已編排路由背後的後端有兩種壞法，而 ingress 用同一個頁面回應兩者：一個**說明服務不可用、並每五秒自行重新載入的 503**（`unavailableRefreshSeconds`，同時也是 `Retry-After` 的取值），直到服務重新應答為止。期間不重新編排任何東西，也不撤下任何路由——位址自始至終保持可路由，因此後端一旦重新有了監聽器，下一次重新載入就會把它端上來。

對一臺家用機器而言這是常態，不是例外：包在升級時重啟，一次設定變更把服務彈一下，一次重開機讓二十個容器按某種順序回來。Caddy 在那幾秒裡說的是 `502 Bad Gateway`，讀起來像*壞了*而不是*正在回來*，除了手動重試之外沒給讀者留下任何可做的事。

這兩種失敗形態在 Caddy 裡是兩套彼此獨立的機制，只涵蓋其中一種的實作會漏掉常見的那種：

- **後端應答了，回傳 5xx。** 那是一次上游*回應*，因此用回應匹配器加 `handle_response` 在 `reverse_proxy` 區塊**內部**攔截——這也是渲染器如今發出的每一個代理都要開一個區塊的原因，純 HTTP 的那些也不例外。
- **後端根本沒有應答**——容器停了、名字解析不了、連線被拒。`reverse_proxy` 失敗，Caddy 拋出它自己的錯誤，而 `handle_response` 永遠看不到它；那屬於**站點區塊**層級的 `handle_errors`。在家用機器上這才是常見的那一種，也正是只處理 5xx 的實作會靜悄悄漏掉的那一種，因為它守候的那個 5xx 從來不會到來。

兩條路徑都渲染為 Caddyfile **片段（snippet）**（`render_unavailable.go`），每份檔案定義一次，由每個站點區塊匯入——因此頁面不會在各個 vhost 之間漂移，而無論這臺機器服務多少條路由，檔案都保持在可讀的體量。即使路由集為空也照樣發出這些片段：沒人匯入的片段永遠不會被解析成指令，而一段忽有忽無的前言會在第一條路由到來時白白彈一次 Caddy。

- **只有瀏覽器才會拿到這個頁面。** 兩條路徑都以 `method GET HEAD` 加 `Accept: *text/html*` 為閘門。API 客戶端——XHR、webhook、`curl`、路徑後端背後的 DoH 解析器——在回應路徑上拿到上游自己的狀態碼、內文與回應標頭，在錯誤路徑上拿到一個帶 `Retry-After` 的樸素 503。把 HTML 遞給一個正在解析 JSON 的東西，只會把一個壞掉的後端變成它呼叫方裡第二個、更古怪的故障。

  這條穿透只用 `copy_response`，**不**在旁邊寫 `copy_response_headers`，而這個「不寫」是有承重作用的：等到回應處理器執行時，`reverse_proxy` 早已把上游的回應標頭放到了 response writer 上，因此再複製一遍會讓每一個回應標頭都出現兩次——其中包括 `Content-Length`，那已經不是不整潔，而是一則格式非法的訊息。它看上去正像是 `copy_response` 理所當然的搭檔，所以渲染測試和整合斷言都各釘了一道。
- **頁面上的服務名來自渲染出的設定，絕不來自請求。** 這個標籤是匯入時代入的片段引數——路由的主機名，且已經過 `validSiteHost` 校驗。`:80` 兜底區塊改用常數 `Town OS` 標註，因為它應答*每一個*未匹配的主機：那裡的 `Host` 標頭是客戶端隨手寫的任何東西，把它反射進 HTML，就是在唯一一個接受任意名字的 vhost 上開了個注入口。`safeUnavailableLabel` 是讓這句話對將來的呼叫方依然成立的那道守衛。
- **包的 `:80` 轉址區塊兩者都不加**，因為它不代理任何東西——那裡沒有會不可用的後端。

#### 這個頁面是被翻譯過的

頁面上的四句話是詞條（`src/i18n` 中的 `ingress.unavailable.*`），而不是渲染器裡的字面值——Town OS 提供的每一種語言都會得到這個頁面的一份渲染，Caddy 則依每次請求的 `Accept-Language` 挑一份出來。

這與 UI 的做法一致，理由也相同：**讀者的語言是瀏覽器的語言，不是這臺機器的語言。** `I18nContext` 先取已儲存的選擇，再取 `navigator.languages`，只有當瀏覽器要的語言沒有任何詞表涵蓋時，才退回伺服器端的全域 `locale` 設定。重試頁面在 Caddy 裡解析同樣的順序——一個針對 `Accept-Language` 的 `map`，每種語言詞表一列，外加一列帶著這臺機器 locale 的 `default`。手機是葡萄牙語的客人，在一臺設定為德語的機器上讀到的是葡萄牙語。

- **用 `map`，頁面只寫一份——而不是每種語言一個 `handle` 區塊。** 區塊的寫法能用，而且實測過：因為 `import` 是文字展開，每一次展開都會把整個選擇器內嵌進去，於是三十個分支就代表 Caddy 每次重新載入所收到的 JSON 裡有三十份頁面副本——**單一路由就是 115 KB**，一臺有二十條路由的機器，在每次裝套件、改頁面和每小時的 reconcile 時都要推送約 2.3 MB。改用 map 之後，翻譯好的句子放在一張緊湊的表裡，頁面只寫一次，從表裡讀出 `{townos_title}` 之類的值。
- **5xx 那條路徑什麼都不渲染；它拋出 `error 503`**，交給本站點自己的 `handle_errors` —— 這正是讓頁面按站點區塊展開一次、而不是按每個 proxy 展開一次的原因。已驗證：在 `handle_response` 內部拋出的 `error` 確實會到達外層的錯誤處理器，且原請求保持不變，因此 `Accept` 那道閘門依然生效。
- **選擇器是按語言一列，而不是按 locale。** 國家變體（`de-AT`、`en-GB`、`es-MX`……）衍生自它們的基礎語言，只覆寫那個國家確實說得不一樣的字串——這四句都不在其中——所以一個變體一列，只會多出三十條渲染同一頁面的正規表示式。`TestUnavailableLocalesCoverEveryCatalog` 從 `i18n.PopulatedLocales()` 推導出應有的成員集合，並在兩個方向上失敗，因此新加入詞表的語言不會在這裡悄悄停留在英文。
- **繁體中文排在通用 `zh` 之前。** Caddy 依序測試 map 的每一列並取第一個命中，而 `^zh` 會把 `zh-TW` 一併吃掉，所以順序寫反就會把簡體送給臺灣、香港和澳門——對讀不懂的人來說，那看上去正是一份能用的翻譯。每條模式都做了錨定、忽略大小寫，並且在分隔符或整串結束處收尾，因此 `^ja` 不會用日語去回答爪哇語。
- **服務名是格式動詞，不是字串串接。** `"%s is unavailable"` 在日語裡把名字放在與英語不同的位置，所以詞序歸詞條所有；渲染器把 `{args[0]}` 代入翻譯後的句子，而站點區塊匯入該片段時，Caddy 再把主機名代入其中。
- **這臺機器的 locale 透過單元的命令列到達 ingress**（`--locale`、`ingressctl.Config.Locale`，在產生單元時讀取 `locale` 設定）。它是一項全機器的設定而非路由狀態，所以它隨單元走而不是隨 gRPC 推送走；語言一改，它就以其他任何單元變更相同的方式到達 ingress——下一次 reconcile 渲染出不同的單元。沒有詞表的代碼會被忽略而不是被拒絕：這個設定是一行自由文字，寫錯的代價應當是一個英文頁面，而不是一個起不來的 ingress。
- **每一條翻譯字串都為它跨越的兩個邊界做逸出**（`caddyHTMLText`）：它先是一個 Caddyfile 記號，然後是一個 HTML 元素的內容。HTML 逸出順帶去掉了會提前結束記號的引號；反斜線單獨逸出，因為 Caddy 把它讀作逸出字元，而 Go 的 `html` 套件沒有理由去動它。
...
此外還有 **caddy 自己的指標族**：從子行程容器本地的 admin API（`127.0.0.1:<管理連接埠>/metrics`，由 Caddyfile 裡那同一個位址建出，因此挪走的子行程不會把抓取留在 2019 上）取回，原樣附加在回應之後。它們的名稱空間是 `caddy_*`/`go_*`/`process_*`，不可能與 `townos_ingress_*` 衝突。透傳是它們能到達 Prometheus 的唯一途徑——admin API 是刻意不對外釋出的——而它們描述的正是真正服務這臺機器每一個請求的那個行程。回應體不會被重新渲染：把它解析回指標族只會丟失資訊（caddy 匯出直方圖，而 `src/metrics` 刻意不建模直方圖）。
...
- **`Forwarders` / `ResolutionMode` / `Blocklists`** —— Town OS 擁有的那些設定，由儲存的組態與主機探索解析得出。管理器不寫任何檔案：這些正是 `ProgramRolodex` 推進正在執行的服務裡的東西。參見下文「rolodex.yml 只是啟動配置」一節。
- **`Generation`** —— rolodex 在啟動時綁定的 gRPC socket 的身分（裝置號、inode、mtime），它恰好在 rolodex 重啟時改變，因此也就是「一次重啟丟掉了每一項已編程的設定」被察覺的方式。socket 不存在時為空，意味著 rolodex 沒在執行。
...
該模式可由運維在執行時通過 `dns_resolution_mode` 設定配置（`auto` | `recursive` | `forward`；由 `ValidateDNSResolutionMode` 校驗，因此無法解析的值絕不可能到達 `rolodex.yml` 並把 DNS 弄砸）。`main.go` 在啟動時把它讀入 `rolodex.Config`；通過 `POST /settings/set` 的更改會執行 `Controller.RefreshDNSResolutionMode`，後者用 **`SetResolutionMode`** 把新模式編程進正在執行的伺服器——不寫檔案，不重啟單元，DNS 也不中斷。rolodex 遇到不認得的模式會直接拒絕而不是退回預設值，這正是這個值在送出之前先被校驗的原因。
...
**本節是歷史。** `WriteConfig` 與 `RewriteConfig` 都已刪除，Town OS 完全不再寫 rolodex 的設定檔：它們過去算繪的那些設定，現在是編程進正在執行的服務裡的，而留下的那個檔案屬於安裝映像。參見下文「rolodex.yml 只是啟動配置」一節。之所以保留這一節，是因為它描述的那種失敗——一次悄無聲息什麼都沒做的設定寫入，只在「某個功能從來沒生效過」的時候才浮出水面——正是這個子系統反覆製造的那種 bug。

那些回歸測試已隨它們守護的程式碼一起刪除；接替它們的東西列在下一節。

### rolodex.yml 只是啟動配置；設定透過 gRPC 編程

`rolodex.yml` 由安裝映像寫出（`../install` 裡的 `scripts/rolodex-config.sh`，作為 rolodex 的 `ExecStartPre`），除此之外沒有別的寫入者。它只承載兩樣無法在執行中的 rolodex 上設定的東西：DNS 的 **bind 清單**（UDP 與 TCP 上的 `127.0.0.2:53` —— systemd-resolved 被指向的位址，而且監聽器必須在任何 API 呼叫之前就存在）以及 **metrics 監聽器**，rolodex 只在啟動時依該段是否存在把它打開一次。它還寫下 `resolution.mode: auto` 與公共轉發器作為**啟動預設值**，與這裡的 `DefaultForwarders` 一致，因此第一次推送之前的視窗是一個確定的狀態，而不是 rolodex 內建選擇碰巧是什麼就是什麼。其中每一個值都是常數，所以這個檔案在每次啟動時都逐位元組相同。Town OS 完全不寫設定檔。

其餘由 Town OS 擁有的一切 —— 轉發器清單、`resolution.mode`、以及那份阻擋名單 —— 都由 `ProgramRolodex` 透過管理 API 編程進**正在執行的**服務裡。這正是設定變更變得沒有代價的原因：過去套用一次變更意味著重寫檔案並重啟單元，而重啟這台機器唯一的解析伺服器，就是讓機器上的一切都斷一次 DNS。`SetForwarders` 是無條件推送的（rolodex 那一側只是一次普通儲存，而且沒有可用於比對的讀取介面）；`SetResolutionMode` 會先比對，因為切換進 `auto` 會重啟分層探測，定期重複宣告會不斷丟掉一個已經穩定下來的層級；阻擋名單走 `ReconcileBlocklists`，它本來就是讀回再比對（只有一份，不是兩份 —— RBL 那一半已經退役）。

**每次 rolodex 重啟之後都必須重新跑一遍。** rolodex 在啟動時從設定檔播種這一切，並且不會保存任何透過 gRPC 設定的內容，所以一次 Town OS 沒有下令的重啟 —— `Restart=always` 下的崩潰、DHCP 租約變化透過 `town-os-rolodex-config.path` 彈動單元、維運手動重啟 —— 會悄悄把這台機器退回 rolodex 的預設值：以與維運者所選不同的方式解析，並且什麼都不過濾。沒有任何東西會暴露這一點，這些設定不會被任何地方讀回。

重啟訊號是 `rolodex.Manager.Generation()`：rolodex 在啟動時綁定的 gRPC socket 的身分（裝置號、inode、mtime）。rolodex 每次啟動都會 unlink 並重新綁定它，所以新的一次執行總是有新的 inode，而察覺這件事只需要一次 `stat` —— 不需要 D-Bus 往返，不需要這套程式碼尚未暴露的單元屬性，也不需要專門的 RPC。`reconcileRolodexProgramming` 跑在它自己的 **15 秒** 心跳上，只要 generation 沒變就什麼都不做；之所以是秒級而不是分鐘級，是因為這段空隙裡機器的解析與過濾方式和設定不一致。只有在一整趟都成功之後才記錄 generation，因此部分失敗會重試，而不會被當成已完成。

**上游已修正。** rolodex 從前只有在**設定檔裡**啟用了阻擋名單時才會啟動它的「:53 是否可達」探測，因此當名單不再寫進那個檔案之後，探測就再也不會啟動，而一份已設定的阻擋名單恰恰會在那個探測賴以存在的網路上劣化。現在它是 `DnsblChecker::resolver_availability_loop`，無條件派生，並且把關的是檢查器*執行期*的啟用旗標——正是自動解析恢復迴圈早就有的那種形狀。一份透過 gRPC 打開的名單，幾秒之內就能拿到探測。

回歸測試是 `TestProgramRolodex*` 與 `TestReconcileProgramming*`（單元測試），以及 `TestProgrammedSettingsSurviveARolodexRestart` 與 `TestGenerationChangesOnlyWhenRolodexRestarts`（整合測試）—— 後者會對一個真實的 rolodex 編程、重啟它、確認服務自己已經忘記，然後重新編程。

**映像與檔案是配套的一對，兩者之間沒有任何協商。** `rolodex.yml` 由 `../install` 寫出，由這臺機器碰巧在跑的那個 rolodex 映像解析，兩個方向都沒有版本握手——serde 對未知或缺失的欄位一律直接拒絕。某個欄位在映像那一版是必填而檔案裡沒有，或者檔案裡有而映像不認識，啟動時就是一次硬性的 `failed to parse config file`，而在 `Restart=always` 之下那就是一場崩潰迴圈，機器上的一切都沒有了 DNS。這已經在測試臺上發生過一次：發布的映像早於 `rbl` → `dnsbl` 的改名，於是每一個啟動真實 rolodex 的測試都死在 `missing field \`rbl\`` 上，而每一個走 mock 的測試都通過。由此得出的規矩是：安裝倉庫的 `rolodex-config.sh` 與發布的 rolodex 映像要**一起**動；而在這裡升一次 `rolodex-dns/go` 只是半次升級——參見「本機所服務的加密傳輸」一節裡那個兩邊都需要的 SVCB 版本釘。

**只有一道護欄，而且只管一個方向。** `TestRolodexDohBackendMatchesTheInstallScript` 從 `../install/scripts/rolodex-config.sh` 裡解析出 `doh.bind`，與 `RolodexDohBackend` 比對，因此**這一側**任何偏離安裝指令碼的改動都會在這裡失敗。反方向——改的是安裝指令碼——只有當有人隨後跑了這個倉庫的測試才會被抓到，而且在安裝倉庫沒有簽出的地方這個測試會**跳過**，測試容器正是這樣的地方。安裝那一側沒有任何檢查，兩個倉庫之間也沒有生成式的共享常數。請把這道護欄當作只覆蓋它所覆蓋的那個方向。
...
### 這臺機器**對外提供**的加密傳輸：DoH 走 ingress，DoT 與 DoQ 直連

上面講的都是作為*客戶端*的 rolodex——DoH 與 DoT 出現在 `auto` 的分層清單裡，是這臺機器抵達網際網路的方式。本節講的是另一個方向：**對區域網路上的客戶端提供**的加密 DNS，那是另一組監聽器，歸屬也不同。

三者都由安裝映像的 `rolodex.yml`（`../install` 裡的 `scripts/rolodex-config.sh`）打開，這裡什麼也不打開。rolodex 只在啟動時依據各自配置小節是否存在把它們各打開一次，和 metrics 監聽器完全一樣，所以它們都無法透過 gRPC 開啟——參見上文「rolodex.yml 只是啟動配置；設定透過 gRPC 編程」一節。

**DoH 由 ingress 擋在前面。** rolodex 把 DoH 繫結在回送位址 `127.0.0.2:4443`（`RolodexDohBackend`），ingress 則像發布任何一個普通 vhost 那樣把它發布在 `dns.<tld>`（`DohHostLabel`），並在這一跳內部代理時跳過憑證驗證（`dohIngressRoute`）。因此客戶端驗證的是 ingress 的葉子憑證，由本機 CA 在它自己的葉子倉庫（`doh`/`resolver`/`current`）下簽發，所以重簽它絕不會驚動 pages 或 gfeh 的葉子。這正是整套安排的意義所在：rolodex 用自簽憑證終結它自己的 TLS，沒有任何一個會做驗證的 DoH 客戶端會接受它，而也永遠不需要有誰接受——它終結的是一跳從不離開這臺機器的連線。客戶端使用 `https://dns.<tld>/dns-query`。被代理的是整個 vhost 而不是那一個路徑，因為 rolodex 的 DoH 路由只服務這一個路徑、其餘一律 404——在這裡再加一個路徑比對，只會多出一處可以把路徑寫錯的地方。

用 `4443` 而不是 `443` 是有分量的。ingress 發布在 `0.0.0.0:443` 上，而 rolodex 以 `--net host` 執行，因此同一個命名空間裡的萬用 `:443` 與具體的 `127.0.0.2:443` 會讓後繫結的那個 EADDRINUSE——按啟動順序不同，倒下的要麼是 DNS，要麼是 ingress。這個數字還被寫在兩個彼此讀不到對方的倉庫裡：這裡，以及 `../install` 的 `doh.bind`。`TestRolodexDohBackendIsTheLoopbackPortInstallWrites` 把這一側釘在字面量上；`TestRolodexDohBackendMatchesTheInstallScript` 則在兩個倉庫都檢出的地方直接去讀那份安裝指令碼，而在測試容器裡（`../install` 並不存在）跳過。

**vhost 需要一個名字，而只有 DNS 的兩個 reconciler 能給它。** `dns.<tld>` 背後沒有套件、沒有頁面、也沒有物件儲存分割區，所以別的東西永遠不會提到它：`RebuildDNS` 顯式發布它的 A 記錄（在雙棧機器上還有 AAAA，因為那張葉子憑證把主機的兩個位址都寫進了 SAN），而 `ReconcileDNS` 把它帶進期望集合裡，這樣每小時那趟清理孤兒記錄的掃描，才不會在啟動一小時之後刪掉啟動時發布的東西。沒有這條記錄，失敗發生在 TLS 之前、在搆到 ingress 之前——一個乾脆的 NXDOMAIN，讀起來就跟這個功能壓根沒做過一模一樣。回歸測試：`TestRebuildDNSPublishesTheDoHEndpointsName` 與 `TestReconcileDNSKeepsTheDoHEndpointsName`（單元），`TestDohResolvesThroughTheIngress` 與 `TestDohEndpointResolvesOnRealDNS`（整合）。

**DoT 與 DoQ 完全繞開 ingress**，這是一個已知的缺口，而不是設計。ingress 只發布 TCP，它的 Caddy 又被釘在 `protocols h1 h2` 上（`renderCaddyfile`），所以對非 HTTP 的協定，穿過它的四層通路根本不存在。安裝映像把兩者都繫結在區域網路側的 `0.0.0.0:853`——這是標準的一對，DoT 用 TCP，DoQ 用 UDP/QUIC（rolodex 自己給 DoQ 的預設埠是 `8853`，沒有客戶端會去找它）。位址用 `0.0.0.0` 而不是某個具體位址，是因為這些小節各自只接受**一個** bind 字串：釘死第一個全域位址，會讓第一次 DHCP 租約之前什麼都不提供，並且在第二個位址上永遠不提供。`dns.bind` 做不了同樣的事，因為 `:53` 與 podman 自己的 DNS 監聽器衝突；`:853` 沒有這個衝突，代價是繫上了每一個介面，包括 podman 網橋與 overlay。IPv6 沒有被覆蓋——一個 bind 字串沒法同時點名兩個位址族。

**兩者提供的都是由這台機器自己的 CA 簽發的葉憑證。** 安裝映像把 `dot.tls`／`doq.tls` 指向 `/data/tls/dot/{cert,key}.pem`，而 `issueRolodexTransportLeaf` 在每一次 `RebuildDNS` 時恰好寫下那一對，帶的 SAN 與 DoH vhost 的葉憑證相同：`dns.<tld>` 加上這台機器的各個位址。因此一個做校驗的 DoT 或 DoQ 用戶端，是拿家裡本來就信任的那個 CA 去驗證，而不是被塞過來一份自簽憑證要它去釘住。

這件事之所以是一個檔案而不是一次 RPC，原因在於次序。rolodex 是在 systemcontroller *之前*啟動的——在有東西能解析名稱之前，控制器一個映像也拉不下來——所以在首次啟動時，rolodex 讀它的設定時 CA 還不存在。rolodex 把「被指名但尚不存在的憑證」當作「先提供一份產生的，並盯著真正的那一份」，因此簽發這份葉憑證**就是**那次交接：它會在一次 30 秒的輪詢之內被接過去，不需要重啟，也沒有什麼要協調。這同時也讓 rolodex 的憑證熱重載第一次在這台機器上真正活起來；產生的材料是被刻意不輪詢的，所以在有一個監聽器指向檔案之前，那整套機制都是搆不到的程式碼。

路徑落在 `/data` 之下，是因為那是 rolodex 容器唯一能看見的東西——它的單元只掛載 `/town-os/rolodex:/data`，別的都沒有，所以這份葉憑證放在資料目錄裡，而不是與其他所有葉憑證一起放在 btrfs 的 TLS 子卷下。這裡的 `RolodexTLSSubdir` 與 `../install` 的 `scripts/rolodex-config.sh` 裡的 `cert_path`，是同一個掛載寫在兩個倉庫裡的兩半；只改其中一邊，會讓 rolodex 盯著一個沒人去寫的檔案。測試：`TestRolodexTransportLeafLandsInTheMountedDataDir`、`…NamesTheHostAndItsAddresses`、`…IsSignedByTheBoxCA`、`…FollowsAnAddressChange`。

**即使用戶端並不預先信任本機的 CA，它依然能驗證這張憑證**，因為葉憑證被釘在了 rolodex 自身具備權威的那個區裡。`collectRolodexTransportTLSA` 會在每一次 `RebuildDNS` 時，於 `_853._tcp.dns.<tld>` 與 `_853._udp.dns.<tld>` 發佈 DANE-EE 記錄——`3 1 1`，即葉憑證 SPKI 的雜湊——與軟體套件、page 和 gfeh 的釘扎並列。這才是簽發這張葉憑證對一台從未加入過本戶的裝置的意義所在：它能夠抵達這個解析器，因而能取到釘扎，因而無需先安裝 CA 就能檢驗對方遞過來的東西。

一張憑證對應兩筆記錄，因為 TLSA 記錄的歸屬是一個服務**端點**而非一張憑證，而 DoT 與 DoQ 共用連接埠與葉憑證，只在傳輸層上不同。只發佈其中一筆比兩筆都不發佈更糟：一個支援 DANE 的用戶端若為自己選定的傳輸找不到記錄，就會失效阻斷，於是缺失的那一半讀起來是「這台機器的 DoQ 壞了」，而不是「這台機器沒有 DANE」。要能表達這一點，`rolodex.TLSAEntry` 必須帶上 `Proto`——它此前把 `_tcp` 寫死，DoQ 的釘扎根本無從存在。留空仍然表示 tcp，也就是所有提供 HTTP 的軟體套件。
...
**指定記錄需要一個認得 SVCB 的 rolodex**，也就是 `v0.6.0` 或更新——這個記錄型別是在那裡加進去的，go.mod 裡 `rolodex-dns/go` 的 pin 也隨之移動。更舊的伺服端會拒絕這條記錄，`publishDDRDesignation` 以 debug 等級記下並繼續，於是這臺機器提供著沒有任何東西能發現的加密 DNS：能用，但看不見。這正是那次往返要對著**真實的** rolodex 斷言（`TestDDRDesignationRoundTripsThroughRealRolodex`）而不是只對著 mock 的原因——mock 遞給它什麼記錄型別它都會收下。

**還有一次，是以人能讀的形式。** `GET /dns/status` 帶上一個 `encrypted` 物件——DoH 的 URL、DoT／DoQ 的名稱與埠，以及那個探索用的名稱——DNS 頁面把它渲染成唯讀的一塊。這些端點是從 TLD 與入口推導出來的，因此那裡沒有什麼可設定的；缺的一直是任何一種*看見*它們的辦法——在一台已經提供 DoH、DoT 與 DoQ 有一段時間、卻沒有任何東西告訴任何人它們在哪裡的機器上。發布出去的 DoH 埠是入口的 `443`，絕不是 rolodex 內部的 `4443`：一個被指向 `4443` 的用戶端，會撞上一個只在回環上的監聽器，拿著一份它會拒絕的憑證。

**DoT 有一個測試，DoQ 一個也沒有。** `TestDotServesDNSOverTLS` 啟動一個帶 `dot:` 小節的真實 rolodex，並對它完成一次 RFC 7858 交握——用來確認安裝映像寫下的那個小節確實產出了一個會說這套協定的監聽器，而不是一個只會卡住的 socket。它繫結的是回送位址上的臨時埠，而不是生產用的 `0.0.0.0:853`：在 `--net host` 之下，這裡的知名埠就是*宿主機的*，兩次 `make test-full` 會在它上面撞車。DoQ 仍然沒有測試——QUIC 客戶端是本倉庫沒有的依賴，而同一條埠的規則也適用於將來測試它的任何東西。

### 改 TLD 必須把加密 DNS 一起帶走

加密 DNS 同時以四個地方的名字掛在 TLD 上，而每一個都是獨立的發布者：

1. DoH 端點的位址記錄（`publishDohRecord`）；
2. 對外宣告它的 DDR 指定記錄（`publishDDRDesignation`）；
3. DoT 與 DoQ 所提供憑證上的 SAN（`issueRolodexTransportLeaf`）；
4. 那張憑證的 DANE 釘扎（`collectRolodexTransportTLSA`）。

`rolodex.ChangeTLD` 搬的是區域與套件記錄，這四樣一個也不搬。於是 `POST /dns/tld` 把一臺機器從 `home` 改名為 `lan` 之後，它仍然在宣告 `dns.home`，對 `dns.lan` 遞出一張寫著 `dns.home` 的憑證，而在客戶端此刻真正會撥的那個名字上沒有任何釘扎。

**最後這一條是壞掉，而不是降級。** 一個會檢查 DANE 的客戶端，抵達某個端點卻找不到對應的 TLSA 記錄時，會拒絕這次連線——DANE 本來就是幹這個的。所以故障不是「加密 DNS 未經驗證」，而是「加密 DNS 不工作了」，並且一直不工作，直到下一次啟動時 `RebuildDNS` 把這些重新發布。對一臺一個月才重啟一次的機器來說，那就是一個月。

這四樣如今是一次呼叫 `publishEncryptedDNS`，兩個呼叫方都用它：`RebuildDNS` 與改 TLD 的路徑。它們之所以被併成一個函式，正是因為在呼叫點上看不出它們是綁在一起的——每一個都被寫成了自己的發布者、只有一個呼叫方，而第二個呼叫方把四個全忘了。`retireEncryptedDNS` 是它在釘扎這一側的逆操作：舊 TLD 的記錄會隨 `ChangeTLD` 中的區域一起倒掉，但一次不拆區域的改名，會留下一張已經沒人提供的憑證的釘扎，那是同一種「失敗即拒絕」，只是來自另一個方向。

`POST /dns/tld` 現在也會呼叫 `reprogramIngress`，理由相同：vhost 的名字裡嵌著 TLD，DoH 端點的那個也在其中，否則 ingress 會一直用舊名字服務，直到重啟。

回歸測試：`TestTLDChangeRepublishesTheEncryptedDNSEndpoints` 與 `TestTLDChangeLeavesNoPinAtTheOldName`（單元），`TestTLDChangeMovesTheEncryptedEndpointsOnRealDNS`（整合，針對真實 rolodex 與它的即時解析器）。
...
- `POST /dns/tld`（需要管理員）—— 設定 TLD。更改現有 TLD，重新註冊所有已安裝的套件，在新名字下重新發布加密 DNS 的各個端點，並重新編程 ingress 使其 vhost 跟上。參見「改 TLD 必須把加密 DNS 一起帶走」一節。
- `POST /dns/setup`（需要管理員）—— 初始化 DNS 並註冊所有已安裝的包。
- `GET /dns/dnsbl`（需要鑑權）—— 獲取 DNSBL（域名黑名單，正向名稱）配置：全域啟用標誌、各提供方區域及其**已解析為實際生效值**的拒絕碼、列表級的 `refusal_cooldown_secs`，以及 `rotated_out`（當前因拒絕查詢而被輪換出去的提供方，附帶拒絕碼與剩餘秒數）。參見 [Refusal codes](#拒絕碼提供方說別再問了不等於說這個被列入了)。
- `POST /dns/dnsbl`（需要管理員）—— 替換 DNSBL 配置。接受一個啟用標誌、一個列表級的 `refusal_cooldown_secs`，以及一組 `{zone, enabled, refusal_codes, refusal_cooldown_secs}` 提供方。區域會被校驗為完全限定主機名，並轉小寫、去空白、去重；拒絕碼由 `ValidateRefusalCodes` 校驗（IPv4 地址或 `address/prefix`，按字首掩碼，`"none"` 只能單獨出現，不允許重複）。
- `GET /dns/rbl/local`（需要鑑權）—— 列出本地阻擋名單條目（`{name, reason}`）。該路徑保留了它歷史上的 `rbl` 寫法：它是與 UI 之間已發布的契約，而且條目本身沒有變——消失的只是那些曾經共用這個字首的 RBL *提供方*查詢。
- `POST /dns/rbl/local/add`（需要管理員）—— 新增本地阻擋名單條目。接受一個名稱（域名或 IP）與可選原因。名稱會被校驗（域名或 IP）、轉小寫並去空白。
- `POST /dns/rbl/local/remove`（需要管理員）—— 按名稱移除本地阻擋名單條目。
...
DNS 的只讀端點（`/dns/status`、`/dns/records`、`/dns/rbl/local`、`/dns/dnsbl/allowlist`、`/dns/services`、`GET /dns/tld`、`GET /dns/dnsbl`）被排除在審計日誌之外。白名單的*寫*操作會被審計（把一個名稱從所有黑名單中豁免是一項需要問責的變更）；與它們所對應的黑名單寫操作一樣，它們在 `account.RouteActions` 中沒有具名動作——由路徑本身標識它們。

### 阻擋名單

Rolodex（0.2.4+）提供兩種互補的垃圾/惡意/廣告攔截機制，外加（0.4.3+）一種撤銷機制與一種"不相信拒絕了查詢的提供方"的機制，全部通過 DNS API 與 `rolodex.Client` 封裝暴露（`SetDnsblConfig`/`GetDnsblConfig`、`AddLocalBlocklistEntry`/`RemoveLocalBlocklistEntry`/`ListLocalBlocklistEntries`、`AddDnsblAllowlistEntry`/`RemoveDnsblAllowlistEntry`/`ListDnsblAllowlistEntries`）。全部由 **rolodex 按需查詢**——Town OS 從不下載、解析或預快取黑名單訂閱源。

**RBL 那一半已經退役。** 反向 IP 的提供方查詢、`SetRblConfig`/`GetRblConfig` 以及 `GET`/`POST /dns/rbl` 端點全都沒有了；rolodex 把 `src/rbl.rs` 改名為 `src/dnsbl.rs`，只留下了基於名稱的那次檢查。在這次改名中留存下來的是*本地*阻擋名單——它的 HTTP 路徑仍然寫作 `/dns/rbl/local*`，因為那是已發布的契約——以及 `dns_rbl_config` 這個設定鍵，如今沒有任何東西讀它，`controller_dns_blocklists.go` 裡點名它只是為了不讓這個鍵被別的東西複用。

注意該封裝的 `SetDnsblConfig` 把列表級的拒絕冷卻時間作為末位引數（`SetDnsblConfig(ctx, enabled, providers, refusalCooldownSecs)`）；它對映到上游的 `SetDnsblConfigWithRefusalCooldown`，因為上游那種保持引數個數不變的寫法是為了外部 API 相容性而存在的，而內部封裝並不需要這一點。

- **DNSBL**（域名黑名單）—— 域名黑名單區域，按需通過把被查詢的域名前置到該區域來發起查詢（例如 `googleadservices.com` + `dbl.spamhaus.org`）。DNSBL 的命中優先於轉發/迭代得到的答案。通過 `/dns/dnsbl` 配置為一組 `{zone, enabled, refusal_codes, refusal_cooldown_secs}` 提供方，外加一個全域啟用標誌與一個列表級的 `refusal_cooldown_secs`。
- **本地阻擋名單條目** —— 一份由資料庫支撐的名稱/IP 列表，通過 `/dns/rbl/local*` 手動管理，在外部提供方之前被檢查。**域名**型別的本地條目會以 `NXDOMAIN` 阻斷該域名的正向 A/AAAA 查詢，並立即生效（rolodex 在新增時更新記憶體快取）。
- **DNSBL 白名單**（rolodex 0.4.3+）—— 運維應對第三方訂閱源誤報的逃生艙口，通過 `/dns/dnsbl/allowlist*` 管理。一個條目覆蓋該名稱**以及它之下的每一個名稱**，因此把 `vendor.example` 加入白名單也會豁免 `cdn.vendor.example`。它會**短路整個基於名稱的檢查**，優先於已配置的 DNSBL 提供方以及任何匹配的本地阻擋名單條目，並且它在提供方查詢*之前*執行，因此被豁免的名稱永遠不會發出那次查詢。同樣由資料庫支撐並帶記憶體快取，因此立即生效。

  沒有它，面對一個把家庭所需名稱列入黑名單的訂閱源，唯一的補救辦法就是停用整個提供方。請注意它與本地黑名單的不對稱：白名單條目**只能是名稱**，絕不能是 IP，因為它所短路的正是基於名稱的那次檢查——如今 RBL 那一半已經退役，這也是僅剩的一種提供方查詢。
...
- **`refusal_codes`**，按提供方配置，作用於 DNSBL 提供方列表。每一項是一個 IPv4 地址或 `address/prefix`——之所以支援字首，是因為提供方公佈的是整段範圍，而 Spamhaus 把整個 `127.255.255.0/24` 保留給錯誤碼並會隨時間往裡新增新碼，因此把今天的三個枚舉出來，會導致明天的第四個被悄悄讀作命中。
...
**版本下限：** 早於拒絕碼處理的 rolodex 會接受這些欄位——proto3 忽略未知欄位——卻什麼也不儲存。mock 測試無法把這與成功區分開，因為 mock 會把遞給它的東西原樣回傳。`TestRolodexDnsblRefusalCodesRoundtripReal` 斷言：**空的**已配置列表讀回來必須是*已解析*的，而這正是老鏡像通不過的斷言。

**不存在訂閱源攝取/預快取**：提供方區域就是配置的單位；UI 提供一份精選的知名 DNSBL 區域列表作為一鍵快捷新增，但使用者可以新增任何區域。提供方區域的寫入會替換整份配置（經校驗、轉小寫、去重）。

**快捷新增列表是一種背書，並據此標準精選**（`ui/src/routes/dns/BlocklistsTab.jsx` 中的 `DNSBL_SUGGESTIONS`）。一個區域只有在家用機器開箱即可使用時才應出現在那裡：仍在運營、免費，並且無需註冊步驟即可應答一個自遞迴的解析器。當前有 Spamhaus DBL、SURBL、URIBL、NordSpam DBL 與 Spam Eating Monkey。
...
另請注意，DNSBL（域名）區域是針對郵件中的垃圾 URL 調優的，而非針對廣告或追蹤器——廣告/追蹤器攔截屬於訂閱源的領域，而那[被刻意排除在範圍之外](#阻擋名單)。曾經與它們互補的那些反向 IP 區域已隨 RBL 那一半一同消失，而它們對瀏覽本來也幾乎沒有影響：它們只在反向 DNS 查詢中出現 IP 時才被查詢，而普通瀏覽幾乎不產生這類查詢。
...
- **Blocklists** —— DNSBL 的提供方區域區塊（全域啟用開關、按區域的啟用/移除、按區域的拒絕碼設定、建議區域快捷新增、自定義區域新增——全部為按需查詢），外加一張手動本地條目表（新增/移除）。每個區塊的開頭會列出當前因拒絕查詢而被退避的提供方（如果有的話）。沒有訂閱源，沒有"應用"按鈕，什麼也不快取。
...
當機器上還沒有 Grafana 鏡像時，該拉取會在自己的 goroutine 中進行，並且只有鏡像落盤之後才替換 unit。有三件事依賴這個順序：設定請求不會為約 771 MB 而阻塞；在此期間由之前的後端繼續應答 `:5308`；並且——因為該 unit 是 `Type=simple`，systemd 在 `podman run` fork 的那一刻就認為它已啟動——機器不會在仍在下載時就報告 Grafana 已就緒。在整個視窗期內 `/monitoring/status` 都報告 `grafana: false`（`MonitoringUIPending`），這正是讓儀表板停留在「正在啟動」頁面、而不是去嵌入一個仍在提供舊後端的埠的原因。期間的第二次儲存是空操作而非第二次拉取；拉取失敗會清除該旗標並記錄日誌。
...
共有三個儀表盤，而且**兩種後端都從同一批查詢渲染出同樣的這三個**。它們之所以分開而不是併成一長頁，是因為它們回答不同的問題：System 是運維在機器髮卡時看的，DNS 是他們在某個名稱無法解析時開啟的，Controller 則是他們在 Town OS 所執行的某樣東西沒有在執行時開啟的。把八個 DNS 面板與十四個 controller 面板折進 overview，只會把那四個主機面板——人們開啟它的理由——埋掉。
...
**Controller**（Grafana uid `town-os-controller`，"Town OS Controller"）—— 基於 `systemcontroller` 抓取 job 的十四個面板，也是唯一讀取本機自身 [`townos_*` 指標](#system-controller-指標)的儀表盤。

排列順序就是出問題時運維實際的閱讀順序：什麼掛了、API 在做什麼、什麼在失敗、磁碟是什麼樣子、controller 行程自身撐得怎麼樣——最後才是清單，那本來就是 UI 裡的另一個頁面，在這裡只是一條幾乎不動的線。在正常運轉的機器上永遠不動的面板被放在最下面，而不是佔據最上面幾行、把真正會動的面板擠開。

1. **Service Health** —— `townos_system_unit_active` 與 `townos_package_unit_active`，每個單元一條序列，固定在 0–1。這個面板說明的是*哪一個*服務掛了，而不是掛了幾個，因此它排在最前。軸之所以固定，是因為該指標是布林值：自動縮放時，一台完全健康的機器會被畫成 1.0 附近的一團噪聲，恰恰在什麼都沒出問題的時候顯得嚇人。
2. **Service Units by State** —— `townos_system_units` 與 `townos_package_units` 按 state，同處一個面板且**不堆疊**：它們是兩個各自獨立的總數，堆疊會畫出一個誰也管不著的合計高度。
...
4. **API Latency** —— `rate(townos_http_request_seconds_total)` 除以 `rate(townos_http_requests_total)`，兩側都按 `method` 求和：每個請求的平均秒數。它把「機器很忙」與「機器卡住了」區分開，而第 3 個面板做不到這件事——一個每秒服務兩個請求的控制面，無論每個請求耗時 5 毫秒還是 5 秒，畫出來都一模一樣。用平均值而不是分位數，是因為這裡的暴露格式不帶 histogram（見 [System Controller 指標](#system-controller-指標)）；有東西開始阻塞時平均值就會動，而這正是要問的問題。
...
6. **Audit Events** —— `rate(townos_audit_events_total)` 按 `result`，堆疊。
...
8. **Town OS Disk Fill** —— 同一塊磁碟佔 `townos_disk_total_bytes` 的百分比，固定在 0–100。第 7 個面板無法回答「離滿還有多遠」，除非讀的人對著一根隨機器而變的軸心算；而固定的軸讓斜率變得可讀：自動縮放時，從 4% 爬到 5% 與從 90% 爬到 99% 畫出來一模一樣。分母刻意不做箝制，因此採集器讀不到的檔案系統會讓線斷掉，而不是畫出一個自信的 0%。
9. **Controller CPU** —— `100 * rate(townos_process_cpu_seconds_total)`。下界固定為 0，上界刻意**不**固定：這是按核秒計的，一個用滿兩個核的 controller 讀數是 200，而 100 的上限恰好會把這個面板存在的理由——失控——裁掉。
10. **Controller Memory** —— `townos_memory_heap_bytes` 與 `townos_memory_rss_bytes` 並列。兩者之間的差就是診斷：heap 攀升說明 controller 抓著本該釋放的物件不放；heap 平坦而 RSS 攀升，說明記憶體去了 Go 配置器記不到的地方。
11. **Controller Concurrency** —— `townos_goroutines`、`townos_open_files`、`townos_http_requests_in_flight`。三個計數在健康的機器上是平的，在洩漏的機器上會無止境地爬。它們共處一個面板，是因為它們通常一起動——一個永不返回的處理器同時佔著一個 goroutine、一個描述符和一個在途請求——也因為其中任何一個單獨拿出來都只是一條沒人會為它開分頁的線。
12. **Controller Uptime** —— `time() - townos_start_time_seconds`。訊號是那道鋸齒，而不是高度：一個在 `Restart=always` 之下悄悄崩潰重啟的 controller，在這裡的其他每個面板上都顯得健康。
13. **Inventory** —— 已安裝的套件、等待中的升級、已配置的倉庫，以及按名稱空間的 `townos_filesystems`。合成一個面板，而不是過去的三個：它們都是幾十這個量級的計數，因此共用一根軸仍然可讀，並且在一個地方回答「這台機器上有什麼」。
14. **Accounts** —— `townos_accounts` 按 kind，旁邊放 `townos_accounts_granted`，**不堆疊**。這些 kind 確實把賬戶列表劃分了一次，但持有授權的計數是 user 這一桶的*子集*；堆疊起來，這個面板畫出的總數會超過實際存在的賬戶數。

第 9 到第 11 個面板回答的是「機器為什麼慢」而不是「機器在跑什麼」，而這件事別的任何儀表盤都答不上來：這裡其他每一個指標族描述的都是 controller *管理*的東西，而當 controller 把 goroutine 洩漏進 swap 時，那一切看上去都很健康。Node Exporter 也補不上這個缺口——它報告的是宿主機，而在一台整個工作就是跑容器的機器上，controller 自己那一份在宿主機的總量裡是看不見的。

`townos_up` 與 `townos_packages_available` 刻意**不**作圖。前者是抓取存活性的常量，一條平直的 1 不成其為面板。後者是以千計的目錄規模：共用一根軸時，它會把旁邊那些計數壓成貼著底的一條線；而運維真正想從它那裡得到的東西——「是不是有倉庫不答應了」——是 `townos_repository_errors`，那有它自己的面板。`townos_disk_total_bytes` 只作為第 8 個面板的分母出現，從不作為一條序列：第 7 個面板已經把加起來等於它的那兩半堆疊好了。
...
**抓取的健康狀況也在這裡回報，因為別的地方都不報。** `/monitoring/status` 還帶回 `scrape_targets`（Prometheus 自己對它抓取的每一個 job 的看法，各自帶 `job`、`instance`、`health`、`scrape_url`，失敗的那些還帶 Prometheus 記下的 `last_error`）、`down_jobs`（只有至少一個目標失敗的那些 job 的名字，也就是儀表板橫幅要說的那句話），以及 `scrape_targets_error`（在 Prometheus 明明活著的機器上，這份清單為什麼是空的）。`monitoring.FetchScrapeTargets` 從回送位址上的 Prometheus 讀 `GET /api/v1/targets`，帶 3 秒的上限，這樣一個卡住的 Prometheus 不會把一次狀態輪詢一直吊著；而且**只有在 prometheus 單元處於 active 時才會去問**——單元停著的時候，它的單元狀態已經把話說完了，再疊一個連線錯誤只會讀成第二個不同的故障。

Prometheus 還沒抓過的目標報的是 `unknown` 而不是 `down`：每次重啟之後的第一個間隔裡，所有目標都處在那個狀態，把它算作 down 會讓每一次啟動都看起來像一次故障。

這個東西存在，是因為一個從來就沒成功過的抓取 job，在別的任何地方都看不見。這臺機器已經發過兩次這樣的車：`prometheus.yml` 聲稱 `https`，而 `:5309` 上跑的是明文；以及 rolodex 的 `metrics:` 小節從來沒能到達它的設定檔。兩次裡，所有單元都是 active，`systemctl --failed` 是空的，而受影響的面板畫出來的是**空圖而不是錯誤**——與一個沒什麼可說的服務毫無分別。回歸測試是 `TestFetchScrapeTargets*` 與 `TestHTTPMonitoringStatus{ReportsDownScrapeJobs,SaysWhyItCouldNotAsk,SkipsTargetsWhenPrometheusIsStopped}`（單元），以及 `TestMonitoringStatusReportsADownScrapeJob`（整合，對著一個真實的 Prometheus，其中一個 job 指向一個沒人監聽的埠）。
...
| `townos_http_request_seconds_total{method,status}` | counter | 服務請求所花的秒數，標籤與上一行*完全相同*，因為這兩者只有相除才有用 |
| `townos_http_requests_in_flight` | gauge | 此刻正在服務的請求數；為零時也發出，且 `/metrics` 請求自身被排除在外 |
| `townos_goroutines` | gauge | controller 行程的，不是宿主機的 |
| `townos_memory_heap_bytes` | gauge | 存活的堆物件（`HeapAlloc`），不是 `HeapSys`——有東西洩漏時會攀升的正是前者 |
| `townos_memory_rss_bytes` | gauge | 取自 `/proc/self/statm` 的常駐頁數，按頁大小換算，而不是假定 4096 |
| `townos_open_files` | gauge | `/proc/self/fd` 裡的條目數；Go 執行期不追蹤描述符，洩漏要到幾小時後以 `EMFILE` 的形式浮現 |
| `townos_process_cpu_seconds_total` | counter | 取自 `getrusage` 的使用者態加核心態時間 |

除 `townos_up` 與 `townos_packages_available` 之外，以上全部由 [Controller 儀表盤](#儀表盤)作圖，其面板集合是針對 `monitoring.ControllerDashboardMetrics()` 宣告的，因此兩份清單不可能漂移開。`townos_disk_total_bytes` 只作為磁碟佔用率面板的分母出現。

行程相關的這幾族是其中最新的，也是唯一描述 controller 自身而非它所管理之物的。`/proc` 與 `getrusage` 在每次抓取時各自獨立讀取，讀取失敗的那一族被省略而不是讓整次抓取失敗——這也是整合測試斷言的是正值而不僅僅是存在的原因：零意味著這次讀取根本沒有發生。
...
- **uPlot 模式**：在 React 中用 uPlot 直接渲染面板，在 5308 埠查詢 Prometheus。System 網格把自己釘在視口內（四個面板，每行兩個）；DNS 與 Controller 網格**不這樣做**——八個或十四個面板擠進一屏後每個只剩約 100px 甚至更少的畫布高度，到那個程度延遲圖就只是裝飾了，因此面板採用固定高度，頁面滾動。
...
監控、Rolodex 配置、核心鏡像拉取、TLS CA、ingress、pages 服務、物件儲存、網路 reconcile 與 UI 容器的啟動失敗都是非致命的；系統會在沒有它們的情況下繼續執行。所有容器鏡像拉取都使用下文所述的 `ensureImage` 助手（`systemcontroller.EnsureImage`）。非必要服務的拉取失敗會記錄到 stderr 且不阻止啟動，使系統即便在網路暫時不可用時也能啟動。

**啟動會刷新浮動鏡像標籤** —— `EnsureImage` 在決定如何處理一個引用之前，先用 `FloatingImageRef` 對其分類：

- **浮動** —— `latest` 家族的任意寫法：`latest`、`rc.latest`、`release.latest`，帶或不帶 `-x86_64` / `-aarch64` 後綴，以及完全不帶標籤的引用（podman 會將其解析為 `:latest`）。它們在**每次**啟動時都重新拉取，即便鏡像已在本機儲存中。對一個會移動的標籤而言，「存在」與「最新」是兩個不同的問題；而對未發生變化的標籤執行 `podman pull` 只是一次 manifest 檢查，並非下載。
- **固定** —— 帶日期的標籤（`rc.<date>-<arch>`、`release.<date>-<arch>`）、digest 引用、按實例產生的測試標籤，以及任何 `localhost/` 下的鏡像（它們沒有可供刷新的登錄檔）。只在缺失時拉取。

對本機*已存在*的鏡像刷新失敗不算錯誤：會使用本機副本並記錄該失敗，因此沒有登錄檔時機器仍能啟動 —— 這與它自己的 unit 使用 `--pull=missing` 而非 `--pull=always` 是同一個原因。

**控制器自我更新** —— `SelfUpdate` 作為 `boot_services` 的第一步執行，是同一個問題的另一半：`EnsureImage` 能讓機器執行的每個服務保持最新，卻對執行它的這個行程無能為力。控制器自己的 unit 刻意使用 `--pull=missing`（崩潰後的重啟不能依賴登錄檔可用），因此在維運人員按下*刷新系統服務*之前，執行中的控制器一直停留在這台機器安裝時的鏡像上 —— 一台除了「負責更新的東西」之外什麼都更新的機器，恰恰唯獨它自己永遠無法交付針對自身的修復。

這一步會：

1. 向 podman 詢問*本*容器是從哪個鏡像引用啟動的（`{{.ImageName}}`），而不是從 `TOWN_OS_TAG` 推導。安裝建置系統會把它拿到的 `CONTROLLER_IMAGE` 原樣寫進 unit —— 那可能是另一個完全不同的登錄檔 —— 刷新一個 unit 實際並不執行的引用，就會在兩個毫不相干的鏡像之間比較，於是每次啟動都重啟這台機器。
2. 當該引用是固定的（維運人員刻意固定，或 `make dev` 下的 `localhost/` 鏡像）時什麼都不做。
3. 對它執行 `EnsureImage`，然後把標籤現在所指的鏡像 ID 與本行程正在執行的 ID 相比。
4. 兩者不同時，把目標 ID 寫入 `<btrfsPath>/town-os-self-update`，並請求 systemd 重啟 `town-os-systemcontroller.service`。unit 的 `--pull=missing` 隨即啟動剛剛刷新到本機的那個鏡像。此次啟動會在此等待，而不是帶著即將被丟棄的工作繼續進入 reconcile。

那個標記檔就是迴圈護欄：如果重啟回來後仍在執行同一個舊鏡像 —— 例如 unit 解析該標籤的方式與我們不同 —— 機器只嘗試一次，隨後記錄日誌並繼續，而不是每次啟動都重啟、永無止境。標籤再次移動就是一個新目標，會獲得自己的一次嘗試；而被 systemd 拒絕的重啟會清除該標記，因為一次從未發生的重啟不應壓制下一次。

對既有機器的影響：它執行的控制器必須先包含這段程式碼才能用上它，所以仍然需要恰好一次手動的*刷新系統服務*（或重新安裝）來完成過渡。此後重啟一次就夠了。

`TOWN_OS_IMAGE_REFRESH=0`（以及 `false`/`no`/`off`）會關閉刷新 —— 包括自我更新，它隨後只會發現「什麼都沒變」 —— 此時固定標籤的規則適用於所有鏡像；缺失的鏡像仍然會拉取。`make dev` 與兩個測試容器都設定了它，因為它們執行的每個鏡像都是刻意從本機鏡像快取載入的 —— 在那裡刷新會在每次啟動時去 quay.io 取回一個本就刻意放置的鏡像。任何無法辨識的值都維持刷新開啟：一台機器悄悄不再取得自己的服務，比多拉取一次更糟。在容器上設定它只做了一半：那些容器裡的 PID 1 是 systemd，所以測試單元的 `PassEnvironment` 白名單（`integration/testdata/town-os-systemcontroller.service`）也必須寫上這個變數，否則它止步於 PID 1，而唯一需要它的那個行程 —— 系統控制器 —— 永遠看不到。白名單當時沒有寫上，於是每一次測試與 dev 啟動都在刷新：rolodex、Prometheus 與 node-exporter 在啟動拉取集裡從 quay.io 重新拉取，而測試腳本只給 `/status/ping` 120 秒作答，一次執行是否失敗取決於上行鏈路。`TestHarnessEnvironmentReachesTheController` 現在從 `make/test.sh`、`make/dev.sh` 與 `system_port_env` 推導出必需的名字，整合測試則讀取執行中控制器的 `/proc/<pid>/environ` 來證明它們確實抵達。

這個區分之所以重要，是因為安裝器不附帶任何鏡像，而每台機器的標籤預設都是浮動的（`rc.latest-<arch>`）。沒有它，「已載入」就等同於「最新」，於是每個系統服務都停留在這台機器第一次啟動時下載的鏡像上 —— 一台重裝過的虛擬機無論重啟多少次都還在提供數月之前的 UI，唯一能推進它的只有維運人員的 `POST /system-services/refresh`。
...
剩下的問題是：誰來算繪 dev 機器據以綁定的 `rolodex.yml`。沒有人——這正是上面那個致命等待所揭出的 bug：rolodex 的 DNS 監聽器只在啟動時從那個檔案裡打開**一次**，而 Town OS 透過 gRPC 說什麼都打不開一個；`ProgramRolodex` 擁有轉發器、解析模式和兩份阻斷清單，唯獨刻意不擁有 bind（`controller_rolodex_config.go`）。真實機器上由 install 映像算繪它（`../install/scripts/rolodex-config.sh`，它唯一的寫者）。`make dev` 不跑安裝器，於是 `${STATE_DIR}/dev-rolodex` 是空的，而 `town-os-system--rolodex.service`——它的 `ExecStartPre` 建立的是**目錄**——讓 rolodex 以 `--config /data/rolodex.yml` 啟動，指向一個從未存在過的檔案。單元照樣進入 active，因為容器本身起得好好的，缺的只是監聽器；這正是那個等待既問位址、又不止問單元的原因。現在 `seed_dev_rolodex_config`（`make/dns.sh`）在容器掛載該目錄之前於主機上寫好它：只綁回環——DNS 的 bind、指標監聽器、控制器要撥的 gRPC 通訊端、一對起步用的轉發器——並且每次執行都重寫，因為 `dev-rolodex` 要持久保存 `rolodex.db`，而舊 schema 的組態會比寫下它的那個 checkout 活得更久。刻意在主機上播種而不烤進映像：同一個單元檔案也隨整合測試映像發布，而在一個 `--net host` 的測試容器裡，綁定 `127.0.0.2:53` 的 `rolodex.yml` 會在每次執行時綁掉**主機的** `:53`。沒有它時，每次啟動還要額外付出兩段無條件的三十秒等待——`Manager.WaitForDNSReady`，以及 `reconcileDNSAndNetworks` 裡的 gRPC 通訊端撥號迴圈——而這都發生在 `require_controller_ready` 給出的 120 秒預算之內，偏偏這次啟動在徹底完成之前對 `/status/ping` 一律回 503。

同一次啟動也決定了 `make dev` 在啟動它之前必須往容器裡放什麼。每個產生的單元跑的 `podman run` 都**不帶** `--pull=never`，所以一個不在內層儲存裡的映像檔並不是一個會快速失敗的錯誤——它是一次發生在 `podman run` 內部的映像倉庫拉取，而與此同時 `require_controller_ready` 的 120 秒和該服務自己的就緒等待都在倒數。ingress 與 pages 服務所用的 caddy 都不在那份清單裡（caddy 在整合工裝裡也不在），於是 dev 機器在啟動期間從 quay 和 Docker Hub 去取它們，而對應的 tar 早就躺在這個 checkout 的映像快取裡。現在兩者都會被載入，ingress 由 `ingress-image` 這個前置目標在本地建置。另有兩樣東西本不該被載入：**Grafana** 約 1.17 GB，且是那個*非*預設的監控後端，除非儲存的後端是 `grafana`，否則啟動根本不會執行它——因此除非 `DEV_LOAD_GRAFANA=1`，一律跳過；而 **gfeh** 不再是 `dev` 的前置目標，因為它是唯一內容來自儲存庫之外的本地映像檔，因而每個日曆日都要把整棵 `gfehd` 相依樹重新編譯一遍，所有人都要付，無論用不用。`make gfeh-image` 會在 dev 中啟用物件儲存，正如 `make/dev.sh` 一直聲稱的那樣。載入本身如今經由唯讀掛載在 `/image-cache` 的映像快取進行，而不是 `podman cp`，這省掉了每個 tar 的一整次寫入；沒有該掛載的容器仍會回退到複製。最後，systemcontroller 在 dev 映像檔裡**不再被 enable**：systemd 過去在 `podman run -d` 回傳的那一瞬間就啟動它，面對的是一個空的儲存，於是買來一次註定失敗的啟動——整整 30 秒的 DNS 等待、對一切尚未載入之物的拉取、對著不存在的映像檔啟動單元、複製每一個套件儲存庫——這一切都被 `make/dev.sh` 在載入完成後發出的那次啟動丟棄，而且這一切還在跟那次載入搶資源。rolodex 仍然保持 enable，因為它必須在控制器啟動之前就已在服務，而它的單元本就是為熬過這段等待而寫的。
...
| `dns_dnsbl_config` | 未配置（與"空"不是一回事） | `POST /dns/dnsbl` |
