...
- **Network controller image is built on the host as the first boot operation** — the NC container image (`localhost/town-os-networkcontroller:local`) is built on the **host's** podman (via `nsenter -t 1 -m -u -i -n -- podman build`) as the very first operation in the systemcontroller startup, before rolodex config, image pulls, or anything else. This is because all system service units and package NC units run on the host's podman, so the image must exist in the host's image store. The build directory is placed under `/town-os/build/` (shared bind-mount visible from the host). The NC binary (`/town-os-networkcontroller`) is baked into the systemcontroller image at build time, guaranteeing the NC always matches the running systemcontroller. The `alpine:latest` base image is pulled via `ensureImage` (also on the host) if not already loaded. The build uses `--dns=8.8.8.8` so it does not depend on rolodex DNS. The build is non-fatal; if the network is unavailable at boot, the system controller starts without per-package networking and the image is built on the next restart. Never cache or skip the NC image build. All podman commands in the systemcontroller (`ensureImage`, `buildNetworkControllerImage`) use the `hostPodman()` helper which wraps commands with `nsenter` to execute in the host's namespaces.
...
12. **Build network controller image on host** **(non-fatal)** — the very first runtime operation. Copies the NC binary from `/town-os-networkcontroller`, pulls `alpine:latest` on the host via `ensureImage`, runs `podman build` on the host via `nsenter`. Uses `--dns=8.8.8.8` so it does not depend on rolodex. Build directory is `/town-os/build/`.
...
15. **Write Rolodex config and restart if changed** **(non-fatal)** — Rolodex is a boot service managed by systemd. The systemcontroller writes `rolodex.yml` (idempotent: skips if file is newer than the binary or content is unchanged) and restarts the service only when the file was written.
...
18. **Pull container images on host** **(non-fatal)** — Prometheus, Node Exporter, UI image, optionally Grafana. Uses `ensureImage` via `hostPodman` (skips pull if already loaded on host).
