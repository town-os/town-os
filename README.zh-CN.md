...
> **英文原件为准**。繁体中文译本见 [README.zh-TW.md](README.zh-TW.md)。
> 仓库中的其他文档也有中文译本：
> [CLAUDE.zh-CN.md](CLAUDE.zh-CN.md)（构建与代码风格规则；繁体：[CLAUDE.zh-TW.md](CLAUDE.zh-TW.md)）与
> [DESIGN.zh-CN.md](DESIGN.zh-CN.md)（架构与功能规格说明；繁体：[DESIGN.zh-TW.md](DESIGN.zh-TW.md)）。
...
整个界面都做了国际化。所有面向用户的字符串——后端错误信息与前端 UI 文本——都经由一个以 BCP 47 语言环境代码（例如 `en-US`、`de-DE`）为键的消息目录路由。**有 30 种语言在前后端都完整翻译**：阿拉伯语、孟加拉语、中文（简体与繁体）、克罗地亚语、捷克语、丹麦语、荷兰语、英语、芬兰语、法语、德语、印地语、匈牙利语、意大利语、日语、韩语、波兰语、葡萄牙语、罗马尼亚语、俄语、梵语、斯洛伐克语、斯洛文尼亚语、西班牙语、瑞典语、泰语、土耳其语、乌克兰语与越南语。

在这些语言之上还有 **18 种国家变体**——`en-GB`、`en-AU`、`en-CA`、`en-IN`、`en-NZ`、`en-ZA`、`de-AT`、`de-CH`、`fr-BE`、`fr-CA`、`fr-CH`、`es-AR`、`es-MX`、`pt-PT`、`nl-BE`、`ar-AE`、`ar-EG`、`bn-IN`——合计 **48 个可选的语言环境**。变体并不是第二份翻译：它继承所属语言的目录，只列出该国家确实说得不一样的那些字符串，因此一条新消息在其基础语言拿到它的那一刻就抵达每一个国家，而对一条法语字符串的修正只需做一次，而不是在四个文件里各做一遍。其中有几份覆盖清单坦然就是空的。加拿大英语保留美式的 *-ize* 拼写；阿根廷西班牙语以 *voseo* 闻名——但 voseo 替换的是亲近的 *tú*，而本目录全程以 *usted* 称呼读者，这在布宜诺斯艾利斯与在马德里读起来并无二致。空清单仍然意味着已审阅，而不是被遗忘。系统设置中的语言界面以母语文字呈现 21 种常用语言，并带一个可展开的、包含 89 个国家/地区代码的清单；没有对应目录的会显示出来但被禁用。

UI 从你的浏览器（`navigator.languages`）选择语言。精确匹配优先，因此 `de-CH` 得到的是瑞士德语，而不是被折叠到德国的那一份。中文按文字消歧。对于我们没有提供任何目录的国家，语言会落到它实际阅读的那一份——`es-CO` 落到墨西哥西班牙语而不是半岛西班牙语，`en-IE` 落到英式而不是美式——而一个光秃秃的 `en` 或 `pt` 会解析到一个有名有姓的默认值，而不是碰巧最先加载的那个变体。明确的选择会按浏览器记住，因此这台机器的全局语言设置不再覆盖每个人所看到的内容——它只是 Town OS 没有对应目录的语言的回退值。
