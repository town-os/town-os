...
17. **Stage `boot_services`: pull core container images** **(non-fatal)** — the NC image, Prometheus, Node Exporter, the UI image, the object-storage (gfeh) image, the ingress image, and Grafana when that backend is selected, in parallel via `parallelEnsureImages` (skips a pull when the image is already loaded). Every image a boot-time unit references belongs here: a unit whose image is not local pulls it from inside `podman run`, so its readiness wait races a registry download. gfeh and then the ingress were each missing from this list in turn, and each read as a service that was simply down. The monitoring UI needs no entry — on the uPlot backend it runs the NC image, which is already first in the set.
