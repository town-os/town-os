...
- **`WriteConfig`** -- renders `rolodex.yml` into `DataDir` and writes it whenever the bytes differ from what is on disk. Returns a boolean indicating whether the file actually changed (so the caller can restart the systemd unit only when there is something to restart it for). **It reconciles unconditionally, and that is load-bearing** — see [The config that froze](#the-config-that-froze).
...
The mode is operator-configurable at runtime via the `dns_resolution_mode` setting (`auto` | `recursive` | `forward`; validated by `ValidateDNSResolutionMode`, so an unparseable value can never reach `rolodex.yml` and brick DNS). `main.go` reads it into `rolodex.Config` at boot; a change through `POST /settings/set` runs `Controller.RefreshDNSResolutionMode`, which calls **`Manager.RewriteConfig()`** and restarts the rolodex unit. `RewriteConfig` is now the runtime *name* for `WriteConfig` and nothing more — the two behave identically. They used to differ, and that difference is the subject of the next section.
...
Nothing real was protected. Every value in the file is rendered from Town OS state — DNS port, forwarders, resolution mode, both blocklists, the metrics port — and every runtime path that changes one of them already overwrote the file regardless (that is what `RewriteConfig` was). A hand edit survived until the next settings change before; now it survives until the next boot.

The regression tests are `TestWriteConfigReplacesConfigNewerThanTheBinary` and `TestWriteConfigAddsSectionsMissingFromAnOlderRendering` (unit), and `TestUpgradedControllerOpensRolodexMetricsListener` (integration), which plants a pre-`9689461` config with a future mtime, reconciles it through the boot entry point, and scrapes the listener a real rolodex container then opens.
