...
3. **Bind `:5309` with the boot handler** — `NewBootStatus()` + `NewRootHandler(NewBootHandler(bs))` bind the listener immediately, before any startup work. Until the swap in step 30 the socket answers only `GET /status/ping` (503 with `{booting, step, done, boot_id}`) and `GET /boot-status` (SSE); everything else is 403.
...
13. **Derive the NC image** — `quay.io/town/networkcontroller:<tag>`, overridable via `NC_IMAGE`. Pulled (step 18), never built.
14. **Start background repo refresh** — goroutine polls every 5 minutes.
15. **Stage `boot_dns`: build the rolodex manager and wait for DNS** **(non-fatal)** — Rolodex is a boot service managed by systemd, and by this point it is already answering: it starts before the systemcontroller from the `rolodex.yml` the install image wrote, because the controller cannot pull an image until something resolves names. **Nothing is written to `rolodex.yml` here and nothing restarts the service** — see [rolodex.yml is bootstrap-only](#rolodexyml-is-bootstrap-only-settings-are-programmed-over-grpc), and [The config that froze](#the-config-that-froze) for the failure that ended the write. What this stage builds is the `rolodex.Manager`, from the settings Town OS owns: `resolution.mode` from `dns_resolution_mode` (an unparseable stored value falls back to the default rather than programming a mode rolodex would refuse), the forwarder list from `dns_local_forwarders` (when it is on, the list is discovered from the host's resolvers on every boot, so a box that changed networks picks the new one up without the operator touching anything — see [Local forwarders](#local-forwarders)), and the blocklist from its stored setting. Every one of those is pushed into the **running** server later in this boot, by `ProgramRolodex` in the reconcile step. The rolodex container runs with `--net host` and binds DNS to `127.0.0.2:{port}` directly. Then wait for DNS readiness (TCP connect poll), and configure systemd-resolved to route the TLD to rolodex — **skipped when `TOWN_OS_DNS_PORT` has moved rolodex off `:53`**, since a resolved per-domain server address carries no port and would blackhole every query for that TLD.
...
17. **Stage `boot_services`, first step: self-update** **(non-fatal)** — `SelfUpdate` refreshes the controller's *own* image and, when the tag it runs under has come to name a different image, asks systemd to restart `town-os-systemcontroller.service` and stops this boot there (see *The controller updates itself*).
18. **Stage `boot_services`: pull core container images** **(non-fatal)** — the NC image, Prometheus, Node Exporter, the UI image, the object-storage (gfeh) image, the ingress image, and Grafana when that backend is selected, in parallel via `parallelEnsureImages` (a **floating** tag is re-pulled on every boot; a pinned one is fetched only when it is missing — see *Boot refreshes floating image tags*). Every image a boot-time unit references belongs here: a unit whose image is not local pulls it from inside `podman run`, so its readiness wait races a registry download. gfeh and then the ingress were each missing from this list in turn, and each read as a service that was simply down. The monitoring UI needs no entry — on the uPlot backend it runs the NC image, which is already first in the set.
19. **Start monitoring system services** **(all non-fatal)** — legacy NC/socket monitoring units from the previous design are torn down first (they still hold `-p 9090`/`-p 5308` and would crash-loop the new services). Node Exporter, Prometheus, and the Monitoring UI all run `--net host`; node-exporter and Prometheus bind loopback, and only the monitoring UI's `:5308` is LAN-facing. All three ports come from `monitoringPortsFromEnv()`, whose zero value is the production defaults ([System-service host ports](#system-service-host-ports)). Then install the nightly podman prune timer **(non-fatal)**. The daily update timer is not installed here — it ships with the installer, see [Automatic updates](#automatic-updates).
20. **Ensure the local TLS CA** **(non-fatal)** — `tls.EnsureCA(<btrfsPath>/tls)` before reconcile, so reconcile can issue leaf certs as it walks installed packages.
21. **Start the ingress and the pages service** **(non-fatal)** — `ingressctl.Manager` installs and starts `town-os-system--ingress` (shared `:443` SNI + `:80` Host router), dual-stack only when the host has a global IPv6. The pages Caddy service starts alongside it. Both are skipped when `INGRESS_IMAGE` is explicitly set to empty (dev mode).
22. **Reconcile object storage** **(non-fatal)** — `ReconcileGfeh` ensures one gfeh partition per network: the `gfeh/<network>` subvolume (chowned to uid 2000), the rendered `gfehd.yaml`, and the `town-os-system--gfeh-<network>` unit, restarted only when the rendered content changed. Skipped entirely when `GFEH_IMAGE` is explicitly empty, and skipped when the ingress is disabled (the four HTTP views are only reachable through it). The partitions' *names* are published later and asynchronously — see step 31. See [Object Storage (gfeh)](#object-storage-gfeh).
23. **Detect version change** — compare the running container's image SHA (`/proc/1/cgroup` → `podman inspect`) against `<btrfsPath>/town-os-version`. Sets `versionChanged` for reconcile.
24. **Reconcile** — iterates all installed packages and restores runtime state:
...
Neither failure is caught by the suites, for different reasons. The **unit** suite stands in a fake gfehd and never runs the daemon at all. The **integration and UI** suites do run the real one — `make/test.sh` loads the built image into both containers, because there is no stand-in that would prove a partition starts, answers its admin socket, and enforces its own ceilings — but a merely *old* daemon still builds, pushes, installs, and starts. It shows up only as object storage that never starts on a real box. For the record, since it is why the floor existed: 0.1.1 cannot parse a partition config carrying any SMB user (every gfehd config struct is `#[serde(deny_unknown_fields)]` and its `SmbConfig` has no `users` field), and it authenticates the instant it starts, which during boot is the `:5309` stub answering 403 to everything but the ping.

**Both builds defeat the layer cache, by different means.** `cargo install gfehd` is a byte-identical `RUN` line on every build, so its layer is a permanent cache hit — a build whose whole contract is "whatever crates.io holds today" would otherwise serve the first build's crate forever, silently, with clean logs every time. There is no cheaper cache key: knowing when the crate changed means asking crates.io, which is the thing the build is for.

- `release-gfeh` passes **`--no-cache`**. Nothing weaker is acceptable in something that ships.
- `gfeh-local` passes a day-granularity **`GFEH_CACHE_DATE`** build-arg. The fixture is a prerequisite of every integration and dev run, so `--no-cache` there would recompile the Rust dependency tree on each one; a pure cache hit, though, would freeze it on whatever gfehd was current the first time it was built on that machine — and the integration and UI suites start real partitions against it. Daily is the compromise: a cache hit within the day, a rebuild at the rollover.

Either way the cargo registry volume survives, so the dependency tree is re-compiled but not re-downloaded. This is a specific case of a general build rule ([CLAUDE.md](CLAUDE.md)): a local image built from repo source cannot drift, because a source change busts its cache; one whose content is fetched at build time needs an explicit bust.
...
**Enrollment on it is refused, deliberately and at both layers.** `POST /networks/peers/add` returns a 400 for `home`, and `NetworkManager.AddPeer` returns `ErrNetworkDNSOnly` regardless of who calls it. This matters because [every account belongs to the home network](#every-account-belongs-to-the-home-network): if enrollment there were accepted, membership alone would amount to a way onto a tunnel, and the stored peer would describe a tunnel that does not exist and never will. Peers are created dynamically on a real overlay, so a caller that wants one names it.

The refusal used to be **incidental**: nothing checked the network, and the handler fell through to `netip.ParsePrefix` on the seeded row's empty `Subnet`, which failed and surfaced as a **500**. That reads as a broken box rather than a refusal, it tells the caller nothing about why, and it would have stopped refusing the moment anything wrote a subnet onto that row. The guard is by name, ahead of the server-side keypair generation, with a transportless-row check behind it for any network whose subnet is empty for some other reason.
...
- `POST /networks/peers/add` (`requirePeerEnroll`: admin or the `wireguard` grant, confined to the caller's networks) -- register a peer. When `public_key` is empty the server generates a keypair and returns the private key plus a ready-to-import device config. Accepts an optional `endpoint` and a `rolodex` flag. **The home network is a 400** — it is DNS-only and carries no peers, no matter who asks (see [The home network always exists](#the-home-network-always-exists)).
...
Output is **sorted by hostname** so the rendered bytes are deterministic across reconciles — that is what lets the supervisor no-op a reload whose content has not changed. Globals are `auto_https off` (Town OS manages certs), `protocols h1 h2` (the ingress publishes TCP only, so H3/QUIC over UDP is unreachable), and the admin endpoint. The Caddy admin API is deliberately **left enabled**: the supervisor programs new routes with `caddy reload`, which talks to that endpoint, so `admin off` would break every route update after the first boot.

Its address is **written into the config** as `admin 127.0.0.1:<port>` rather than left to Caddy's default, defaulting to Caddy's own `2019` (`ingress.DefaultAdminPort`, overridable with `--caddy-admin-port` / `ingress.WithCaddyAdminPort`). Production keeps 2019 — the ingress container has its own network namespace and publishes nothing but :443, :80 and the loopback metrics port, so the admin API is reachable from inside that container and nowhere else. **Anything running Caddy in the host namespace must relocate it**, and every test does: `go test ./src/...` runs on the host outright and the integration container is `--net host`, so two concurrent `make test-full` runs would put two Caddy children on the one :2019 — the second fails to bind and exits, and before it does, either run's `caddy reload` can land on the other's admin API and program it with the wrong Caddyfile (IRON RULE). Rendering the address is also what *points* `caddy reload` at it: that command reads the admin address out of the config it adapts. The `127.0.0.1` spelling is deliberate — `localhost` resolves to `::1` first on a dual-stack box while Caddy binds only one of the two.
...
### When the backend is down: the retry page

A backend behind a programmed route can be broken in two ways, and the ingress answers both with the same page: a **503 that says the service is unavailable and reloads itself every five seconds** (`unavailableRefreshSeconds`, which is also the `Retry-After`) until the service answers again. Nothing is reprogrammed and no route is withdrawn — the address stays routed the whole time, so the moment the backend has a listener again the next reload serves it.

This is the ordinary state of a home box, not an exception: a package restarts on upgrade, a settings change bounces a service, a reboot brings twenty containers back in some order. What Caddy says during those seconds is `502 Bad Gateway`, which reads as *broken* rather than as *coming back*, and leaves the reader with nothing to do but retry by hand.

The two failure shapes are separate mechanisms in Caddy, and an implementation that covered only one would miss the common case:

- **The backend answered, with a 5xx.** That is an upstream *response*, intercepted with a response matcher and `handle_response` **inside** the `reverse_proxy` block — which is why every proxy the renderer emits now opens a block, including the plain-HTTP ones.
- **The backend never answered** — container down, name unresolvable, connection refused. `reverse_proxy` fails and Caddy raises its own error, which `handle_response` never sees; that is `handle_errors` at **site-block** level. This is the common one on a home box, and the one a 5xx-only implementation would silently miss, because the 5xx it is watching for never arrives.

Both paths are rendered as Caddyfile **snippets** (`render_unavailable.go`), defined once per file and imported by each site block — so the page cannot drift between vhosts, and the file stays a readable size no matter how many routes the box serves. The snippets are emitted for an empty route set too: a snippet nothing imports is never parsed as directives, and a preamble that appeared and disappeared would bounce Caddy on the first route.

- **Only a browser gets the page.** Both paths gate on `method GET HEAD` plus `Accept: *text/html*`. An API client — an XHR, a webhook, `curl`, a DoH resolver behind a path backend — gets the upstream's own status, body and headers copied through on the response path, and a plain 503 with `Retry-After` on the error path. Handing HTML to something parsing JSON turns one broken backend into a second, stranger failure in its caller.

  The passthrough is `copy_response` **alone**, with no `copy_response_headers` beside it, and that omission is load-bearing: `reverse_proxy` has already staged the upstream's headers on the response writer by the time a response handler runs, so copying them again emits every one of them twice — `Content-Length` included, which makes the message malformed rather than merely untidy. It reads like the obvious companion directive, which is why both a render test and an integration assertion pin it.
- **The page names the service from the rendered config, never from the request.** The label is a snippet argument substituted at import time — the route's hostname, already validated by `validSiteHost`. The `:80` fallback block is labelled with the constant `Town OS` instead, because it answers for *every* unmatched host: its `Host` header is whatever the client sent, and reflecting that into HTML would be an injection on the one vhost that accepts any name at all. `safeUnavailableLabel` is the guard that keeps that true for a future caller.
- **A package's `:80` redirect block gets neither**, since it proxies nothing — there is no backend there to be unavailable.

#### The page is translated

The four sentences on it are catalog entries (`ingress.unavailable.*` in `src/i18n`), not literals in the renderer — every language Town OS ships gets its own rendering of the page, and Caddy picks one per request from `Accept-Language`.

That mirrors what the UI already does, and for the same reason: **the reader's language is the browser's, not the box's.** `I18nContext` resolves a stored choice, then `navigator.languages`, and only falls back to the server's global `locale` setting when the browser asked for a language no catalog covers. The retry page resolves the same order in Caddy — a `map` over `Accept-Language` with one row per language catalog, and a `default` row carrying the box's locale. A guest whose phone is in Portuguese reads Portuguese on a box configured in German.

- **A `map`, and one copy of the page — not a `handle` block per language.** The block form works and was measured: because an `import` is textual, every expansion inlines the whole selector, so thirty branches meant thirty copies of the page in the JSON Caddy is handed on each reload — **115 KB for a single route**, and a 20-route box would have pushed ~2.3 MB on every package install, page edit and hourly reconcile. The map holds the translated sentences in a compact table and the page is written once, reading `{townos_title}` and friends out of it.
- **The 5xx path renders nothing; it raises `error 503`** into the site's own `handle_errors`, which is what makes the page expand once per site block rather than once per proxy. Verified: an `error` raised inside `handle_response` does reach the enclosing error handler, with the original request intact, so the `Accept` gate still applies.
- **The selector is one row per LANGUAGE, not per locale.** Country variants (`de-AT`, `en-GB`, `es-MX`…) derive from their base language and override only what that country genuinely says differently — none of which is these four sentences — so a row each would be thirty more regexes rendering the same page. `TestUnavailableLocalesCoverEveryCatalog` derives the expected membership from `i18n.PopulatedLocales()` and fails in both directions, so a language added to the catalogs cannot quietly stay English here.
- **Traditional Chinese is matched before the generic `zh`.** Caddy tests map rows in order and takes the first hit, and `^zh` claims `zh-TW`, so the wrong order serves Simplified to Taiwan, Hong Kong and Macau — which looks like a working translation to everyone who cannot read it. Every pattern is anchored, case-insensitive, and ends at a separator or the end of the header, so `^ja` cannot answer Javanese in Japanese.
- **The service name is a format verb, not a concatenation.** `"%s is unavailable"` puts the name in a different place in Japanese than in English, so the catalog owns the word order; the renderer substitutes `{args[0]}` into the translated string, and Caddy substitutes the hostname into that when the site block imports the snippet.
- **The box's locale reaches the ingress on the unit's command line** (`--locale`, `ingressctl.Config.Locale`, read from the `locale` setting at unit generation). It is one box-wide setting rather than route state, so it rides the unit rather than the gRPC push, and a language change reaches the ingress the way every other unit change does — a different rendered unit on the next reconcile. A code with no catalog is ignored rather than rejected: the setting is a free-text row, and the cost of a typo should be an English page, not an ingress that will not start.
- **Every translated string is escaped for both boundaries it crosses** (`caddyHTMLText`): it is a Caddyfile token and then the contents of an HTML element. HTML escaping also removes the quote that would end the token early; the backslash is escaped separately because Caddy reads it as an escape and Go's `html` package has no reason to touch it.
...
Plus **caddy's own families**, fetched from the child's container-local admin API (`127.0.0.1:<admin port>/metrics`, built from the same address the Caddyfile carries so the scrape cannot be left behind on 2019 by a relocated child) and appended to the response verbatim. They are namespaced `caddy_*`/`go_*`/`process_*` and cannot collide with the `townos_ingress_*` ones. Passing them through is the only way they reach Prometheus at all, since the admin API is deliberately unpublished, and they describe the process that actually serves every request the box answers. The body is not re-rendered: parsing it back into families could only lose information (caddy exports histograms, which `src/metrics` deliberately does not model).
...
- **`Forwarders` / `ResolutionMode` / `Blocklists`** -- the settings Town OS owns, resolved from stored configuration and host discovery. The manager writes no file: these are what `ProgramRolodex` pushes into the running server. See [rolodex.yml is bootstrap-only](#rolodexyml-is-bootstrap-only-settings-are-programmed-over-grpc).
- **`Generation`** -- the identity of the gRPC socket rolodex binds at startup (device, inode, mtime), which changes exactly when rolodex restarts and is therefore how a restart that dropped every programmed setting is noticed. Empty when the socket is absent, meaning rolodex is not running.
...
The mode is operator-configurable at runtime via the `dns_resolution_mode` setting (`auto` | `recursive` | `forward`; validated by `ValidateDNSResolutionMode`, so an unparseable value can never reach `rolodex.yml` and brick DNS). `main.go` reads it into `rolodex.Config` at boot; a change through `POST /settings/set` runs `Controller.RefreshDNSResolutionMode`, which programs the running server with **`SetResolutionMode`** — no file, no unit restart, no DNS interruption. rolodex refuses an unrecognized mode outright rather than defaulting, which is why the value is validated before it is sent.

### The config that froze

`WriteConfig` used to skip any `rolodex.yml` whose mtime was newer than the systemcontroller binary's, on the theory that a newer file had been hand-edited. **The file every boot writes is newer than the binary in the image that wrote it**, so from the second boot onward that guard matched the controller's own output, and `WriteConfig` became a permanent no-op. `rolodex.yml` froze at whatever the first controller ever to run on that box rendered, for the life of the box, and no image update could move it. `RewriteConfig` existed only to route around this on operator-initiated changes.

Every section added to the template after a box's first boot therefore never arrived on that box. That shipped twice — `dnsbl:` (`31f7e80`) and then `metrics:` (`9689461`) — and the second one is why a deployed box had no rolodex metrics listener at all. Prometheus scraped `127.0.0.2:9153` every 15 s on schedule, nothing was ever bound there, `rolodex_*` series never existed, and every DNS panel rendered an **empty chart rather than an error** — indistinguishable from an idle resolver. `systemctl --failed` was empty and every container was up throughout.

Nothing real was protected. Every value in the file was rendered from Town OS state — DNS port, forwarders, resolution mode, both blocklists, the metrics port — and every runtime path that changed one of them already overwrote the file regardless (that is what `RewriteConfig` was).

**This section is history.** `WriteConfig` and `RewriteConfig` are both gone, and Town OS writes no rolodex configuration file at all: the settings they used to render are programmed into the running server instead, and the file that remains belongs to the install image. See [rolodex.yml is bootstrap-only](#rolodexyml-is-bootstrap-only-settings-are-programmed-over-grpc). It is kept because the failure it describes — a config write that silently did nothing, surfacing only as a feature that never worked — is the shape of bug this subsystem keeps producing.

Those regression tests are gone with the code they guarded; what replaces them is listed in the next section.

### rolodex.yml is bootstrap-only; settings are programmed over gRPC

`rolodex.yml` is written by the install image (`scripts/rolodex-config.sh` in `../install`, as rolodex's `ExecStartPre`), and by nothing else. It carries exactly the two things that cannot be set on a running rolodex: the DNS **bind list** (`127.0.0.2:53`, UDP and TCP — the address systemd-resolved is pointed at, and the listeners have to exist before any API call can be made) and the **metrics listener**, which rolodex opens once at startup from that section's presence. It also carries `resolution.mode: auto` and the public forwarders as BOOT DEFAULTS, matching `DefaultForwarders` here, so the window before the first push is a defined state rather than rolodex's compiled-in choice. Every value in it is a constant, so the file is byte-identical on every start. Town OS writes no config file at all.

Everything else Town OS owns — the forwarder list, `resolution.mode`, and the blocklist — is programmed into the RUNNING server over the management API by `ProgramRolodex`. That is what makes a settings change free: applying one used to mean rewriting the file and restarting the unit, and restarting the box's only resolver is a DNS outage for everything on it. `SetForwarders` is pushed unconditionally (rolodex's setter is a plain store, and there is no getter to diff against); `SetResolutionMode` is diffed first, because switching into `auto` restarts tier discovery and re-asserting it on a schedule would keep throwing away a settled tier; the blocklist goes through `ReconcileBlocklists`, which was already a readback-and-diff (one list, not two — the RBL half is retired).

**It has to run again after every rolodex restart.** rolodex seeds all of this from its config file at startup and persists nothing set over gRPC, so a restart Town OS did not order — a crash under `Restart=always`, a DHCP lease change bouncing the unit through `town-os-rolodex-config.path`, an operator restarting it by hand — silently returns the box to rolodex's defaults: resolving a different way than the operator chose, and filtering nothing. Nothing surfaces that; the settings are not read back by anything.

The restart signal is `rolodex.Manager.Generation()`: the identity (device, inode, mtime) of the gRPC socket rolodex binds at startup. rolodex unlinks and rebinds it on every start, so a new run always has a new inode, and noticing costs one `stat` — no D-Bus round trip, no unit property this codebase does not already expose, no RPC. `reconcileRolodexProgramming` runs on its own **15-second** tick and does nothing at all while the generation is unchanged; the interval is seconds rather than minutes because the gap is a box resolving and filtering differently than configured. The generation is recorded only after a fully successful pass, so a partial failure retries rather than being remembered as done.

**Fixed upstream.** rolodex used to spawn its ":53 is reachable" probe only when a blocklist was enabled *in its config file*, so with the lists no longer written there the probe never started and a configured blocklist degraded on exactly the networks the probe exists for. It is now `DnsblChecker::resolver_availability_loop`, spawned unconditionally and gated on the checker's *runtime* enabled flag — the same shape the auto-resolution recovery loop already had. A list turned on over gRPC gets the probe within seconds.

The regression tests are `TestProgramRolodex*` and `TestReconcileProgramming*` (unit), and `TestProgrammedSettingsSurviveARolodexRestart` and `TestGenerationChangesOnlyWhenRolodexRestarts` (integration) — which program a real rolodex, restart it, confirm the server itself has forgotten, and reprogram.

**The image and the file are a matched pair, and nothing negotiates between them.** `rolodex.yml` is written by `../install` and parsed by whatever rolodex image the box happens to be running, with no version handshake in either direction — serde rejects an unknown or missing field outright. A field that is required at the image's revision and absent from the file, or present in the file and unknown to the image, is a hard `failed to parse config file` at startup, and under `Restart=always` that is a crash loop with DNS down for everything on the box. It has already happened once in the harness: the published image predated the `rbl` → `dnsbl` rename, and every test that started a real rolodex died on `missing field \`rbl\`` while every mock-backed test passed. The rule that follows is that the install repo's `rolodex-config.sh` and the published rolodex image move **together**, and that a bump to `rolodex-dns/go` here is only half a bump: see [Encrypted transports](#encrypted-transports-the-box-serves-doh-through-the-ingress-dot-and-doq-direct) for the SVCB pin that needed both.

**One guard, one direction.** `TestRolodexDohBackendMatchesTheInstallScript` parses `doh.bind` out of `../install/scripts/rolodex-config.sh` and compares it to `RolodexDohBackend`, so an edit on THIS side that walks away from the install script fails here. The reverse — an edit to the install script — is caught only if somebody then runs this repo's tests, and the test **skips** where the install repo is not checked out, which includes the test container. There is no check on the install side, and no generated constant shared between the two repositories. Treat the guard as covering the direction it covers.
...
**`auto` discovers on its own, without the flag.** The tier is reached only after the roots and the encrypted upstreams have both failed, so the choice in front of it is never "roots or the local resolver" — it is "the local resolver or SERVFAIL". Leaving `DefaultForwarders` there on a network that filters `:53` to its own servers buys no privacy and guarantees the failure: those are precisely the addresses being dropped, so every query that reaches the tier pays both timeouts and dies anyway. Nothing is protected by the box refusing to notice the one resolver that answers. `Manager.forwarders` therefore takes the discovery path when the mode is `auto` and no explicit list is configured, whatever `dns_local_forwarders` says.

**Off is still the default in the modes where the trade is real, and that direction still matters.** The local resolver sees every name the household looks up, which is the thing resolving from the roots exists to avoid. That is a trade an operator makes knowingly, not one a box makes for them the first time a network misbehaves — and in `forward`, where the tier is the only upstream and takes every query always, it is exactly the trade being made. So the flag still governs `forward`; in `recursive` the list is unused either way.

**In `auto`, turning the flag off stops suppressing discovery only where there is no explicit list to fall back on.** It is easy to state this too broadly — "auto ignores the flag" is wrong. With `dns_forwarders` set the flag still decides: off keeps the operator's list, on derives one and overrides it, which is what setting the flag has always meant. The flag goes inert precisely in the case where honoring "off" would mean programming `DefaultForwarders` into a tier reached only because everything else already failed.

It has to be inert in that case, because the flag cannot express "explicitly off" separately from "never set" — it is a bool defaulting to `false`, so both are the same stored value, and honoring the first would mean never discovering for the second, which is the box the feature exists for. An operator who wants no local resolver in the path asks for `recursive`, which states that directly, or names the tier's contents with `dns_forwarders`. The regression tests are `TestAutoDiscoversWithoutTheFlag`, `TestAutoIgnoresTheFlagBeingOffWithNoExplicitList`, `TestAutoStillConsultsTheFlagAgainstAnExplicitList`, `TestAutoLeavesAnExplicitForwarderListAlone` and `TestNonAutoModesDoNotDiscoverWithoutTheFlag`.

**Discovery is now on the default code path, which makes it a test hazard.** `auto` is the default mode, so a zero-valued `rolodex.Config` takes the discovery path, and `ForwarderDiscovery` falls back to the machine's real `/run/systemd/resolve/resolv.conf` and `/proc/net/route` with a nil `Probe` meaning a live DNS query. Any test that builds a `Manager` and reaches `Forwarders` — `ProgramRolodex` does, on every call — must pin `ResolvConfPaths`, `RouteTablePath` and `ForwarderProbe`, or it silently depends on the network it runs on and passes on any box whose gateway resolves. `newProgramTestManager` fills all three in when the caller did not, and `pinRolodexDiscovery` does the same for every `rolodex.Config` the integration suite builds; the local-forwarders suite is the one place that points them at fixtures on purpose, because there what discovery finds *is* the subject. Reading the mode inside `forwarders` also put it under the manager's lock — `SetResolutionMode` wrote `cfg.ResolutionMode` unlocked back when only the rolodex.yml rendering read it, and that is a race once every settings write and every reprogramming pass resolves the forwarder list.
...
**The default gateway is a candidate too, and on a Town OS box it is usually the only one.** `GatewaysFrom` parses `/proc/net/route` (`hostRouteTablePath`) for default routes that are both up and via a gateway, decoding the native-endian hex the kernel prints — read big-endian, `192.168.122.1` becomes `1.122.168.192`, an address that points at nothing. It comes from `/proc` rather than from shelling out to `ip route` because the systemcontroller runs `--net host`, so that file *is* the host's routing table. IPv4 only: `/proc/net/ipv6_route` is a different and fiddlier format, and because every candidate is probed, the cost of the gap is a missing candidate rather than a wrong one.

The gateway is not a nicety, it is what makes this feature work at all. Three separate decisions conspire to empty every `resolv.conf` on a running box of a usable address: ttyforce writes `[DHCPv4] UseDNS=no` (and the v6 equivalent) on its networkd units so the DHCP-offered servers never become a per-link resolver outranking rolodex; `bootstrap-dns.sh` in `../install` points systemd-resolved at `DNSLoopback` whenever rolodex is up, so the uplink file reads `nameserver 127.0.0.2`; and `/etc/resolv.conf` is resolved's own `127.0.0.53` stub. All three are correctly discarded as query loops — which left file-based discovery with nothing to find on exactly the boxes it exists for. The gateway survives because it comes from the lease's *router* option, not its DNS option, so suppressing DHCP DNS does not suppress it. ttyforce reached the same conclusion for its own initrd DNS check in 0.4.7.

**A candidate is not kept until it has answered.** `ForwarderDiscovery.Validated` probes every candidate concurrently and keeps the ones that resolved `DefaultForwarderProbeName` through them, bounded by `DefaultForwarderProbeTimeout`. Discovery alone is not enough, and that is the point: a candidate list assembled from configuration is a list of addresses somebody *believes* are resolvers, and on a filtered network the belief is exactly what is wrong. `auto` reaches its forwarder tier only after the roots and the encrypted upstreams have failed, so a dead address there is not a harmless spare — it is a tier that cannot answer sitting in front of the SERVFAIL, charging every query the full per-forwarder timeout on the way past. The bar is a resolved name rather than a reachable socket, because a UDP `:53` being black-holed by a filter is indistinguishable from a working one until something waits for an answer that never comes.

Every candidate is probed rather than stopping at the first winner: the list is rolodex's whole local tier, and a second working resolver in it is the difference between one resolver's outage being invisible and being an outage. Where both a `resolv.conf` entry and the gateway answer, both are kept, `resolv.conf` first — it is what the network explicitly said to use, and the gateway is the weaker inference of the two.

**The validated list is cached for `DefaultForwarderProbeTTL`.** `ProgramRolodex` re-pushes the forwarders on every tick and `GET /dns/status` reports them on request; probing on each would pay the probe timeout on a schedule forever on a box where nothing answers, and the answer only changes when the network does. `SetLocalForwarders` drops the cache, so an operator toggling the setting is not served a stale list — and that is what gives `RefreshDNSLocalForwarders`, which deliberately does not short-circuit on an unchanged flag, something new to find. `Manager.Forwarders` therefore takes a `context.Context`: answering it can require a round of DNS queries, and a caller that has given up should not leave them running.

**Discovery that finds nothing keeps the forwarders already configured.** `Manager.forwarders()` falls back to `Config.Forwarders`, then to `DefaultForwarders`, so turning the switch on can never leave the local tier pointing at nothing — which would be strictly worse than the public defaults it was turned on to replace. Nothing *answering* lands in the same place as nothing being *found*, deliberately: to the tier being configured the two are the same situation.

### Typed forwarders

**The boundary with rolodex is written down in `../rolodex-dns/TOWNOS_CONTRACT.md`, and it is verified.** That document is the authoritative list of everything crossing between the two repositories in both directions — the gRPC surface, the forwarder grammar below, the addresses each side writes independently, and what rolodex requires back from Town OS. `make check-townos-sync` in that repository checks this tree against it and fails on drift. Two obligations it places on this side are easy to violate without noticing: **do not sort the forwarder list to express preference** (the tier is derived from the forwarder, so an ordering here is at best redundant), and **do not assume a forwarder is `ip:port`** (splitting on `:` is wrong for `tls://name@ip:853` and worse for an IPv6 literal).


A forwarder names its transport. A bare `ip:port` is plaintext UDP — what this list has always carried, so every value written before this parses unchanged — and a scheme selects anything else: `tcp://`, `tls://` (DoT), `https://` (DoH), `quic://` (DoQ). `ParseForwarder` in `src/rolodex/forwarder.go` validates a spec and reports its transport; `ValidateForwarders` checks a whole list.

The `name@ip` authority in an encrypted spec (`tls://cloudflare-dns.com@1.1.1.1:853`) carries both halves such an upstream needs — the address to dial and the name to validate its certificate against. The address must be a literal, and that is the point rather than a limitation: a forwarder named only by hostname could not be reached by the resolver that would have to resolve it. Omitting `name@` validates against the address's own IP SANs, which is a real check and not a waiver.

**This is the only way an encrypted upstream becomes configurable at runtime.** rolodex reads its `secure_upstreams:` once at startup, from the bootstrap config the install image owns, and exposes no gRPC setter for it. So on a network where only the encrypted transports work — which is the case `auto`'s tier chain exists for — the one tier that could answer was also the one tier nothing could reconfigure without restarting the box's only resolver, while the tier that *was* programmable (`SetForwarders`) could only carry plaintext addresses that such a network drops. Typing the forwarder list closes that gap: `SetForwarders` already took `repeated string`, so this needs no wire change, only a spec rolodex knows how to parse.

`dns_forwarders` (empty by default, validated by `ValidateDNSForwarders`) is the operator's list, comma-separated. Empty means `DefaultForwarders`. `Controller.RefreshDNSForwarders` pushes it to the running server, so a change costs no restart. Validation is all-or-nothing: rolodex applies a forwarder list as a replacement, so a list accepted with one entry silently dropped would leave the resolver configured with something the operator did not ask for and cannot see. `main.go` reads the setting at boot and drops an unparseable entry with a warning rather than failing the boot — this is the box's resolver, and refusing to start over one bad forwarder takes DNS down for everything on it.

Which tier a forwarder lands in is decided by rolodex, from the forwarder rather than from the list it arrived in: encrypted first, then plaintext to a private address, then plaintext to a public one. Town OS does not order the list, and should not try to — see the `auto` tier chain in `../rolodex-dns/DESIGN.md`.

**Two parsers, one contract, nothing enforcing it.** `forwarder.go` here and `src/forwarder.rs` in `../rolodex-dns` accept the same specs, and no generated code or build-time check ties them together — the two repositories cannot see each other. `src/rolodex/forwarder_test.go` deliberately pins the same cases the Rust unit tests do, so a change made on one side and not the other surfaces as a forwarder Town OS accepts and rolodex refuses. Treat that as the only guard there is.

**Tests must set `Config.ForwarderProbe`.** Left nil the probe is a real DNS query to whatever the test machine's gateway is — a suite that reaches the network, whose result depends on the network it reached, and which on any box whose gateway resolves would pass regardless of what the code under it did. `Config.RouteTablePath` and `Config.ResolvConfPaths` point discovery at fixtures for the same reason.

`main.go` reads the setting into `rolodex.Config` at boot (an unparseable stored value is read as off — the safe direction), so a box that changed networks picks the new resolver up on the next boot with no operator action. A change through `POST /settings/set` runs `Controller.RefreshDNSLocalForwarders`, which — unlike the resolution mode — does **not** short-circuit when the flag is unchanged: with it already on, the discovered addresses themselves can have moved, and re-rendering is how that reaches rolodex. The list is pushed with `SetForwarders`, whose server side is a plain store, so an identical push costs one RPC and changes nothing observable.
...
### Encrypted transports the box SERVES: DoH through the ingress, DoT and DoQ direct

Everything above is rolodex as a *client*: DoH and DoT appear in `auto`'s tier list as ways this box reaches the internet. This section is the other direction — encrypted DNS **served to clients on the LAN**, which is a different set of listeners with a different owner.

All three are opened by the install image's `rolodex.yml` (`scripts/rolodex-config.sh` in `../install`) and by nothing here. rolodex opens each one once at startup from the presence of its config section, exactly like the metrics listener, so none of them can be switched on over gRPC — see [rolodex.yml is bootstrap-only](#rolodexyml-is-bootstrap-only-settings-are-programmed-over-grpc).

**DoH is fronted by the ingress.** rolodex binds DoH on loopback at `127.0.0.2:4443` (`RolodexDohBackend`), and the ingress publishes it as an ordinary vhost at `dns.<tld>` (`DohHostLabel`), proxying that internal hop with verification skipped (`dohIngressRoute`). The certificate a client validates is therefore the ingress's leaf, issued from the box's CA under its own leaf repo (`doh`/`resolver`/`current`) so reissuing it never disturbs the pages or gfeh leaves. That is the whole point of the arrangement: rolodex terminates its own TLS with a self-signed certificate no validating DoH client accepts, and none ever has to, because it terminates a hop that never leaves the box. Clients use `https://dns.<tld>/dns-query`. The entire vhost is proxied rather than that one path, because rolodex's DoH router serves it and 404s everything else — a path matcher here would only add a second place for the path to be wrong.

`4443` rather than `443` is load-bearing. The ingress is published on `0.0.0.0:443` and rolodex runs `--net host`, so a wildcard `:443` and a specific `127.0.0.2:443` in one namespace is EADDRINUSE for whichever binds second — DNS or the ingress goes down, depending on boot order. The number is also written in two repositories that cannot read each other: here, and `doh.bind` in `../install`. `TestRolodexDohBackendIsTheLoopbackPortInstallWrites` pins this side against the literal; `TestRolodexDohBackendMatchesTheInstallScript` reads the install script itself wherever both repositories are checked out, and skips inside the test container, where `../install` is not.

**The vhost needs a name, and only the DNS reconcilers can give it one.** `dns.<tld>` has no package, no page and no object-storage partition behind it, so nothing else would ever name it: `RebuildDNS` publishes its A record (and the AAAA on a dual-stack box, because the leaf carries both host addresses as SANs), and `ReconcileDNS` carries it in the desired set so the hourly orphan sweep does not delete an hour after boot what the boot published. Without the record, the failure lands before TLS and before the ingress is reached — a plain NXDOMAIN, which reads exactly like a feature that was never built. Regression tests: `TestRebuildDNSPublishesTheDoHEndpointsName` and `TestReconcileDNSKeepsTheDoHEndpointsName` (unit), `TestDohResolvesThroughTheIngress` and `TestDohEndpointResolvesOnRealDNS` (integration).

**DoT and DoQ bypass the ingress entirely**, and that is a known gap rather than a design. The ingress publishes TCP only and its Caddy is pinned to `protocols h1 h2` (`renderCaddyfile`), so there is no layer-4 path through it for a protocol that is not HTTP. The install image binds both on the LAN at `0.0.0.0:853` — the standard pair, TCP for DoT and UDP/QUIC for DoQ (rolodex's own DoQ default is `8853`, which no client looks for). The address is `0.0.0.0` rather than a specific one because each section takes a **single** bind string: pinning the first global address would serve nothing before the first DHCP lease and nothing at all on a second address. `dns.bind` cannot do the same because `:53` collides with podman's own DNS listener; `:853` has no such conflict, at the cost of binding every interface including podman bridges and the overlay. IPv6 is not covered — one bind string cannot name both families.

**Both serve a leaf issued by the box's own CA.** The install image points `dot.tls`/`doq.tls` at `/data/tls/dot/{cert,key}.pem` and `issueRolodexTransportLeaf` writes exactly that pair on every `RebuildDNS`, with the same SANs the DoH vhost's leaf carries: `dns.<tld>` plus the box's addresses. A validating DoT or DoQ client therefore verifies against a CA the household already trusts, instead of being handed a self-signed certificate to pin.

The ordering is why this is a file rather than an RPC. rolodex starts *before* the systemcontroller — the controller cannot pull an image until something resolves names — so on a first boot the CA does not exist when rolodex reads its config. rolodex treats a named-but-absent certificate as "serve a generated one and watch for the real one", so issuing the leaf **is** the handoff: it is adopted within one 30-second poll, with no restart and nothing to coordinate. That also makes rolodex's certificate hot reload live on this box for the first time; generated material is deliberately never polled, so until a listener pointed at files, the whole mechanism was unreachable code.

The path is under `/data` because that is the only thing the rolodex container can see — its unit mounts `/town-os/rolodex:/data` and nothing else, so the leaf goes in the data directory rather than under the btrfs TLS subvolume with every other leaf. `RolodexTLSSubdir` here and `cert_path` in `../install`'s `scripts/rolodex-config.sh` are two halves of one mount written in two repos; changing either alone leaves rolodex watching a file nothing writes. Tests: `TestRolodexTransportLeafLandsInTheMountedDataDir`, `…NamesTheHostAndItsAddresses`, `…IsSignedByTheBoxCA`, `…FollowsAnAddressChange`.

**A client that does not already trust the box's CA can verify the certificate anyway**, because the leaf is pinned in the zone rolodex is authoritative for. `collectRolodexTransportTLSA` publishes DANE-EE records — `3 1 1`, the leaf's SPKI hashed — at `_853._tcp.dns.<tld>` and `_853._udp.dns.<tld>` on every `RebuildDNS`, alongside the package, page and gfeh pins. That is what makes issuing the leaf worth anything to a device the household never enrolled: it can reach this resolver, so it can fetch the pin, so it can check what it is being handed without installing a CA first.

Two records for one certificate, because a TLSA record is owned by a service **endpoint** rather than by a certificate, and DoT and DoQ share the port and the leaf while differing in transport. Publishing one and not the other is worse than publishing neither: a DANE-aware client that finds no record for the transport it picked fails closed, so the missing half reads as "this box's DoQ is broken" rather than "this box has no DANE". Expressing that at all needed `rolodex.TLSAEntry` to carry a `Proto` — it hardcoded `_tcp`, so the DoQ pin had no way to exist. Empty still means tcp, which is every HTTP-supplying package.

Nothing is published before the leaf is on disk, for the same reason `collectGfehTLSA` skips a partition whose leaf has not been issued: a pin for a certificate that does not exist yet makes every client refuse the connection once it finally does. Tests: `TestRolodexTransportTLSAPinsBothEncryptedDNSEndpoints`, `…PublishesNothingWithoutALeaf`, and `TestTLSANameProtocol` for the owner-name construction. End to end, `TestDotAdoptsTheCAIssuedLeafWithoutARestart` proves rolodex swaps to the issued leaf within a poll and with no restart — the handoff no unit test can observe, since it is about another process noticing a file — and `TestRolodexPublishesTheDoQPinUnderUDP` proves a live rolodex stores and returns the pin under `_853._udp`, which a mock client cannot.

**And it is renewed while the box is merely up.** `IssueLeaf` reissues a certificate that is inside 30 days of expiry, but only when something calls it — and the callers were boot and a confirmed internal-IP change, so renewal was a side effect of rebooting. `reconcileRolodexTransportTLSA` puts it on the hourly drift pass (`ReconcileDNS`, which now carries the CA and the btrfs base for exactly this), where the pins are reconciled in the same call: a reissued leaf carries a fresh key, so the association published for the old one stops matching the moment the file is written, and a DANE client that finds TLSA records and matches none REFUSES the connection. The new pin goes up **before** the old one comes down — rolodex adopts the renewed certificate within one 30-second poll, and both records present is the only state in which either certificate validates. The same pass republishes a pin the zone has lost (a re-initialized rolodex, a teardown that took the zone with it) without waiting for a renewal, and rewrites nothing at all while the certificate is current.

**Clients are told where all this is, twice.** `publishDDRDesignation` stores SVCB records at `_dns.resolver.arpa.` (RFC 9462) naming the DoH URL, the DoT port and the DoQ port under `dns.<tld>`, in that preference order — DoH first because `:443` survives the DPI that filters `:853`. A DDR-aware client resolves that one name against this box and needs to be told nothing else. The designation lives in `arpa.`, which rolodex never resolves upstream, and that is the point: the refusal sits below every local lookup, so only the resolver being asked can answer for its own designation.

It is removed before it is rewritten, unlike every other publisher here. `RebuildDNS` starts by tearing the TLD's zone down, which is what makes the others idempotent — but `_dns.resolver.arpa.` is in no zone this box owns, so the teardown never reaches it, and without the removal each rebuild would stack another copy or log a duplicate failure. Removing first is also what makes a TLD change take effect rather than advertise both. Tests: `TestDDRDesignationIsIdempotentAcrossRebuilds`, `TestDDRDesignationFollowsATLDChange`.

**The designation needs a rolodex that knows SVCB**, which is `v0.6.0` or newer — the record type was added there, and go.mod's `rolodex-dns/go` pin moved with it. An older server rejects the record, `publishDDRDesignation` logs that at debug and carries on, and the box serves encrypted DNS that nothing can discover: working, and invisible. That is why the round trip is asserted against a REAL rolodex (`TestDDRDesignationRoundTripsThroughRealRolodex`) rather than only against a mock, which would accept any record type it was handed.

**And once, in a form a person can read.** `GET /dns/status` carries an `encrypted` object — the DoH URL, the DoT/DoQ names and ports, and the discovery name — which the DNS screen renders read-only. The endpoints are derived from the TLD and the ingress, so there is nothing there to set; what was missing was any way to *see* them, on a box that had served DoH, DoT and DoQ for some time with nothing telling anybody where. The published DoH port is the ingress's `443`, never rolodex's internal `4443`: a client sent to `4443` would reach a loopback-only listener holding a certificate it would refuse.

**DoT has one test; DoQ has none.** `TestDotServesDNSOverTLS` starts a real rolodex with a `dot:` section and completes an RFC 7858 exchange against it — the check that the section the install image writes actually produces a listener speaking the protocol, rather than a socket that hangs. It binds loopback on an ephemeral port rather than the production `0.0.0.0:853`: under `--net host` a well-known port here is the *host's*, and two `make test-full` runs would collide on it. DoQ stays untested — a QUIC client is a dependency this repo does not have, and the same port rule applies to whatever eventually tests it.

### A TLD change has to take encrypted DNS with it

Encrypted DNS is named after the TLD in four places at once, and every one of them is a separate publisher:

1. the DoH endpoint's address records (`publishDohRecord`),
2. the DDR designation that advertises it (`publishDDRDesignation`),
3. the SANs on the certificate DoT and DoQ serve (`issueRolodexTransportLeaf`),
4. the DANE pins for that certificate (`collectRolodexTransportTLSA`).

`rolodex.ChangeTLD` moves the zone and the package records and none of these, so `POST /dns/tld` renamed a box from `home` to `lan` and left it advertising `dns.home`, serving `dns.lan` a certificate that named `dns.home`, and publishing no pin at the name a client would now dial.

**That last one is a break, not a downgrade.** A DANE-aware client that reaches an endpoint and finds no TLSA record for it refuses the connection — that is what DANE is for. So the failure is not "encrypted DNS is unverified", it is "encrypted DNS stops working", and it stays stopped until the next boot's `RebuildDNS` republishes everything. On a box that reboots monthly, that is a month.

The four are now one call, `publishEncryptedDNS`, and both callers use it: `RebuildDNS` and the TLD change. They are a single function specifically because nothing at a call site made their coupling visible — each was written as its own publisher with one caller, and the second caller forgot all four. `retireEncryptedDNS` is its inverse, for the pins: the old TLD's records go down with its zone in `ChangeTLD`, but a rename that leaves a zone standing would otherwise strand a pin for a certificate nothing serves, which is the same closed-fail from the other direction.

`POST /dns/tld` also calls `reprogramIngress` now, for the same reason: the vhost names embed the TLD, the DoH endpoint's among them, and the ingress was otherwise serving the old names until a restart.

Regression tests: `TestTLDChangeRepublishesTheEncryptedDNSEndpoints` and `TestTLDChangeLeavesNoPinAtTheOldName` (unit), `TestTLDChangeMovesTheEncryptedEndpointsOnRealDNS` (integration, against a real rolodex and its live resolver).
...
- `POST /dns/tld` (admin required) -- set the TLD. Changes the existing TLD, re-registers all installed packages, republishes the encrypted-DNS endpoints under the new name, and reprograms the ingress so its vhosts follow. See [A TLD change has to take encrypted DNS with it](#a-tld-change-has-to-take-encrypted-dns-with-it).
- `POST /dns/setup` (admin required) -- initialize DNS and register all installed packages.
- `GET /dns/dnsbl` (auth required) -- get the DNSBL (domain blocklist, forward-name) configuration: global enabled flag, provider zones with their refusal codes **resolved to what is in effect**, the list-wide `refusal_cooldown_secs`, and `rotated_out` (providers currently backed off after refusing a query, with the code and seconds remaining). See [Refusal codes](#refusal-codes-a-provider-saying-stop-asking-is-not-saying-this-is-listed).
- `POST /dns/dnsbl` (admin required) -- replace the DNSBL configuration. Accepts an enabled flag, a list-wide `refusal_cooldown_secs`, and a list of `{zone, enabled, refusal_codes, refusal_cooldown_secs}` providers. Zones are validated as fully-qualified hostnames, lowercased, trimmed, and de-duplicated; refusal codes are validated by `ValidateRefusalCodes` (IPv4 address or `address/prefix`, masked to the prefix, `"none"` only as a lone entry, no duplicates).
- `GET /dns/rbl/local` (auth required) -- list the local blocklist entries (`{name, reason}`). The path keeps its historical `rbl` spelling: it is a published contract with the UI, and the entries are unchanged -- only the RBL *provider* lookups that once shared the prefix are gone.
- `POST /dns/rbl/local/add` (admin required) -- add a local blocklist entry. Accepts a name (domain or IP) and an optional reason. The name is validated (domain or IP), lowercased, and trimmed.
- `POST /dns/rbl/local/remove` (admin required) -- remove a local blocklist entry by name.
...
DNS read-only endpoints (`/dns/status`, `/dns/records`, `/dns/rbl/local`, `/dns/dnsbl/allowlist`, `/dns/services`, `GET /dns/tld`, `GET /dns/dnsbl`) are excluded from audit logging. The allowlist *writes* are audited (exempting a name from every blocklist is an accountable change); like the blocklist writes they mirror, they carry no named action in `account.RouteActions` — the path identifies them.

### Blocklists

Rolodex (0.2.4+) provides two complementary spam/malware/ad blocking mechanisms, plus (0.4.3+) one mechanism for undoing them and one for not believing a provider that refused the query, all exposed through the DNS API and the `rolodex.Client` wrapper (`SetDnsblConfig`/`GetDnsblConfig`, `AddLocalBlocklistEntry`/`RemoveLocalBlocklistEntry`/`ListLocalBlocklistEntries`, `AddDnsblAllowlistEntry`/`RemoveDnsblAllowlistEntry`/`ListDnsblAllowlistEntries`). All are **queried on demand by rolodex** — Town OS never downloads, parses, or pre-caches blocklist feeds.

**The RBL half is retired.** Reverse-IP provider lookups, `SetRblConfig`/`GetRblConfig` and the `GET`/`POST /dns/rbl` endpoints are all gone; rolodex renamed `src/rbl.rs` to `src/dnsbl.rs` and kept only the name-based check. What survives the rename is the *local* blocklist, whose HTTP paths still read `/dns/rbl/local*` because they are a published contract, and the `dns_rbl_config` settings key, which nothing reads and which is named in `controller_dns_blocklists.go` only so it is not reused.

Note the wrapper's `SetDnsblConfig` takes the list-wide refusal cooldown as a trailing argument (`SetDnsblConfig(ctx, enabled, providers, refusalCooldownSecs)`); it maps onto upstream's `SetDnsblConfigWithRefusalCooldown`, since the arity-preserving upstream spelling exists for external API compatibility that an internal wrapper does not need.

- **DNSBL** (domain blocklist) -- domain blocklist zones queried on demand by prepending the looked-up domain to the zone (e.g. `googleadservices.com` + `dbl.spamhaus.org`). DNSBL listings take precedence over forwarded/iterative answers. Configured via `/dns/dnsbl` as a list of `{zone, enabled, refusal_codes, refusal_cooldown_secs}` providers plus a global enabled flag and a list-wide `refusal_cooldown_secs`.
- **Local blocklist entries** -- a DB-backed list of names/IPs managed manually via `/dns/rbl/local*`, checked before external providers. A **domain-name** local entry blocks forward A/AAAA lookups for that domain with `NXDOMAIN`, and takes effect immediately (rolodex updates an in-memory cache on add).
- **DNSBL allowlist** (rolodex 0.4.3+) -- the operator's escape hatch from a third-party feed's false positive, managed via `/dns/dnsbl/allowlist*`. An entry covers the name **and every name beneath it**, so allowlisting `vendor.example` also exempts `cdn.vendor.example`. It **short-circuits the whole name-based check**, beating both the configured DNSBL providers and any matching local blocklist entry, and it runs *before* the provider lookup so an exempted name never issues one. Also DB-backed with an in-memory cache, so it takes effect immediately.

  Without it the only remedy for a feed that lists a name the household needs is to disable the whole provider. Note the asymmetry with the local blocklist: an allowlist entry is a **name only**, never an IP, because the check it short-circuits is the name-based one — which is the only kind of provider lookup there is, now that the IP-based RBL half is retired.
...
**Version floor:** a rolodex predating refusal handling accepts these fields — proto3 ignores unknown fields — and stores nothing. The mocked tests cannot tell that from success, because a mock echoes back whatever it was handed. `TestRolodexDnsblRefusalCodesRoundtripReal` asserts that an **empty** configured list reads back *resolved*, which is the assertion an old image fails.

There is **no feed ingestion / pre-caching**: provider zones are the unit of configuration, and the UI offers a curated list of well-known DNSBL zones as one-click quick-adds, but the user may add any zone. Provider-zone writes replace the whole config (validated, lowercased, de-duplicated).

**The quick-add list is an endorsement, and is curated on that basis** (`DNSBL_SUGGESTIONS` in `ui/src/routes/dns/BlocklistsTab.jsx`). A zone belongs there only if a household box can use it as shipped: still operating, free, and answering a self-recursing resolver with no registration step. Currently Spamhaus DBL, SURBL, URIBL, NordSpam DBL and Spam Eating Monkey.
...
Note also that the DNSBL (domain) zones are tuned for spam URLs in email rather than ads or trackers — ad/tracker blocking would be feed territory, which is [deliberately out of scope](#blocklists). The reverse-IP zones that once complemented them are gone with the RBL half, and they barely mattered for browsing anyway: they were only consulted for IPs found in reverse DNS queries, which ordinary browsing hardly generates.
...
- **Blocklists** -- the DNSBL provider-zone section (global enable switch, per-zone enable/remove, per-zone refusal-code settings, suggested-zone quick-adds, custom-zone add — all queried on demand) plus a manual local-entry table (add/remove). Each section leads with the providers currently backed off after refusing a query, when there are any. No feeds, no apply, nothing cached.
...
When the box does not already have the Grafana image, that pull runs on its own goroutine and the unit is swapped only after it lands. Three things depend on that ordering: the settings request does not block on ~771 MB, the previous backend keeps answering `:5308` for the duration, and — because the unit is `Type=simple`, so systemd calls it started the moment `podman run` forks — the box does not report Grafana up while it is still downloading. `/monitoring/status` reports `grafana: false` for the whole window (`MonitoringUIPending`), which is what keeps the dashboard on its "starting up" screen instead of framing a port that is still serving the old backend. A concurrent second save is a no-op rather than a second pull, and a failed pull clears the flag and is logged.
...
**Scrape health is reported here too, because nothing else reports it.** `/monitoring/status` carries `scrape_targets` (Prometheus's own view of every job it scrapes, each with `job`, `instance`, `health`, `scrape_url` and — for a failing one — the `last_error` Prometheus recorded), `down_jobs` (just the names with at least one failing target, which is what the dashboard banner says), and `scrape_targets_error` (why the list is empty on a box where Prometheus is up). `monitoring.FetchScrapeTargets` reads `GET /api/v1/targets` from the loopback Prometheus, with a 3-second bound so a stalled Prometheus cannot hold a status poll open, and it is asked **only when the prometheus unit is active** — with the unit stopped, its state already says so, and a connection error on top would read as a second fault.

A target Prometheus has not scraped yet reports `unknown`, not `down`: every target is in that state for one interval after a restart, and calling it down would make every boot look like an outage.

This exists because a scrape job that has never worked is invisible from everywhere else. Both metrics failures this box has shipped were exactly that: `prometheus.yml` claimed `https` while `:5309` served cleartext, and rolodex's `metrics:` section never reached its config file. In both cases every unit was active, `systemctl --failed` was empty, and the affected panels drew an **empty chart rather than an error** — indistinguishable from a service with nothing to say. The regression tests are `TestFetchScrapeTargets*` and `TestHTTPMonitoringStatus{ReportsDownScrapeJobs,SaysWhyItCouldNotAsk,SkipsTargetsWhenPrometheusIsStopped}` (unit) and `TestMonitoringStatusReportsADownScrapeJob` (integration, against a real Prometheus with one job pointed at a dead port).
...
**The scrape target is not recomposed anywhere.** `MetricsScrapeTarget(listenAddr)` derives it from the same string the server binds and `main.go` hands the result to `monitoring.Ports.ControllerMetrics` — the same single-source-of-truth reason `PackageNetworkState.FQDN` and `Manager.MetricsAddr()` exist. A wildcard bind (`:5309`, `0.0.0.0:5309`, `[::]:5309`) is rewritten to `localhost` because a wildcard is not an address anything can connect to; an explicitly pinned host is left alone, since rewriting it would aim the scrape at an address the controller is deliberately not on. An empty result omits the job rather than aiming it at a guess.

**And the scheme is asked of the socket, not derived a second time.** `ControllerMetricsScheme` is `https` when `systemcontroller.ListenerSpeaksTLS` completes a TLS handshake against the controller's own listener, and the job then also carries `insecure_skip_verify` — the leaf is issued by the box's own CA, which Prometheus has no reason to trust and no clean way to be handed, and the scrape is loopback inside the host namespace so nothing else can answer as it.

It used to read `TOWN_OS_TLS` a second time instead, and on a deployed box the two derivations disagreed: `prometheus.yml` carried `scheme: https` while `:5309` served cleartext, so Prometheus opened a TLS connection, got a plaintext HTTP response, and **failed every scrape for the life of the config**. `insecure_skip_verify` is no help against that — it relaxes certificate validation, and the problem is that there is no handshake at all. Every one of the twenty-odd `townos_*` metrics above was exported and dropped on the floor, and the only trace was a job sitting `down` inside Prometheus's own target list, which nothing on the box surfaces. A handshake cannot disagree with the socket; it also covers `TOWN_OS_TLS_CERT`/`TOWN_OS_TLS_KEY`, which terminate TLS without going through the variable the old derivation read.

The probe is bounded at three seconds and falls back to the boot's own view when nothing accepts the connection — there is no socket to ask, and emitting no job costs the scrape just as surely as a wrong scheme. When the observed scheme and the configured one disagree, `SchemeDisagreement` reports it once on stderr: the scrape is repaired either way, but a box serving the login password in the clear after the operator asked for TLS is a security problem the repair would otherwise hide. Covered by `TestListenerSpeaksTLS*` (unit) and `TestPrometheusScrapesTheSchemeTheListenerActuallySpeaks` (integration), which runs a real Prometheus against both a cleartext and a TLS listener.
...
**The installer ships two images and the controller acquires the rest.** `install.sh` writes exactly two image references into the units it stages — `quay.io/town/town` (the systemcontroller) and `quay.io/town/rolodex` — and bakes no image content into the squashfs at all. Every other system-service image (UI, ingress, network controller, object storage) is pulled **on the box** by the controller: at boot by `parallelEnsureImages` over `coreBootImages`, and on demand by `POST /system-services/refresh`. That is why those repositories must be anonymously readable: the box carries no registry credentials, and nothing in the controller ever writes an `auth.json` or runs `podman login`. An image whose repository is private is an image this design cannot fetch, and the failure is a unit that crash-loops on `unauthorized` rather than anything that names the real cause.

**A daily timer runs the same update procedure the UI's button runs, and it ships with the installer.** `town-os-update.timer` and its service live in `../install/systemd/` and are enabled at image build time (the `ENABLE_UNITS` list in `make/install.sh`), so a box has them from first boot rather than acquiring them from a controller that must already be running. The timer fires at `04:23` and POSTs to `/system-services/refresh` — deliberately the same endpoint rather than a second implementation of "pull everything", so the scheduled path cannot drift from the manual one. The unit decides *when*; the controller decides *what*. It is `Persistent=true`, so a box that was powered off at 04:23 updates on the next boot instead of waiting another day — on a box that is off more than it is on, that catch-up is the only update it will ever get. The schedule is deliberately not the podman prune's `03:17`: a prune collecting unreferenced images while an update pulls fresh ones is a race with nothing to gain.

**The timer's POST is uncredentialed, and that is why the route is `localhostOrAdmin`.** The unit has no token to present, so `/system-services/refresh` admits unauthenticated callers **from loopback only** and still requires admin from anywhere else — the same exemption `GET /metrics` relies on, sound for the same reason: a packet with a `127.0.0.0/8` source cannot be routed to the box from the network. The generated unit therefore targets `127.0.0.1` explicitly and never the box's routable address, which would both send an uncredentialed POST across the LAN and be rejected at the far end. The port tracks the controller's `-listen` address so a relocated controller (the integration harness gives every run its own port) still updates itself.

**`auto_update_enabled` gates the timer, not the operator.** The timer marks its own call with `?scheduled=1`; only a call carrying that marker consults the setting. An admin pressing the update button always updates, because a switch labelled "update automatically" has nothing to say about an explicit request. A skipped scheduled run answers `200` with `{"status":"skipped"}` rather than an error — the timer asked whether to update and got a valid answer of "not now", which is not a failure for `systemctl status` to show in red. The timer stays installed and running while the setting is off, so flipping it takes effect on the next fire with no unit surgery and no way for unit state and setting to disagree.

The setting defaults to **on**, and unrecognised values read as on. Off is a closed list (`0`, `false`, `off`, `no`, case-insensitive); everything else — including a typo and an unreadable settings row — leaves updates running. The asymmetry is deliberate: because the installer ships only two images, a box that stops pulling is a box that never acquires most of its services, so the cost of guessing "off" wrongly is far higher than the cost of one extra pull.
...
Startup failures for monitoring, Rolodex config, core image pulls, the TLS CA, the ingress, the pages service, object storage, network reconcile, and the UI container are non-fatal; the system continues without them. All container image pulls use the `ensureImage` helper (`systemcontroller.EnsureImage`) described below. Pull failures for non-essential services are logged to stderr and do not prevent startup, allowing the system to boot even when the network is temporarily unavailable.

**Boot refreshes floating image tags** — `EnsureImage` classifies a reference with `FloatingImageRef` before deciding what to do with it:

- **Floating** — the `latest` family in any spelling: `latest`, `rc.latest`, `release.latest`, each with or without the `-x86_64` / `-aarch64` suffix, plus a reference carrying no tag at all (podman resolves it to `:latest`). These are re-pulled on **every** boot, even when the image is already in the local store. Present and current are different questions for a tag that moves, and `podman pull` on an unchanged tag is a manifest check rather than a download.
- **Pinned** — dated tags (`rc.<date>-<arch>`, `release.<date>-<arch>`), digest references, per-instance test tags, and anything under `localhost/` (which has no registry to refresh from). Fetched only when missing.

A failed refresh of an image that *is* present locally is not an error: the local copy is used and the failure is logged, so a box still boots with no registry — the same reason its own unit runs `--pull=missing` rather than `--pull=always`.

**The controller updates itself** — `SelfUpdate` runs as the first step of `boot_services`, and is the other half of the same problem: `EnsureImage` keeps every service a box runs current, but it cannot do anything about the process executing it. The controller's own unit is `--pull=missing` on purpose (a crash-restart must not depend on the registry being up), so the running controller stayed on the image the box was installed with until an operator pressed *refresh system services* — a box that updates everything except the thing doing the updating is the one component that can never ship a fix for itself.

The step:

1. Asks podman which image reference *this* container was started from (`{{.ImageName}}`) rather than deriving it from `TOWN_OS_TAG`. The install build system substitutes whatever `CONTROLLER_IMAGE` it was given into the unit — possibly another registry entirely — and refreshing a reference the unit does not actually run would compare two unrelated images and restart the box on every boot.
2. Does nothing when that reference is pinned (an operator pinning the box, or a `localhost/` image under `make dev`).
3. `EnsureImage`s it, then compares the id the tag names now against the id this process is running.
4. When they differ, writes the target id to `<btrfsPath>/town-os-self-update` and asks systemd to restart `town-os-systemcontroller.service`. The unit's `--pull=missing` then starts the image the refresh just placed locally. The boot waits rather than continuing into reconcile with work that is about to be discarded.

The marker is the loop guard: if a restart comes back running the same old image — a unit that resolves the tag differently than we did — the box tries once and then logs and moves on, instead of restarting on every boot forever. A tag that moves again is a new target and gets its own attempt, and a restart systemd refuses clears the marker, because a restart that never ran must not suppress the next one.

The consequence for an existing box: the controller it is running has to contain this code before it can use it, so exactly one manual *refresh system services* (or a reinstall) is still needed to cross over. After that a reboot is enough.

`TOWN_OS_IMAGE_REFRESH=0` (also `false`/`no`/`off`) turns the refresh off — including the self-update, which then finds nothing moved — so the pinned rule applies to everything; a missing image is still fetched. `make dev` and both test containers set it, because they load every image they run from the local image cache on purpose — a refresh there would reach for quay.io on every start to replace an image that was placed deliberately. Any unrecognised value leaves the refresh on: a box that quietly stops acquiring its own services is a worse failure than one extra pull. Setting it on the container is only half of the job: PID 1 in those containers is systemd, so the harness unit's `PassEnvironment` allowlist (`integration/testdata/town-os-systemcontroller.service`) has to name the variable as well, or it stops at PID 1 and the controller — the one process it was set for — never sees it. It did not name it, so the refresh ran on every test and dev boot: rolodex, Prometheus and node-exporter re-pulled from quay.io inside the boot pull set while the harness gave `/status/ping` 120 seconds to answer, and whether a run failed depended on the uplink. `TestHarnessEnvironmentReachesTheController` now derives the required names from `make/test.sh`, `make/dev.sh` and `system_port_env`, and the integration suite reads the running controller's `/proc/<pid>/environ` to prove they arrived.

The distinction matters because the installer ships no images and every box's tag floats by default (`rc.latest-<arch>`). Without it, "already loaded" meant "up to date", and every system service stayed on the image the box downloaded the first time it booted — a reinstalled VM kept serving a months-old UI no matter how many times it was rebooted, and the only thing that ever advanced it was an operator's `POST /system-services/refresh`.
...
That is also why `make dev` is the one target that can be *stopped* by a port: there is a single `127.0.0.2:53` on a machine, and more than one thing wants it — the install repo's host rolodex, another checkout's dev box, an orphaned system-service container from a dev run that was killed. `require_free_rolodex_dns` (`make/dns.sh`) therefore refuses to start once this checkout's own container is removed and the address is still held, naming what to stop. The redirect used to wait on a bare TCP connect to that address, which succeeds just as well when the answer is somebody else's resolver: this checkout's rolodex would fail to bind, and the host would be pointed at a rolodex that knows nothing about this dev box — indistinguishable, from the browser, from the dev box's own DNS being broken. `wait_for_rolodex_dns` now requires both halves, the unit being active (which is what makes the resolver *ours*) and the address being bound (which is what makes it usable, since the unit goes active the moment `podman run` is up, before rolodex has opened a socket), and it is **fatal** rather than a warning — a dev box whose resolver never came up is not a dev box, and carrying on produced one that looked started while every `.home` name on the host went to the LAN resolver. The redirect happens as soon as the controller is ready, before the account and the UI server, because everything after that point is a dev box that is already up. Covered by `TestDevDNSRefusesToStartWhenTheRolodexAddressIsTaken` and the `TestDevDNSWait*` tests, which stub `ss`, `resolvectl` and `${SUDO}` — no test may touch the host's resolver.

Which leaves the question of who renders the `rolodex.yml` a dev box binds from. Nobody did, and that is the bug the fatal wait above found: rolodex opens its DNS listener **once**, at startup, from that file, and nothing Town OS can say over gRPC will open one — `ProgramRolodex` owns the forwarders, the resolution mode and both blocklists, and deliberately not the binds (`controller_rolodex_config.go`). On a real box the install image renders it (`../install/scripts/rolodex-config.sh`, its only writer). `make dev` runs no installer, so `${STATE_DIR}/dev-rolodex` was empty and `town-os-system--rolodex.service` — whose `ExecStartPre` creates the *directory* — started rolodex `--config /data/rolodex.yml` against a file that had never existed. The unit went active regardless, because the container starts fine and only the listener is missing, which is exactly why the wait asks about the address and not just the unit. `seed_dev_rolodex_config` (`make/dns.sh`) now writes it on the host before the container mounts the directory: loopback only — the DNS binds, the metrics listener, the gRPC socket the controller dials, a starting forwarder pair — and rewritten every run, because `dev-rolodex` persists to hold `rolodex.db` and a config from an older schema would outlive the checkout that wrote it. It is seeded on the host rather than baked into the image on purpose: the same unit file ships in the integration test image, and a `rolodex.yml` binding `127.0.0.2:53` inside a `--net host` test container would bind the **host's** `:53` on every run. Absent this, each boot also paid two unconditional thirty-second waits — `Manager.WaitForDNSReady`, then the gRPC socket dial loop in `reconcileDNSAndNetworks` — inside the 120s budget `require_controller_ready` allows for a boot that answers `/status/ping` with 503 until it is wholly finished.

The same boot is what decides what `make dev` has to put in the container before starting it. Every generated unit runs `podman run` **without** `--pull=never`, so an image that is not in the inner store is not an error that fails fast — it is a registry pull happening inside `podman run` while `require_controller_ready`'s 120s and the service's own readiness wait both count down. The ingress and the pages service's caddy were both missing from that list (caddy from the integration harness too), so a dev box fetched them from quay and Docker Hub during boot with the tars already sitting in the checkout's image cache. Both are loaded now, the ingress built by an `ingress-image` prerequisite. Two things were being loaded that should not have been: **Grafana** is ~1.17 GB and is the monitoring backend that is *not* the default, so boot never runs it unless the stored backend is `grafana` — it is skipped unless `DEV_LOAD_GRAFANA=1`; and **gfeh** is no longer a prerequisite of `dev`, because it is the one local image whose content comes from outside the repo and so recompiles the whole `gfehd` dependency tree once per calendar day, for everyone, used or not. `make gfeh-image` enables object storage in dev exactly as `make/dev.sh` has always claimed. Loading itself goes through the image cache mounted read-only at `/image-cache` rather than `podman cp`, which removes one full write of every tar; a container without the mount still falls back to copying. Finally the systemcontroller is **no longer enabled** in the dev image: systemd started it the instant `podman run -d` returned, against an empty store, which bought a guaranteed bad boot — the full 30s DNS wait, pulls for everything not yet loaded, units started against absent images, every package repository cloned — all discarded by the start `make/dev.sh` issues once loading is done, and all of it competing with that loading. rolodex stays enabled, because it has to be serving before the controller boots and its unit is built to survive the wait.
...
set to empty, push it" ([Blocklists](#blocklists)).

| Key | Default when absent | Written by |
| --- | --- | --- |
| `monitoring_backend`     | `uplot` | `POST /settings/set` |
| `dns_dnsbl_config` | unconfigured (not the same as empty) | `POST /dns/dnsbl` |
