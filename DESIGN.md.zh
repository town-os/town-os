...
> **本文件是 [DESIGN.md](DESIGN.md) 的中文译本。英文原件为准。**
...
- `POST /networks/peers/add`（`requirePeerEnroll`：管理员或 `wireguard` 授权，且限定在调用者的网络内）—— 登记一个 peer。当 `public_key` 为空时，服务器生成密钥对并返回私钥以及一份可直接导入的设备配置。接受可选的 `endpoint` 与一个 `rolodex` 标志。
