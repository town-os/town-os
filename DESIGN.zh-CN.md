...
26. **编排 ingress** **(非致命)** —— 等待就绪，拨号其 gRPC socket，`RebuildIngress` 以声明式方式推送完整路由集（HTTP 包 + pages + 对象存储的视图与索引），与 `RebuildDNS` 是同一模型。它还会在同一遍中，从构建这些路由所用的同一站点集合渲染分区生成的两个页面——它的[分区索引页](#分区索引页)与[已发布文件索引页](#已发布文件索引页)——因为路由不能在它所服务的字节存在之前就被编排。
...
索引页**只**承载视图，而它们本来就在 DNS 中。不含主体、授权或配额，也不含已发布的链接——这是运维者拿来向别人解释"对象存储是什么"的那一页，而其上的 `/f/<token>` 是一份没人要求发布在那里的持有即用凭据。已发布的链接有它们自己的页面，见下文。

### 已发布文件索引页

`http.gfeh.<tld>` 是这台机器上唯一一个存在意义全在于把文件交给别人的名称，而它也恰恰是那个返回 404 的名称：gfehd 只应答 `/f/<token>`，别的一概不应答，包括它自己的根。它的根现在提供一个**已发布文件索引页**——每一个启用的暴露，带名称，并链接到它所服务的那个令牌。

这是机器上唯一一个拥有**两个后端**的名称，而 ingress `Route` 上的 `PathBackend` 正是为此而存在（见 [Ingress](#ingress)）：

- **`/` 由 pages 容器处理**，从 `gfeh-index/http.gfeh.<tld>/` 渲染 `gfeh.RenderPublicIndex`，走的是与分区索引页完全相同的目录检查、仅在变更时写入、符号链接与剪枝（`writeGfehIndexFile`）。两个写入者就是两种关于索引住在哪里、以及剪枝可以删除哪些名称的意见。
- **其他每一条路径仍归 gfehd。** 匹配器就是根，不多一分：前缀匹配会遮蔽 gfehd 日后可能长出的路径，而那正是这一页所修复的失败的反面。根是 gfehd 通过返回 404 明确说过它不提供服务的那一条路径。
- **只有字节确实存在的名称才会被编排路径后端。** `reconcileGfehPublicIndexes` 返回它写下的内容，`gfehPublicIndexBackends` 只为这些名称布置路由；把一个没有 webroot 条目的名称的 `/` 送往 pages 容器，只是把 gfehd 的 404 换成 Caddy 的 404——同一个失败，换了个标志而已。
- **`gfehIndexHostnames` 也覆盖这个名称**，否则 pages 的剪枝会在第一遍就删掉它的 webroot 条目，索引页将一直 404 到 ingress 下一次重建。
- **联系不上的守护进程保留自己的页面。** `ListExposures` 失败时，上一次的渲染结果原样留下，而不是被改写成"尚未发布任何内容"：短暂的中断并不能证明有人撤回了全部文件，而那样处理的 reconcile 会在每次重启时清空索引，再在下一遍里把它填回去。

**它刻意枚举持有即用的凭据。** 一个 `/f/<token>` URL 就是一份 capability——持有它即是授权的全部——因此发布一个文件不再意味着"我发过链接的人可以拿到"，而是意味着"任何能解析这个名称的人都会看到它被列出"。这就是这一页所做的交易：一份已发布文件的目录是值得拥有的，而一份省略了链接的目录只是一串谁都打不开的文件名。由此得出的是，**撤回是让文件重新变回私有的唯一手段**；被禁用的暴露不贡献任何一行，因为那一行会去宣传一个解析不到任何东西的链接。
...
这两种失败都逃过了测试套件，但原因各不相同。**单元**测试套件用一个假的 gfehd 顶替，根本不会运行真正的守护进程。**集成与 UI** 测试套件确实会运行真的——`make/test.sh` 会把构建好的镜像加载进这两个容器，因为没有任何替身能够证明一个分区确实启动、确实应答其管理套接字、确实执行自身的配额上限——但一个仅仅是*陈旧*的守护进程照样能构建、推送、安装并启动。它只会表现为真实机器上对象存储永远起不来。为存档起见，这也正是当初存在下限的原因：0.1.1 无法解析任何带有 SMB 用户的分区配置（gfehd 的每个配置结构体都是 `#[serde(deny_unknown_fields)]`，而它的 `SmbConfig` 没有 `users` 字段），并且它一启动就去认证，而那时在启动过程中回应的是 `:5309` 存根，除 ping 之外一律 403。

**两种构建都以各自的方式击破层缓存。** `cargo install gfehd` 在每次构建时都是逐字节相同的一行 `RUN`，因此它那一层是永久的缓存命中——否则，一个全部契约就是"采用今天 crates.io 上的版本"的构建，会永远提供第一次构建时的那个 crate，悄无声息，而且每次的日志都干干净净。也不存在更廉价的缓存键：想知道 crate 何时变化就得去问 crates.io，而那正是这次构建要做的事。

- `release-gfeh` 传入 **`--no-cache`**。对于要发布出去的东西，任何更弱的做法都不可接受。
- `gfeh-local` 传入按天粒度的 **`GFEH_CACHE_DATE`** 构建参数。该夹具是每一次集成与 dev 运行的前置条件，因此在这里用 `--no-cache` 会让每一次运行都重新编译整个 Rust 依赖树；但纯粹的缓存命中又会把它冻结在该机器上首次构建时的那个 gfehd——而集成与 UI 测试套件会针对它启动真实的分区。按天是折中：当天之内是缓存命中，跨日则重新构建。

无论哪种方式，cargo 的 registry 卷都会留存，因此依赖树会被重新编译，但不会被重新下载。这是一条通用构建规则的具体体现（见 [CLAUDE.md](CLAUDE.md)）：从仓库源码构建的本地镜像不会漂移，因为源码变更会使其缓存失效；而内容在构建时抓取的镜像则需要显式的缓存失效机制。
...
- **Links 标签页渲染的是完整 URL，且由服务端组装。** `GfehExposureView.URL` 由 `gfehPublishedLinkBase` 构建——`https://<http-view-fqdn>/f/`——它来自为 ingress vhost 与叶子证书 SAN 命名的同一个收集器，因此已发布的链接在构造上就是 ingress 会路由、证书也覆盖的名称。它不在浏览器里组装，是因为 UI 将不得不知道四件服务器早已掌握的事实：提供服务的名称是 *http 视图的*，而不是分区的或本机的；它是在该分区自己网络的 TLD 之下限定的，而非全局 TLD；路由是 `/f/<token>`；以及上报的端口绝不能出现。当分区不提供任何 HTTP 视图时该字段为空——这是诚实的答案，因为那时确实没有任何东西在服务那个令牌——而被禁用的暴露渲染为纯文本，而不是一个可点击的 404。这个标签页里每一个启用的链接，也都公开列在 http 视图的根上（[已发布文件索引页](#已发布文件索引页)），因此让一个文件不再被列出的是撤回，而不仅仅是不再分享——而且标签页会在表格上方明说这一点（`objects.links_public_notice`），因为这是唯一展示这些链接的界面，也就唯有它能在有人以为链接只属于收件人、从而又发布一个之前把话说清楚。该提示仅在链接**既启用又确实在被提供**时出现，判据与那一列相同，都是 `URL` 字段：没有 HTTP 视图的分区没有可供索引页落脚的根，此时的警告将描述一件并未发生的暴露。
...
### 一个 vhost，两个后端：`PathBackend`

一条路由通常只指名一个服务，机器上几乎每条路由都是如此。`Route.path_backends` 是那个例外：一组 `(路径, 后端)` 对，渲染成若干 Caddy `handle` 块，排在承载路由自身后端的那个裸 `handle` 之前。它之所以存在，是因为一个后端可以拥有一个名称，却并不拥有该名称之下的每一条路径——它唯一的调用者是[已发布文件索引页](#已发布文件索引页)，它从 pages 容器提供 `/`，而 `http.gfeh.<tld>` 上其他每一条路径都抵达 gfehd。

- **裸 `handle` 永远在最后。** `handle` 块互斥，并按书写顺序求值，而裸的那个匹配一切，因此排在它之后的任何东西都是死配置。
- **路径后端按给定顺序渲染**，不排序。排序对 Town OS 实际编排的那些匹配器是安全的（它们并不重叠），但对确实重叠的一对，它会悄悄改变谁胜出。
- **路径像主机名和后端一样被校验**（`validPathMatcher`：绝对路径、URL 非保留字符外加 `*`、长度有上限）。坏的那个被**单独**丢弃，而不是连同它的路由一起：路由自身的后端是兜底，因此丢掉一个路径后端只损失它所主张的那一条路径，而这个名称背后的服务照常应答。重复的路径同样被丢弃——Caddy 接受两个相同的匹配器，只是永远到不了第二个。
- **路径后端没有自己的 `backend_tls`**，一律以明文 HTTP 代理。那个标志描述的是一条路由*为哪个*服务而设，而这个字段唯一要抵达的东西，和其他每一个 page 一样在 `:80` 上讲 HTTP。
...
位于某条已编排路由背后的后端有两种坏法，而 ingress 用同一个页面回应两者：一个**说明服务不可用、并每五秒自行重新加载的 503**（`unavailableRefreshSeconds`，同时也是 `Retry-After` 的取值），直到服务重新应答为止。期间不重新编排任何东西，也不撤下任何路由——地址自始至终保持可路由，因此后端一旦重新有了监听器，下一次重新加载就会把它端上来。

对一台家用机器而言这是常态，不是例外：包在升级时重启，一次设置变更把服务弹一下，一次重启让二十个容器按某种顺序回来。Caddy 在那几秒里说的是 `502 Bad Gateway`，读起来像*坏了*而不是*正在回来*，除了手动重试之外没给读者留下任何可做的事。

这两种失败形态在 Caddy 里是两套彼此独立的机制，只覆盖其中一种的实现会漏掉常见的那种：

- **后端应答了，返回 5xx。** 那是一次上游*响应*，因此用响应匹配器加 `handle_response` 在 `reverse_proxy` 块**内部**拦截——这也是渲染器如今发出的每一个代理都要开一个块的原因，纯 HTTP 的那些也不例外。
- **后端根本没有应答**——容器停了、名字解析不了、连接被拒。`reverse_proxy` 失败，Caddy 抛出它自己的错误，而 `handle_response` 永远看不到它；那属于**站点块**层面的 `handle_errors`。在家用机器上这才是常见的那一种，也正是只处理 5xx 的实现会静悄悄漏掉的那一种，因为它守候的那个 5xx 从来不会到来。

两条路径都渲染为 Caddyfile **片段（snippet）**（`render_unavailable.go`），每份文件定义一次，由每个站点块导入——因此页面不会在各个 vhost 之间漂移，而无论这台机器服务多少条路由，文件都保持在可读的体量。即使路由集为空也照样发出这些片段：没人导入的片段永远不会被解析成指令，而一段忽有忽无的前言会在第一条路由到来时白白弹一次 Caddy。

- **只有浏览器才会拿到这个页面。** 两条路径都以 `method GET HEAD` 加 `Accept: *text/html*` 为闸门。API 客户端——XHR、webhook、`curl`、路径后端背后的 DoH 解析器——在响应路径上拿到上游自己的状态码、正文与响应头，在错误路径上拿到一个带 `Retry-After` 的朴素 503。把 HTML 递给一个正在解析 JSON 的东西，只会把一个坏掉的后端变成它调用方里第二个、更古怪的故障。

  这条透传只用 `copy_response`，**不**在旁边写 `copy_response_headers`，而这个「不写」是有承重作用的：等到响应处理器运行时，`reverse_proxy` 早已把上游的响应头放到了 response writer 上，因此再复制一遍会让每一个响应头都出现两次——其中包括 `Content-Length`，那已经不是不整洁，而是一条格式非法的报文。它看上去正像是 `copy_response` 理所当然的搭档，所以渲染测试和集成断言都各钉了一道。
- **页面上的服务名来自渲染出的配置，绝不来自请求。** 这个标签是导入时代入的片段参数——路由的主机名，且已经过 `validSiteHost` 校验。`:80` 兜底块改用常量 `Town OS` 标注，因为它应答*每一个*未匹配的主机：那里的 `Host` 头是客户端随手写的任何东西，把它反射进 HTML，就是在唯一一个接受任意名字的 vhost 上开了个注入口。`safeUnavailableLabel` 是让这句话对将来的调用方依然成立的那道守卫。
- **包的 `:80` 跳转块两者都不加**，因为它不代理任何东西——那里没有会不可用的后端。

#### 这个页面是被翻译过的

页面上的四句话是词条（`src/i18n` 中的 `ingress.unavailable.*`），而不是渲染器里的字面量——Town OS 提供的每一种语言都会得到这个页面的一份渲染，Caddy 则按每次请求的 `Accept-Language` 挑一份出来。

这与 UI 的做法一致，理由也相同：**读者的语言是浏览器的语言，不是这台机器的语言。** `I18nContext` 先取已保存的选择，再取 `navigator.languages`，只有当浏览器要的语言没有任何词表覆盖时，才退回服务端的全局 `locale` 设置。重试页面在 Caddy 里解析同样的顺序——一个针对 `Accept-Language` 的 `map`，每种语言词表一行，外加一行携带这台机器 locale 的 `default`。手机是葡萄牙语的客人，在一台配置为德语的机器上读到的是葡萄牙语。

- **用 `map`，页面只写一份——而不是每种语言一个 `handle` 块。** 块的写法能用，而且被实测过：因为 `import` 是文本展开，每一次展开都会把整个选择器内联进去，于是三十个分支就意味着 Caddy 每次重载所接收的 JSON 里有三十份页面副本——**单条路由就是 115 KB**，一台有二十条路由的机器，在每次装包、改页面和每小时的 reconcile 时都要推送约 2.3 MB。改用 map 后，翻译好的句子放在一张紧凑的表里，页面只写一次，从表里读出 `{townos_title}` 之类的值。
- **5xx 那条路径什么都不渲染；它抛出 `error 503`**，交给本站点自己的 `handle_errors` —— 这正是让页面按站点块展开一次、而不是按每个 proxy 展开一次的原因。已验证：在 `handle_response` 内部抛出的 `error` 确实会到达外层的错误处理器，且原请求保持不变，因此 `Accept` 那道闸门依然生效。
- **选择器是按语言一行，而不是按 locale。** 国家变体（`de-AT`、`en-GB`、`es-MX`……）派生自它们的基础语言，只覆盖那个国家确实说得不一样的字符串——这四句都不在其中——所以一个变体一行，只会多出三十条渲染同一页面的正则。`TestUnavailableLocalesCoverEveryCatalog` 从 `i18n.PopulatedLocales()` 推导出应有的成员集合，并在两个方向上失败，因此新加入词表的语言不会在这里悄悄停留在英文。
- **繁体中文排在通用 `zh` 之前。** Caddy 按顺序测试 map 的每一行并取第一个命中，而 `^zh` 会把 `zh-TW` 一并吃掉，所以顺序写反就会把简体送给台湾、香港和澳门——对读不懂的人来说，那看上去正是一份能用的翻译。每条模式都做了锚定、忽略大小写，并且在分隔符或整串结束处收尾，因此 `^ja` 不会用日语去回答爪哇语。
- **服务名是格式动词，不是字符串拼接。** `"%s is unavailable"` 在日语里把名字放在与英语不同的位置，所以词序归词条所有；渲染器把 `{args[0]}` 代入翻译后的句子，而站点块导入该片段时，Caddy 再把主机名代入其中。
- **这台机器的 locale 通过单元的命令行到达 ingress**（`--locale`、`ingressctl.Config.Locale`，在生成单元时读取 `locale` 设置）。它是一项全机器的设置而非路由状态，所以它随单元走而不是随 gRPC 推送走；语言一改，它就以其他任何单元变更相同的方式到达 ingress——下一次 reconcile 渲染出不同的单元。没有词表的代码会被忽略而不是被拒绝：这个设置是一行自由文本，写错的代价应当是一个英文页面，而不是一个起不来的 ingress。
- **每一条翻译串都为它跨越的两个边界做转义**（`caddyHTMLText`）：它先是一个 Caddyfile 记号，然后是一个 HTML 元素的内容。HTML 转义顺带去掉了会提前结束记号的引号；反斜杠单独转义，因为 Caddy 把它读作转义符，而 Go 的 `html` 包没有理由去动它。

### ingress 的指标

ingress 在**属于它自己的第三个监听器**上提供 Prometheus 文本展示格式（`ingress.MetricsPath = "/metrics"`，`ingress.DefaultMetricsPort = 9146`），由 `src/metrics` 渲染，方式与控制器的端点完全相同。它不能搭在已有的那两个监听器上：那两个是这台机器的正门，一个 `/metrics` 的 vhost 会让局域网与 overlay 上的任何人都能通过 SNI 访问到它。

**它只发布在 `127.0.0.1` 上**（`-p 127.0.0.1:9146:9146`），这是"ingress 与网络接口无关"这条规则唯一刻意的例外。这份抓取列出了这台机器所服务的每一个主机名，并标出其中哪些还没有证书——一张"什么已发布、什么只完成了一半"的地图——而它前面没有任何认证。Prometheus 以 `--net host` 运行，因此它经回环直达该发布端口，无需 podman 网络跳转，与 node-exporter 和控制器的目标完全一样。

**抓取目标不在任何地方被重新拼装。** `ingressctl.MetricsAddrFor(port)` 同时构建 unit 的 `-p` 规格与 `monitoring.Ports.IngressMetrics`，与 `rolodex.Manager.MetricsAddr()` 是同一种单一事实来源的安排。当 `INGRESS_IMAGE` 为空时，该 job 被**省略**：一台没有 ingress 的机器应当报告"没有 ingress"，而不是一个读起来永久宕机的路由器。

`ingress` job 导出的内容：

| 指标 | 类型 | 说明 |
|---|---|---|
| `townos_ingress_up` | gauge | 只要在服务就恒为 1；不在服务时整个消失 |
| `townos_ingress_start_time_seconds` | gauge | 运行时长是 `time() - 本值`，用抓取端的时钟 |
| `townos_ingress_caddy_up` | gauge | 本次抓取中 caddy 子进程的 admin API 有应答时为 1 |
| `townos_ingress_routes{tls}` | gauge | `local`/`acme`/`pending`——`pending` 表示已编排但还没有叶子证书 |
| `townos_ingress_route_https_ready{hostname}` | gauge | 每条路由 1/0，让运维看到*哪个*名字无法提供 HTTPS |
| `townos_ingress_path_backends` | gauge | 所有路由上按路径划分的后端总数 |
...
- **`townos_ingress_dropped_total` 正是 `renderCaddyfileTally` 存在的理由。** 主机名畸形、后端不可用或路径匹配器不合法的路由会被**单独**丢弃而不是让整次渲染失败——这意味着它经 gRPC 被接受、被记一行日志，然后就再也没有被服务过。日志是错误的仪器：在已经知道出事之前，没有人会去读 ingress 的 journal。
- **路径后端按路由验证一次，而不是按 vhost 验证一次。** 一条 page 路由既 HTTPS 就绪又是 `ServeHttp`，会从同一份路径集合渲染出两个站点块；在写入器内部验证会把每一次拒绝都记两遍。
- **每个 counter 序列都以零播种。** `metrics.Render` 会跳过没有样本的指标族，因此未播种的 `townos_ingress_reloads_total{result="failure"}` 在第一次失败之前根本不存在——那样的告警要等它所监视的事件已经发生过一次之后才可能触发。
- **`townos_ingress_last_reload_success_time_seconds` 在第一次重载成功之前被省略**，而不是发成 0：`time() - 0` 读起来是一份 56 年前的配置，而不是"还没有配置"。
- **一次抓取绝不整体失败**，也绝不拖垮路由器。够不着的 caddy 子进程只让一个 gauge 变成 0，而不是让整个响应消失；绑定不上的指标监听器会被记录并被承受下来，因为为了一个监控缺口而拒绝转发流量是错误的取舍。

目前还没有仪表盘渲染这些序列——不像 `rolodex` 与 `systemcontroller` 两个 job，它们的面板集是对着 `RolodexDashboardMetrics()`/`ControllerDashboardMetrics()` 声明的。这些序列已被采集、可被查询；面板集是另一次改动。
...
**`auto` 会自行发现，不需要这个开关。** 只有在根与加密上游都已失败*之后*才会到达这一层，所以摆在它面前的选择从来不是「根，还是本地解析器」，而是「本地解析器，还是 SERVFAIL」。在一个把 `:53` 过滤到只剩自家服务器的网络上，把 `DefaultForwarders` 留在那里既买不到隐私，又坐实了失败：那些恰恰就是被丢弃的地址，因此每一个到达该层的查询都要付掉两次超时，然后照样死掉。机器拒绝去注意那唯一会应答的解析器，并没有保护到任何东西。所以当模式为 `auto` 且没有配置显式列表时，`Manager.forwarders` 就会走发现这条路——无论 `dns_local_forwarders` 怎么说。

**在权衡真实存在的那些模式里，默认仍然是关闭，而那个方向仍然要紧。** 本地解析器会看到这个家庭查询的每一个名称，而那正是从根解析所要避免的事情。这是运维在知情下做出的权衡，而不是机器在网络第一次出问题时替他做的决定——而在 `forward` 中，这一层是唯一上游、永远承接每一个查询，那正是实际正在做的那笔权衡。所以这个开关仍然管着 `forward`；在 `recursive` 中，那份列表反正也不会被用到。

**在 `auto` 中，关掉这个开关不再抑制发现，仅限于没有显式列表可退守的场合。** 这一点很容易说得过宽——「auto 会忽略这个开关」是错的。当 `dns_forwarders` 已设置时，开关仍然说了算：关，就保留运维的那份列表；开，就导出一份并覆盖它——而这正是打开这个开关一直以来的含义。开关变得不起作用的，恰恰只是那种情形：尊重「关」将意味着把 `DefaultForwarders` 编程进一个仅仅因为其他一切都已失败才会到达的层。

在那种情形下它只能不起作用，因为这个开关无法把「刻意关掉」与「从未设置过」区分表达——它是一个默认为 `false` 的布尔值，两者是同一个存储值，而尊重前者就意味着永远不为后者去发现，而后者恰恰就是这个功能存在的理由所在的那类机器。不希望路径中出现本地解析器的运维，应当选择直接表达该诉求的 `recursive`，或者用 `dns_forwarders` 指名这一层的内容。回归测试是 `TestAutoDiscoversWithoutTheFlag`、`TestAutoIgnoresTheFlagBeingOffWithNoExplicitList`、`TestAutoStillConsultsTheFlagAgainstAnExplicitList`、`TestAutoLeavesAnExplicitForwarderListAlone` 与 `TestNonAutoModesDoNotDiscoverWithoutTheFlag`。

**发现如今位于默认代码路径上，这让它成了测试的隐患。** `auto` 是默认模式，所以一个零值的 `rolodex.Config` 也会走上发现这条路，而 `ForwarderDiscovery` 会回退到机器真实的 `/run/systemd/resolve/resolv.conf` 与 `/proc/net/route`，且 `Probe` 为 nil 意味着一次真实的 DNS 查询。任何构建 `Manager` 并触及 `Forwarders` 的测试——`ProgramRolodex` 每次调用都会触及——都必须钉住 `ResolvConfPaths`、`RouteTablePath` 与 `ForwarderProbe`，否则它就悄悄依赖于自己运行所在的网络，并且在任何网关能解析的机器上都会通过。当调用方没有填时，`newProgramTestManager` 会把这三项补齐；而集成测试套件构建的每一个 `rolodex.Config`，都由 `pinRolodexDiscovery` 做同样的事。唯一有意把它们指向固定装置的地方是本地转发器那一套测试，因为在那里，发现所找到的东西*正是*被测的主题。把模式的读取放进 `forwarders`，也把它纳入了管理器的锁之下——当年只有 rolodex.yml 的渲染会读它，所以 `SetResolutionMode` 写 `cfg.ResolutionMode` 时没有上锁；可一旦每次设置写入、每一轮重新编程都要解析转发器列表，那就是一处竞态了。
...
有四类测试把两侧绑在一起，因为再没有别的东西连接它们：

- `TestRolodexDashboardMirroredInFrontendQueries` 与 `TestControllerDashboardMirroredInFrontendQueries` 从 Go 测试中读取 `ui/src/components/monitoring/queries.js`，若任一侧提到了另一侧没有的指标族则失败——与 `TestBootStepsFrontendInSyncWithBackend` 对启动阶段所用的是同一种防漂移手段。
- rolodex 抓取集成测试断言**所固定的 rolodex 镜像确实导出了** `monitoring.RolodexDashboardMetrics()` 中的每一个指标族，而 `TestControllerDashboardMetricsAreServed` 针对 `monitoring.ControllerDashboardMetrics()`、对着 controller 自身端点的一次真实抓取做同样的断言。两者都以 `# TYPE` 行匹配，这样名称是另一个前缀的指标族就无法为一个缺失的族背书。面板提到本机并不发出的指标族时，会渲染出一张空图，而那与一台空闲的机器无法区分。
...
- `MonitoringDashboard.test.jsx` 断言每个标签页挂载的都是**各不相同的** uPlot 组件。标签页列表里写的是组件本身，而不是由某个分支去挑一个，因为一个被遗漏了分支的标签页会落到 System 图表上——在错误的标题下渲染出一个真实的仪表盘，而那看上去像是正常工作。

controller 仪表盘是唯一依赖于一个可能缺席的抓取 job 的：`ports.ControllerMetrics` 由 `-listen` 的值推导而来，而 `WritePrometheusConfig` 在推导不出时会**略去**该 job，而不是猜一个地址。被略去的 job 意味着一个没有数据的仪表盘，而不是一个坏掉的仪表盘。
...
那些 uid 是常量（`OverviewDashboardUID`、`DNSDashboardUID`、`ControllerDashboardUID`），因为 Web UI 会深链它们。漂移的 uid 在任何地方都不会产生错误——Grafana 只会在 iframe 里呈现一个 "dashboard not found" 页面。

DNS 与 controller 仪表盘是**由面板规格构建并序列化**出来的（`src/monitoring/dashboard_dns.go`、`src/monitoring/dashboard_controller.go`），而不是像仍然如此的旧 overview 仪表盘那样拼接进一份 JSON 模板。仪表盘中畸形的 JSON 代价不是少一个面板；它会让置备失败，于是该仪表盘根本不会出现。面板的 target 携带对象形式的数据源引用（`{"type":"prometheus","uid":GrafanaDatasourceUID}`）——Grafana 13+ 无法解析 target 中的旧字符串形式，会在不报任何错误的情况下渲染 "No data"。
...
侧边栏导航中的监控标签打开一个仪表盘页面，页面上带有 **System / DNS / Controller 子标签页**，与其他每一个带子标签页的界面一样可通过 `?tab=system|dns|controller` 深链，因此有人在故障期间正在看的仪表盘能挺过一次刷新，也可以被分享成链接。未知的 `?tab=` 值会回退到 System，而不是什么也不渲染。这份标签页清单是同一个数组，既持有要挂载的 uPlot 组件，也持有展示相同面板的 Grafana uid，因此不可能出现某个标签页在一种后端有、在另一种没有的情况——而在那里写明组件本身、而不是按标签页的值去分支，正是防止某个被遗漏的分支把 System 图表渲染到另一个标签页标题之下的手段。
...
**安装器只交付两个镜像，其余由控制器自行获取。** `install.sh` 只把两个镜像引用写进它布置的单元文件——`quay.io/town/town`（systemcontroller）与 `quay.io/town/rolodex`——并且完全不把任何镜像内容烘焙进 squashfs。其余每一个系统服务镜像（UI、ingress、网络控制器、对象存储）都由控制器**在本机上**拉取：启动时经由 `coreBootImages` 交给 `parallelEnsureImages`，按需时经由 `POST /system-services/refresh`。这正是那些仓库必须允许匿名读取的原因：机器本身不携带任何registry 凭据，控制器中也没有任何代码会写出 `auth.json` 或执行 `podman login`。仓库私有的镜像就是这套设计取不到的镜像，而其表现形式是一个不断以 `unauthorized` 崩溃重启的单元，而不是任何会点明真正原因的东西。

**一个每日定时器运行的，正是 UI 上那个按钮所运行的同一套更新流程，而它随安装器一同交付。** `town-os-update.timer` 及其 service 位于 `../install/systemd/`，并在镜像构建期就被启用（`make/install.sh` 中的 `ENABLE_UNITS` 清单），因此机器从首次启动起就拥有它们，而不是要向一个必须先跑起来的控制器索取。该定时器在 `04:23` 触发，并向 `/system-services/refresh` 发起 POST——刻意复用同一个端点，而不是再实现一遍"拉取全部"，这样计划路径就不可能与手动路径发生漂移。单元只决定*何时*，控制器决定*做什么*。它是 `Persistent=true` 的，因此在 04:23 处于关机状态的机器会在下次启动时补做更新，而不是再等一天——对于关机时间多过开机时间的机器，这个补做就是它唯一会得到的更新。该时刻刻意避开 podman prune 的 `03:17`：一边回收未被引用的镜像、一边拉取新镜像，是一场毫无收益的竞态。

**定时器的 POST 不带凭据，这正是该路由为 `localhostOrAdmin` 的原因。** 该单元没有任何令牌可以出示，因此 `/system-services/refresh` **仅**接纳来自回环地址的未认证调用，而来自其他任何位置的调用仍需管理员权限——这与 `GET /metrics` 所依赖的豁免相同，理由也相同：源地址为 `127.0.0.0/8` 的报文无法从网络被路由到本机。因此生成的单元显式指向 `127.0.0.1`，而绝不指向本机的可路由地址；后者既会把一个未认证的 POST 送上局域网，又会在对端被拒绝。端口会跟随控制器的 `-listen` 地址，因此被重定位端口的控制器（集成测试框架为每次运行分配各自的端口）依然能够自我更新。

**`auto_update_enabled` 约束的是定时器，而不是操作者。** 定时器会用 `?scheduled=1` 标记自己的调用；只有带该标记的调用才会去查阅该设置。管理员按下更新按钮时总是会更新，因为一个名为"自动更新"的开关，对一次显式请求无话可说。被跳过的计划运行返回 `200` 与 `{"status":"skipped"}` 而非错误——定时器询问了是否该更新，并得到了"现在不用"这个有效答复，这不是该让 `systemctl status` 标红的失败。即便设置为关闭，定时器依然保持安装并运行，因此翻转该设置会在下一次触发时生效，无需改动单元，也不存在单元状态与设置各执一词的可能。

该设置默认为**开启**，且无法识别的值一律视为开启。关闭是一个封闭列表（`0`、`false`、`off`、`no`，不区分大小写）；其余一切——包括手滑的错字与读取失败的设置行——都让更新继续运行。这种不对称是刻意的：正因为安装器只交付两个镜像，一台停止拉取的机器就是一台永远无法获得其余大部分服务的机器，所以把"关闭"猜错的代价，远高于多拉取一次的代价。
...
`TOWN_OS_IMAGE_REFRESH=0`（以及 `false`/`no`/`off`）会关闭刷新 —— 包括自更新，它随后只会发现「什么都没变」 —— 此时固定标签的规则适用于所有镜像；缺失的镜像仍然会拉取。`make dev` 与两个测试容器都设置了它，因为它们运行的每个镜像都是特意从本地镜像缓存加载的 —— 在那里刷新会在每次启动时去 quay.io 取回一个本就特意放置的镜像。任何无法识别的值都保持刷新开启：一台机器悄悄不再获取自己的服务，比多拉取一次更糟。在容器上设置它只做了一半：那些容器里的 PID 1 是 systemd，所以测试单元的 `PassEnvironment` 白名单（`integration/testdata/town-os-systemcontroller.service`）也必须写上这个变量，否则它止步于 PID 1，而唯一需要它的那个进程 —— 系统控制器 —— 永远看不到。白名单当时没有写上，于是每一次测试与 dev 启动都在刷新：rolodex、Prometheus 与 node-exporter 在启动拉取集里从 quay.io 重新拉取，而测试脚本只给 `/status/ping` 120 秒作答，一次运行是否失败取决于上行链路。`TestHarnessEnvironmentReachesTheController` 现在从 `make/test.sh`、`make/dev.sh` 与 `system_port_env` 推导出必需的名字，集成测试则读取运行中控制器的 `/proc/<pid>/environ` 来证明它们确实到达。
...
`make dev` 也因此成了唯一可能被一个端口拦下来的目标：一台机器上只有一个 `127.0.0.2:53`，而想要它的东西不止一个——install 仓库的宿主 rolodex、另一个 checkout 的 dev 机器、某次被硬杀掉的 dev 运行遗留下来的系统服务容器。于是 `require_free_rolodex_dns`（`make/dns.sh`）会在本 checkout 自己的容器已被移除、而该地址仍被占用时拒绝启动，并指名该停掉什么。以前的重定向只是等待对该地址的一次 TCP 连接成功，可当应答者是别人的解析器时，这个连接同样会成功：本 checkout 的 rolodex 绑定失败，而宿主被指向一个对这台 dev 机器一无所知的 rolodex——从浏览器看去，与这台 dev 机器自己的 DNS 坏掉毫无区别。`wait_for_rolodex_dns` 现在要求两半都成立：单元处于 active（这是让解析器成为*我们的*那一半），以及地址已被绑定（这是让它可用的那一半，因为 `podman run` 一起来单元就 active 了，那时 rolodex 还没打开套接字）；并且它是**致命的**而非一句警告——解析器从未起来的 dev 机器算不上 dev 机器，而继续跑下去只会造出一台看起来已启动、宿主上每个 `.home` 名字却都跑去问 LAN 解析器的机器。重定向在控制器就绪后立刻发生，早于账号和 UI 服务器，因为那一行之后的一切都已经是一台起来了的 dev 机器。由 `TestDevDNSRefusesToStartWhenTheRolodexAddressIsTaken` 及若干 `TestDevDNSWait*` 测试覆盖，它们对 `ss`、`resolvectl` 与 `${SUDO}` 都打了桩——任何测试都不得碰宿主的解析器。

剩下的问题是：谁来渲染 dev 机器据以绑定的 `rolodex.yml`。没有人——这正是上面那个致命等待所揭出的 bug：rolodex 的 DNS 监听器只在启动时从那个文件里打开**一次**，而 Town OS 通过 gRPC 说什么都打不开一个；`ProgramRolodex` 拥有转发器、解析模式和两份阻断列表，唯独刻意不拥有 bind（`controller_rolodex_config.go`）。真实机器上由 install 镜像渲染它（`../install/scripts/rolodex-config.sh`，它唯一的写者）。`make dev` 不跑安装器，于是 `${STATE_DIR}/dev-rolodex` 是空的，而 `town-os-system--rolodex.service`——它的 `ExecStartPre` 创建的是**目录**——让 rolodex 以 `--config /data/rolodex.yml` 启动，指向一个从未存在过的文件。单元照样进入 active，因为容器本身起得好好的，缺的只是监听器；这正是那个等待既问地址、又不止问单元的原因。现在 `seed_dev_rolodex_config`（`make/dns.sh`）在容器挂载该目录之前于宿主上写好它：只绑回环——DNS 的 bind、指标监听器、控制器要拨的 gRPC 套接字、一对起步用的转发器——并且每次运行都重写，因为 `dev-rolodex` 要持久保存 `rolodex.db`，而旧 schema 的配置会比写下它的那个 checkout 活得更久。刻意在宿主上播种而不烤进镜像：同一个单元文件也随集成测试镜像发布，而在一个 `--net host` 的测试容器里，绑定 `127.0.0.2:53` 的 `rolodex.yml` 会在每次运行时绑掉**宿主的** `:53`。没有它时，每次启动还要额外付出两段无条件的三十秒等待——`Manager.WaitForDNSReady`，以及 `reconcileDNSAndNetworks` 里的 gRPC 套接字拨号循环——而这都发生在 `require_controller_ready` 给出的 120 秒预算之内，偏偏这次启动在彻底完成之前对 `/status/ping` 一律回 503。

同一次启动也决定了 `make dev` 在启动它之前必须往容器里放什么。每个生成的单元跑的 `podman run` 都**不带** `--pull=never`，所以一个不在内层存储里的镜像并不是一个会快速失败的错误——它是一次发生在 `podman run` 内部的镜像仓库拉取，而与此同时 `require_controller_ready` 的 120 秒和该服务自己的就绪等待都在倒数。ingress 与 pages 服务所用的 caddy 都不在那份清单里（caddy 在集成工装里也不在），于是 dev 机器在启动期间从 quay 和 Docker Hub 去取它们，而对应的 tar 早就躺在这个 checkout 的镜像缓存里。现在两者都会被加载，ingress 由 `ingress-image` 这个前置目标在本地构建。另有两样东西本不该被加载：**Grafana** 约 1.17 GB，且是那个*非*默认的监控后端，除非存储的后端是 `grafana`，否则启动根本不会运行它——因此除非 `DEV_LOAD_GRAFANA=1`，一律跳过；而 **gfeh** 不再是 `dev` 的前置目标，因为它是唯一内容来自仓库之外的本地镜像，因而每个日历日都要把整棵 `gfehd` 依赖树重新编译一遍，所有人都要付，无论用不用。`make gfeh-image` 会在 dev 中启用对象存储，正如 `make/dev.sh` 一直声称的那样。加载本身如今经由只读挂载在 `/image-cache` 的镜像缓存进行，而不是 `podman cp`，这省掉了每个 tar 的一整次写入；没有该挂载的容器仍会回退到拷贝。最后，systemcontroller 在 dev 镜像里**不再被 enable**：systemd 过去在 `podman run -d` 返回的那一瞬间就启动它，面对的是一个空的存储，于是买来一次注定失败的启动——整整 30 秒的 DNS 等待、对一切尚未加载之物的拉取、对着不存在的镜像启动单元、克隆每一个软件包仓库——这一切都被 `make/dev.sh` 在加载完成后发出的那次启动丢弃，而且这一切还在跟那次加载抢资源。rolodex 仍然保持 enable，因为它必须在控制器启动之前就已在服务，而它的单元本就是为熬过这段等待而写的。
...
- `INGRESS_METRICS_PORT` —— ingress 提供 Prometheus `/metrics` 端点的端口，只发布在 `127.0.0.1` 上（默认 `9146`）。它是上面两者之外的第三个监听器，出于与 rolodex 相同的理由需要自己的覆盖项；`ingressctl.MetricsAddrFor()` 是 unit 的 `-p` 规格与 Prometheus 抓取目标共同构建自的那一个函数，因此挪动它会同时挪动两端。参见 [ingress 的指标](#ingress-的指标)。
