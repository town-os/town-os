...
| `make gfeh-image`             | Construye localmente la imagen de almacenamiento de objetos (gfeh) como `localhost/town-os-gfeh:<INSTANCE_ID>` para las pruebas. Descarga la imagen de la cadena de herramientas de Rust, que se deja fuera de `BASE_IMAGES` a propósito porque ocupa ~1,5G y solo la necesita este objetivo. |
...
**Las compilaciones cruzadas preparan sus imágenes base en la arquitectura de destino.** El almacenamiento de podman guarda una sola imagen por `name:tag`, sin sitio para dos arquitecturas, así que una compilación cruzada reapunta una base a la arquitectura de destino y desplaza la copia de la arquitectura del host. La caché de imágenes indexa sus tar por arquitectura, de modo que la copia desplazada vuelve mediante una carga local en lugar de una descarga de red. `BASE_IMAGES_RUNTIME` — las bases que un Containerfile compilable de forma cruzada nombra con un `FROM` a secas, es decir, las etapas que se distribuyen — se prepara en la arquitectura de destino; el resto son imágenes de cadena de herramientas que todo Containerfile cruzado fija con `--platform=$BUILDPLATFORM` porque se ejecutan en el host y compilan de forma cruzada, así que se quedan en la arquitectura del host bajo cualquier `TARGET`. Cada rama de compilación prepara ella misma las bases que necesita, en lugar de depender de una pasada global.

### Publicación y Push

Todas las imágenes de publicación se suben a `quay.io/town/`. Todas las etiquetas que se suben están particionadas por arquitectura: cada host sube su arquitectura nativa como `rc.<fecha>-<arch>` / `rc.latest-<arch>` (candidatas a publicación), `release.<fecha>-<arch>` / `latest-<arch>` (publicaciones) o `<etiqueta>-<arch>` (una etiqueta personalizada mediante `make push-tag PUSH_TAG=<etiqueta>`), donde `<arch>` es la forma cruda de `uname -m` — `x86_64` o `aarch64`, *no* los nombres de plataforma OCI `amd64`/`arm64`. Los nombres simples (`rc.latest`, `latest`, las etiquetas con fecha y una etiqueta personalizada) existen únicamente como listas de manifiestos multiarquitectura ensambladas por `make manifest-rc` / `make manifest-release` / `make manifest-tag` después de que todas las arquitecturas hayan subido; un nombre simple nunca debe subirse como etiqueta de una sola arquitectura, ya que falla en la otra arquitectura con `exec format error`. Hasta que se ejecuta el objetivo de manifiesto correspondiente, el nombre simple no existe, que es el estado honesto y mejor que uno que resuelva a la arquitectura que subiera en último lugar.

**Una publicación cruzada se prueba de forma nativa y omite lo que no puede compilar de forma cruzada.** `release-build` ejecuta su fase de pruebas a través de `make release-test`, que limpia `TARGET` solo para esa recursión: `test-full` construye el banco de pruebas de integración y lo *ejecuta* en este host, así que cada una de esas ramas rechaza de plano un `TARGET` ajeno. Las pruebas validan el código fuente en la máquina que las ejecuta; la parte cruzada de una publicación son los artefactos que se construyen después. El runner de Proton es x86_64 por construcción — GE-Proton distribuye Wine de x86_64 y la imagen añade multiarch de i386 para ejecutar ejecutables de Windows de 32 bits, así que no hay *nada a lo que* compilar de forma cruzada. Por eso una publicación que no sea x86_64 lo descarta de los objetivos agregados, las ramas de push omiten etiquetarlo y sus listas de manifiestos se ensamblan solo sobre x86_64 en lugar de sobre cada entrada de `ARCHES` — se omite una imagen que se comporta exactamente como se diseñó, en vez de fallar por ella.
...
| `make release-proton-image` | Construye la imagen de publicación del runner de Proton (`quay.io/town/proton`). **Requiere `PROTON_ENABLED=1`**; en caso contrario el objetivo no existe. Solo x86_64 — rechaza cualquier otro `TARGET`. |
...
| `make release-test`         | Ejecuta `test-full` de forma nativa, con `TARGET` limpiado para esa recursión. `release-build` depende de esto en lugar de depender directamente de `test-full`, de modo que una publicación cruzada todavía puede probarse. |
| `make release-build`        | Descarga las imágenes, ejecuta `release-test` y después construye las imágenes de publicación. Incluye el runner de Proton cuando `PROTON_ENABLED=1` *y* la publicación es x86_64. |
...
| `make push-tag PUSH_TAG=x`  | Construye y sube todas las imágenes con una etiqueta personalizada, por arquitectura (`x-<arch>`), exactamente como hace `push-rc`. Incluye el runner de Proton cuando `PROTON_ENABLED=1`. |
| `make manifest-tag PUSH_TAG=x` | Ensambla y sube la lista de manifiestos multiarquitectura del nombre simple `x` a partir de las etiquetas por arquitectura. Ejecútalo una vez después de que todas las arquitecturas hayan subido. |
