...
Encima de esos van **18 variantes de país** -- `en-GB`, `en-AU`, `en-CA`, `en-IN`, `en-NZ`, `en-ZA`, `de-AT`, `de-CH`, `fr-BE`, `fr-CA`, `fr-CH`, `es-AR`, `es-MX`, `pt-PT`, `nl-BE`, `ar-AE`, `ar-EG`, `bn-IN` --, lo que da **48 configuraciones regionales que se pueden escoger** en total. Una variante no es una segunda traducción: hereda el catálogo de su idioma y enlista nada más las cadenas que ese país de veras dice distinto, de modo que un mensaje nuevo llega a todos los países en cuanto lo tiene su idioma base, y una corrección en una cadena francesa se hace una vez en vez de en cuatro archivos. Varias de esas listas de sobrescrituras están honestamente vacías. El inglés de Canadá conserva las grafías estadounidenses en *-ize*, y el español de Argentina es famoso por el *voseo* -- pero el voseo reemplaza al *tú* de confianza, y este catálogo le habla al lector de *usted* en todo momento, lo cual se lee igual en Buenos Aires que en Madrid. Una lista vacía de todos modos significa revisada, no olvidada. Una pantalla de idioma en la Configuración del Sistema presenta 21 idiomas comunes en su escritura nativa, con una lista expandible de 89 códigos específicos por país; lo que no tiene catálogo se muestra pero aparece deshabilitado.

La interfaz escoge tu idioma a partir de tu navegador (`navigator.languages`). Una coincidencia exacta gana primero, así que `de-CH` obtiene el alemán de Suiza en vez de plegarse al de Alemania. El chino se desambigua por escritura. Un país para el que no distribuimos nada se va a donde de veras se lee -- `es-CO` al español de México en vez del peninsular, `en-IE` al británico en vez del estadounidense -- y un `en` o un `pt` pelones se resuelven a un predeterminado con nombre en vez de a la variante que resulte cargarse primero. Una elección explícita se recuerda por navegador, de modo que la configuración global de idioma del equipo ya no sobrescribe lo que ve cada persona -- solo es el respaldo para un idioma del que Town OS no tiene catálogo.
...
| `make test-race`                | Corre el linter y las pruebas unitarias de Go bajo el detector de carreras. Más lento que `test` y solo de Go (bun no tiene equivalente); reporta los hallazgos para triarlos en vez de servir de barrera. |
...
| `make gfeh-image`             | Construye localmente la imagen de almacenamiento de objetos (gfeh) como `localhost/town-os-gfeh:<INSTANCE_ID>` para las pruebas. Baja la imagen de la cadena de herramientas de Rust, que se deja fuera de `BASE_IMAGES` a propósito porque pesa ~1.5G y solo la necesita este objetivo. |
...
**Las compilaciones cruzadas preparan sus imágenes base en la arquitectura de destino.** El almacenamiento de podman guarda una sola imagen por `name:tag`, sin lugar para dos arquitecturas, así que una compilación cruzada reapunta una base a la arquitectura de destino y desplaza la copia de la arquitectura del host. La caché de imágenes indexa sus tar por arquitectura, de modo que la copia desplazada regresa con una carga local en lugar de una descarga de red. `BASE_IMAGES_RUNTIME` — las bases que un Containerfile compilable de forma cruzada nombra con un `FROM` a secas, o sea las etapas que se distribuyen — se prepara en la arquitectura de destino; el resto son imágenes de cadena de herramientas que todo Containerfile cruzado fija con `--platform=$BUILDPLATFORM` porque corren en el host y compilan de forma cruzada, así que se quedan en la arquitectura del host bajo cualquier `TARGET`. Cada rama de compilación prepara ella misma las bases que necesita, en lugar de depender de una pasada global.

### Publicación y Push

Todas las imágenes de publicación se suben a `quay.io/town/`. Todas las etiquetas que se suben están particionadas por arquitectura: cada host sube su arquitectura nativa como `rc.<fecha>-<arch>` / `rc.latest-<arch>` (candidatas a publicación), `release.<fecha>-<arch>` / `latest-<arch>` (publicaciones) o `<etiqueta>-<arch>` (una etiqueta personalizada con `make push-tag PUSH_TAG=<etiqueta>`), donde `<arch>` es la forma cruda de `uname -m` — `x86_64` o `aarch64`, *no* los nombres de plataforma OCI `amd64`/`arm64`. Los nombres simples (`rc.latest`, `latest`, las etiquetas con fecha y una etiqueta personalizada) existen únicamente como listas de manifiestos multiarquitectura que ensamblan `make manifest-rc` / `make manifest-release` / `make manifest-tag` después de que todas las arquitecturas hayan subido; un nombre simple nunca debe subirse como etiqueta de una sola arquitectura, porque falla en la otra arquitectura con `exec format error`. Hasta que corre el objetivo de manifiesto correspondiente, el nombre simple no existe, que es el estado honesto y mejor que uno que resuelva a la arquitectura que haya subido al último.

**Una publicación cruzada se prueba de forma nativa y se salta lo que no puede compilar de forma cruzada.** `release-build` corre su fase de pruebas mediante `make release-test`, que limpia `TARGET` nada más para esa recursión: `test-full` construye el banco de pruebas de integración y lo *corre* en este host, así que cada una de esas ramas rechaza de plano un `TARGET` ajeno. Las pruebas validan el código fuente en la máquina que las corre; la parte cruzada de una publicación son los artefactos que se construyen después. El runner de Proton es x86_64 por construcción — GE-Proton trae Wine de x86_64 y la imagen agrega multiarch de i386 para correr ejecutables de Windows de 32 bits, así que no hay *nada a lo que* compilar de forma cruzada. Por eso una publicación que no sea x86_64 lo descarta de los objetivos agregados, las ramas de push se saltan etiquetarlo y sus listas de manifiestos se ensamblan solo sobre x86_64 en lugar de sobre cada entrada de `ARCHES` — se salta una imagen que se comporta exactamente como se diseñó, en vez de fallar por ella.
...
| `make release-proton-image` | Construye la imagen de publicación del runner de Proton (`quay.io/town/proton`). **Requiere `PROTON_ENABLED=1`**; si no, el objetivo no existe. Solo x86_64 — rechaza cualquier otro `TARGET`. |
...
| `make release-gfeh-image`   | Construye la imagen de publicación del almacenamiento de objetos (`quay.io/town/gfeh`).                            |
| `make release-test`         | Corre `test-full` de forma nativa, con `TARGET` limpiado para esa recursión. `release-build` depende de esto en lugar de depender directo de `test-full`, así que una publicación cruzada todavía puede probarse. |
| `make release-build`        | Baja las imágenes, corre `release-test` y después construye las imágenes de publicación. Incluye el runner de Proton cuando `PROTON_ENABLED=1` *y* la publicación es x86_64. |
| `make push`                 | Alias de `push-rc`.                                                                                                |
| `make push-rc`              | Sube todas las imágenes (controlador del sistema, interfaz, controlador de red, ingress, almacenamiento de objetos; Proton cuando `PROTON_ENABLED=1`) como candidatas a publicación por arquitectura (`rc.<fecha>-<arch>` + `rc.latest-<arch>`). |
...
| `make push-gfeh-rc`         | Sube solo la imagen del almacenamiento de objetos como candidata a publicación por arquitectura (`rc.<fecha>-<arch>` + `rc.latest-<arch>`). |
| `make push-gfeh-release`    | Sube solo la imagen del almacenamiento de objetos como publicación por arquitectura (`release.<fecha>-<arch>` + `latest-<arch>`). |
| `make push-tag PUSH_TAG=x`  | Construye y sube todas las imágenes con una etiqueta personalizada, por arquitectura (`x-<arch>`), igualito que `push-rc`. Incluye el runner de Proton cuando `PROTON_ENABLED=1`. |
| `make manifest-tag PUSH_TAG=x` | Ensambla y sube la lista de manifiestos multiarquitectura del nombre simple `x` a partir de las etiquetas por arquitectura. Córrelo una vez después de que todas las arquitecturas hayan subido. |
